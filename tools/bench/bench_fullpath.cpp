...
int lnfs::bench::fullpath_main(int argc, char** argv) {
    ...
    std::vector<std::byte> fh;
    {
        ...
        bool done = false;
        spawn(
            [](backend::MemoryBackend* m, core::ExportTable* exports, core::FileHandleCodec* handles,
               std::vector<std::byte>* out, std::mutex* mu, std::condition_variable* cv, bool* done,
               bool read_proc) -> Task<void> {
    ...
    printf(
        "bench_fullpath: proc=%s ring=%s reactors=%d conns=%d pipeline=%d -> "
        "%llu calls in %.3fs = %.0f rps\n",
        read_proc ? "READ4k" : "GETATTR", rt.ring_kind().c_str(), reactors, conns, pipeline,
