...
int lnfs::bench::nullrpc_main(int argc, char** argv) {
    ...
    Dispatcher disp;
    disp.add({kProg, 3, 3, nullptr, [](void*, ConnCtx& c, RpcCall& call, const Cred&) -> Task<void> {
    ...
    double rps = res.rps();
    printf(
        "bench_nullrpc: ring=%s reactors=%d conns=%d pipeline=%d -> "
        "%llu calls in %.3fs = %.0f rps%s\n",
        rt.ring_kind().c_str(), reactors, conns, pipeline, (unsigned long long)res.completed, res.seconds, rps,
