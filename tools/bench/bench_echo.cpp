...
int lnfs::bench::echo_main(int argc, char** argv) {
    ...
    printf(
        "bench_echo: ring=%s reactors=%d conns=%d pipeline=%d payload=%zu -> "
        "%llu records in %.3fs = %.0f rps\n",
        rt.ring_kind().c_str(), reactors, conns, pipeline, payload, (unsigned long long)res.completed, res.seconds,
        res.rps());
