...
state::StateMgr::Config::StableStore cluster_stable_store(ClusterStore& store) {
    return {
        .load = [&store](uint32_t fsid) -> std::vector<std::string> {
            auto listed = fsid == 0 ? store.list_clients() : store.list_clients(fsid);
            if (!listed) {
                LNFS_WARN("cannot read the cluster reclaim list (fsid {}): {}", fsid, errno_name(listed.error()));
                return {};
            }
...
ProtocolStack::ProtocolStack(const core::ServerConfig& cfg, CoreState& core)
    : drc({.ttl = std::chrono::milliseconds(cfg.drc_ttl_ms), .max_memory = cfg.drc_mem}),
      ...
      state({.boot_epoch = core.epoch,
             ...
             .native_locks = {.manager = [exports = core.exports.get()](uint32_t fsid) -> backend::LockMgr* {
             ...
             .stable = core.cluster ? cluster_stable_store(*core.cluster) : state::StateMgr::Config::StableStore{},
...
ServerIdentity derive_server_identity(const core::ServerConfig& cfg, const core::ClusterConfig& cluster) {
    if (cluster.enabled) {  // config validation rejects explicit owner/scope here
        ...
        if (core::cluster_active_active(cluster)) return {derived + ":" + core::cluster_node_name(cluster), derived};
...
void ProtocolStack::enable_v4(const core::ServerConfig& cfg, const core::ClusterConfig& cluster, CoreState& core,
                              rt::Runtime& runtime) {
    ...
    nfs4.emplace(*core.exports, core.key, locks, state, std::move(identity.owner), std::move(identity.scope),
                 core::cluster_active_active(cluster));
