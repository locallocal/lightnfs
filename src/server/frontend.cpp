...
void Management::detach() {
    ...
    if (!plane->detach(std::chrono::seconds(5)))
        LNFS_WARN("ctl: {} command(s) still using the data plane after detach; continuing", plane->pins());
}
...
std::optional<Frontend> Frontend::start(const core::ServerConfig& cfg, rt::Runtime& runtime, ProtocolStack& stack,
                                        CoreState& core, Management& mgmt) {
    ...
    if (cfg.tls_mode != "off") {
        ...
        tls_cfg.policy = cfg.tls_mode == "required" ? transport::TlsPolicy::kRequired : transport::TlsPolicy::kOptional;
        ...
        LNFS_INFO("RPC-over-TLS enabled (mode={}, mutual={})", cfg.tls_mode, cfg.tls_require_client_cert);
    }

    auto nfs_listener = transport::Listener::create(cfg.port, transport_cfg, stack.dispatcher, runtime, cfg.bind);
    auto mount_listener =
        transport::Listener::create(cfg.mount_port, transport_cfg, stack.dispatcher, runtime, cfg.bind);
    if (!nfs_listener || !mount_listener) {
        LNFS_ERROR("cannot create listeners: nfs={} mount={}", nfs_listener ? "ok" : errno_name(nfs_listener.error()),
    ...
    fe.pool_metric = obs::register_text_provider([nfs = fe.nfs.get(), mount = fe.mount.get()](std::string& out) {
        out += std::format(
    ...
    auto drain = [nfs = fe.nfs.get(), mount = fe.mount.get(), draining = fe.draining]() -> std::string {
    ...
    *fe.plane = DataPlane{.exports = core.exports.get(),
    ...
    if (cfg.rpcbind) {
        ...
        if (!nfs_reg || !mount_reg) LNFS_WARN("rpcbind registration unavailable; use explicit port/mountport options");
    }
    return fe;
}
...
bool Frontend::drain_connections(std::chrono::milliseconds grace, std::chrono::milliseconds close_timeout) {
    auto& registry = transport::ConnRegistry::instance();
    if (!registry.wait_idle(grace)) {
        size_t closed = registry.close_all();
        LNFS_INFO("data plane: {} connection(s) still open after {} ms grace, shutting them down", closed,
                  grace.count());
