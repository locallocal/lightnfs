...
Result<CatalogBoot> load_catalog_exports(ClusterStore& store, core::Config& local, std::string* why) {
    local.exports.clear();
    ...
    if (!doc) return fail("cannot read the catalog: " + std::string(errno_name(doc.error())), doc.error());
    if (!*doc) {
    ...
    if (catalog->meta.version != version)
        return fail(std::format("catalog v{}: its [catalog] version says {}", version, catalog->meta.version),
                    errno_from(EINVAL));
    std::string reason;
    if (auto ok = core::validate_catalog(*catalog, core::cluster_active_active(local.cluster), &reason); !ok)
    ...
    LNFS_INFO("catalog v{}: {} export(s) for this host ({} in the catalog), digest {}", version, local.exports.size(),
              boot.catalog.exports.size(), boot.digest);
    return boot;
}
...
void record_catalog_applied(ClusterStore& store, const std::string& node, uint64_t version, const std::string& digest,
    ...
    if (auto put = store.put_catalog_applied(applied); !put)
        LNFS_WARN("catalog: cannot record catalog.{} = {} {}: {}", node, version, status, errno_name(put.error()));
}
