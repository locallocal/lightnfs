...
class MainLoop {
 public:
    ...
    std::optional<std::string> call(std::function<std::string()> fn, std::chrono::milliseconds timeout) {
    ...
    bool on_loop_thread() const { return std::this_thread::get_id() == loop_thread_.load(std::memory_order_acquire); }
