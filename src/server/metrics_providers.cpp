...
void append_v4_state(std::string& out, state::StateMgr& state) {
    ...
    out += std::format(
        ...
        s.clients, s.sessions, s.opens, s.seq_new, s.seq_replay, s.seq_misordered, s.seq_waits, s.grace ? 1 : 0,
        s.grace_remaining, s.files, s.courtesy, s.lease_expirations, s.reclaim_conflict, s.reclaim_timeout,
        s.reclaim_forced, s.share_denied, s.open_merges, s.lock_states, s.lock_segments, s.lock_owners, s.lock_denied,
        s.delegs, s.deleg_grants, s.deleg_recalls, s.deleg_returns, s.deleg_revokes, s.cb_lock_notifies,
        s.native_lock_denied, s.native_lock_errors, s.native_lock_reclaim_delays);
}

// ---- per-backend cache / jukebox / lock-handle counters --------------------------

// Gluster backend caches + jukebox/lock-descriptor counters (design 06 §6.6).
void append_gluster(std::string& out, const std::string& labels, const backend::GlusterBackend& g) {
    auto s = g.stats();
    out += std::format(
        ...
        labels, s.fd_hits, s.fd_misses, s.fd_upgrades, s.fd_evictions, s.fd_entries, s.obj_hits, s.obj_misses,
        s.obj_entries, s.jukebox, s.lock_fds);
}

// CephFS backend caches + jukebox/blocklist/lock-handle counters (design 06 §6.8).
void append_cephfs(std::string& out, const std::string& labels, const backend::CephBackend& c) {
    auto s = c.stats();
    out += std::format(
...
void append_lustre(std::string& out, const std::string& labels, const backend::LustreBackend& l) {
    auto s = l.stats();
    out += std::format(
...
void append_local(std::string& out, const std::string& labels, const backend::LocalBackend& local) {
    ...
    out += std::format(
        ...
        labels, s.hits, s.misses, s.upgrades, s.evictions, s.overflows, s.entries, s.path_hits, s.path_misses,
        s.path_entries);
}
...
void append_exports(std::string& out, core::ExportTable& exports) {
    ...
    for (const auto& entry : set->entries) {
        ...
        out += std::format(
            ...
            labels, em.read_bytes.load(), em.write_bytes.load(), em.read_ops.load(), em.write_ops.load());
...
void append_runtime(std::string& out, rt::Runtime& runtime) {
    ...
    for (int c = 0; c < rt::kOffloadClasses; ++c) {
        out += std::format(
            ...
            kCls[c], s.depth[c], kCls[c], s.submitted[c], kCls[c], s.completed[c], kCls[c], s.deferred[c]);
    }
    ...
    obs::append_histogram(out, "lightnfs_reactor_loop_duration_seconds", "", rt::Reactor::kLoopBoundsUs, agg, sum_us);
}
...
MetricsRegistration register_metrics_providers(const MetricsSources& src) {
    ...
    if (src.nfs4)
        handles.push_back(
            obs::register_text_provider([engine = src.nfs4](std::string& out) { append_v4_moved(out, *engine); }));
    handles.push_back(obs::register_text_provider([&drc = src.drc](std::string& out) { append_drc(out, drc); }));
    handles.push_back(
        obs::register_text_provider([&state = src.state](std::string& out) { append_v4_state(out, state); }));
    handles.push_back(
        obs::register_text_provider([&exports = src.exports](std::string& out) { append_exports(out, exports); }));
    handles.push_back(
        obs::register_text_provider([&runtime = src.runtime](std::string& out) { append_runtime(out, runtime); }));
