...
std::string cluster_status(const ClusterController& cc, const Result<std::vector<std::string>>& peers,
    ...
    int64_t age_ms =
        snap.fence ? std::chrono::duration_cast<std::chrono::milliseconds>(now - snap.fence_seen).count() : -1;
...
std::string cluster_fs_status(const FsClusterController& fc, const DataPlane* dp,
    ...
    if (json) {
        ...
        for (const auto& fs : snap) {
            ...
            rows += std::format(
                ...
                owned && !fs.view.address.empty() ? std::format("\"{}\"", json_escape(fs.view.address)) : "null",
                fs.view.fs_epoch, fs.fence ? std::to_string(wall_ms - (fs.fence->expires_at_ms - ttl_ms)) : "null",
                fs.fence ? std::to_string(fs.fence->expires_at_ms - wall_ms) : "null", grace_left(fs), fs.takeovers,
                fs.fence_lost, fs.activation_failures);
        }
        return std::format(
    ...
    for (const auto& fs : snap) {
        ...
        out += std::format(
            ...
            "activation_failures={}\n",
            fs.fsid, FsClusterController::fs_role_label(fs), text_list(fs.nodes), owned ? fs.view.node : "none",
            owned && !fs.view.address.empty() ? fs.view.address : "-", fs.view.fs_epoch,
            fs.fence ? std::to_string(wall_ms - (fs.fence->expires_at_ms - ttl_ms)) : "-",
            fs.fence ? std::to_string(fs.fence->expires_at_ms - wall_ms) : "-", grace_left(fs), fs.takeovers,
            fs.fence_lost, fs.activation_failures);
    }
    return out;
}
...
std::string cluster_fs_exports(const FsClusterController& fc, std::string_view node,
    ...
    if (!known)
        return cluster_error(json, std::format("unknown node {} (not registered, not in any export's nodes)", node));
    ...
    if (alive) alive_text = std::find(alive->begin(), alive->end(), node) != alive->end() ? "yes" : "no";
    if (json) {
        std::string rows;
...
std::string CtlServer::answer(const CtlDeps& deps, std::string_view command) {
    ...
    if (cmd.name() == "status") {
        auto uptime =
            deps.started.time_since_epoch().count() == 0
                ? 0
                : std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - deps.started)
                      .count();
        ...
        if (json)
            return std::format(
                ...
                LIGHTNFS_VERSION, uptime, json_escape(role), conns, draining, exports, grace, grace_left);
        return std::format(
            "version={} uptime={}s role={} connections={} draining={} exports={} grace={} "
            "grace_remaining={}s\n",
            LIGHTNFS_VERSION, uptime, role, conns, draining ? 1 : 0, exports, grace ? 1 : 0, grace_left);
    }
    if (cmd.name() == "metrics") return obs::prometheus_text();
    if (cmd.name() == "dump-errors") return json ? obs::dump_error_replies_json() : obs::dump_error_replies();
    ...
    if (cmd.name() == "conns") {
        ...
        if (json) {
            ...
            for (size_t i = 0; i < conns.size(); ++i)
                out += std::format("{}{{\"id\":{},\"peer\":\"{}\",\"age_s\":{}}}", i ? "," : "", conns[i].id,
                                   json_escape(conns[i].peer), conns[i].age_s);
            out += "]\n";
            return out;
        }
        for (const auto& c : conns) out += std::format("id={} peer={} age={}s\n", c.id, c.peer, c.age_s);
    ...
    if (cmd.name() == "drc") {
        ...
        return std::format("inserts={} replays={} waits={} evictions={} entries={} bytes={}\n", s.inserts, s.replays,
                           s.waits, s.evictions, s.entries, s.bytes);
    }
            ...
            for (const auto& entry : set->entries) {
                if (auto* g = dynamic_cast<backend::GlusterBackend*>(entry->backend.get())) {
                    ...
                    if (json) {
                        out += std::format(
                            ...
                            emitted ? "," : "", json_escape(entry->path), s.fd_hits, s.fd_misses, s.fd_upgrades,
                            s.fd_evictions, s.fd_entries, s.obj_hits, s.obj_misses, s.obj_entries, s.jukebox,
                            s.lock_fds);
                    } else {
                        out += std::format(
                            "export={} backend=gluster hits={} misses={} upgrades={} evictions={} "
                            "entries={} obj_hits={} obj_misses={} obj_entries={} jukebox={} lock_fds={}\n",
                            entry->path, s.fd_hits, s.fd_misses, s.fd_upgrades, s.fd_evictions, s.fd_entries,
                ...
                if (json) {
                    out += std::format(
                        ...
                        emitted ? "," : "", json_escape(entry->path), s.hits, s.misses, s.upgrades, s.evictions,
                        s.overflows, s.entries, s.path_hits, s.path_misses, s.path_entries);
                } else {
                    out += std::format(
                        "export={} hits={} misses={} upgrades={} evictions={} overflows={} "
                        "entries={} path_hits={} path_misses={} path_entries={}\n",
                        entry->path, s.hits, s.misses, s.upgrades, s.evictions, s.overflows, s.entries, s.path_hits,
    ...
    if (cmd.name() == "cluster" && !deps.cluster && !deps.fs_cluster) return cluster_not_enabled(json);
...
rt::Task<std::string> CtlServer::answer_async(const CtlDeps& deps, std::string command,
    ...
    if (cmd.name() == "state") {
        ...
        if (json) {
            ...
            co_return std::format(
                ...
                s.clients, s.sessions, s.opens, s.files, s.courtesy, s.grace, s.grace_remaining, s.lease_expirations,
                s.reclaim_conflict, s.reclaim_timeout, s.reclaim_forced, s.share_denied, s.open_merges, s.lock_states,
        ...
        std::string out = std::format(
            ...
            s.clients, s.sessions, s.opens, s.files, s.courtesy, s.grace ? 1 : 0, s.grace_remaining,
            s.lease_expirations, s.reclaim_conflict, s.reclaim_timeout, s.reclaim_forced, s.share_denied, s.open_merges,
            s.lock_states, s.lock_segments, s.lock_owners, s.lock_denied, s.delegs, s.deleg_grants, s.deleg_recalls,
    ...
    if (cmd.name() == "cluster" && deps.cluster) {
            ...
            if (took) {
                ...
                if (json)
                    co_return std::format("{{\"takeover\":true,\"forced\":{},\"epoch\":{},\"role\":\"{}\"}}\n", force,
                                          snap.epoch, role_name(snap.role));
                co_return std::format("takeover started: node={} epoch={} role={}{}\n", snap.node, snap.epoch,
                                      role_name(snap.role), force ? " (forced)" : "");
            }
            auto snap = cc.snapshot();
            if (snap.role != Role::kStandby)
                co_return cluster_error(json, std::format("not standby (role={})", role_name(snap.role)));
            if (took.error() == errno_from(EBUSY))
                co_return cluster_error(json, std::format("fence held by {} (retry with --force to take it)",
        ...
        if (sub == "standby") {
            ...
            co_return cluster_error(json, std::format("not active (role={})", role_name(cc.snapshot().role)));
        }
    ...
    if (cmd.name() == "cluster" && deps.fs_cluster) {
        ...
        if (sub == "migrate") {
            ...
            auto moved = co_await rt::offload([&fc, fsid, &target] { return fc.request_migrate(*fsid, target); });
            if (moved) {
                if (json)
            ...
            if (moved.error() == errno_from(EPERM)) {
                ...
                co_return cluster_error(json,
                                        std::format("fsid {} not active here (role={}, owner={})", *fsid, role, owner));
            }
            if (moved.error() == errno_from(EHOSTDOWN))
                co_return cluster_error(
                    json, std::format("node {} is not a live gateway (no registration or heartbeat)", target));
        ...
        if (sub == "takeover" || sub == "standby") {
            ...
            if (sub == "takeover") {
                auto took = co_await rt::offload([&fc, fsid, force] { return fc.request_takeover(*fsid, force); });
                if (took) {
                    std::string role = fs_role_of(fc, *fsid);
                    uint64_t fs_epoch = 0;
                    for (const auto& fs : fc.snapshot())
                ...
                if (took.error() == errno_from(EBUSY)) {
                    ...
                    co_return cluster_error(
                        json, std::format("fsid {} fence held by {} (retry with --force to take it)", *fsid, holder));
                }
                co_return cluster_error(json, std::format("takeover failed: {}", errno_name(took.error())));
            }
...
Result<std::unique_ptr<CtlServer>> CtlServer::create(const std::string& socket_path, CtlDeps deps) {
    sockaddr_un addr{};
...
void CtlServer::request_stop() {
    stop_.request();
    if (Reactor* r = run_reactor_.load()) spawn([](int fd) -> Task<void> { co_await uring_cancel_fd(fd); }(fd_), *r);
}
...
rt::Task<void> CtlServer::serve(int cfd) {
    ...
    if (::getsockopt(cfd, SOL_SOCKET, SO_PEERCRED, &peer, &plen) != 0 || (peer.uid != 0 && peer.uid != ::geteuid())) {
...
Result<std::unique_ptr<MetricsHttp>> MetricsHttp::create(uint16_t port, const std::string& bind_addr,
...
bool MetricsHttp::allowed(const sockaddr_storage& peer) const {
    ...
    return std::any_of(allow_.begin(), allow_.end(), [&](const core::Cidr& c) { return c.contains(peer); });
}
