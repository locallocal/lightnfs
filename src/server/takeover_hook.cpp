...
std::vector<std::string> hook_environment(const backend::ClusterIdentity& id, std::string_view prev_node, uint32_t fsid,
                                          std::string_view reason) {
    static constexpr std::string_view kOurs[] = {
        "LNFS_CLUSTER_ID=", "LNFS_NODE=", "LNFS_EPOCH=", "LNFS_PREV_NODE=", "LNFS_FSID=", "LNFS_REASON="};
    ...
    env.push_back("LNFS_REASON=" + std::string(reason));
    return env;
}
...
Result<void> run_takeover_hook(const std::string& path, const backend::ClusterIdentity& id, std::string_view prev_node,
                               std::chrono::milliseconds timeout, uint32_t fsid, std::string_view reason) {
    auto env = hook_environment(id, prev_node, fsid, reason);
    ...
    LNFS_INFO(
        "cluster: takeover hook {} started (pid {}, node={} epoch={} prev={} fsid={} "
        "reason={})",
        path, pid, id.node, id.epoch, prev_node, fsid, reason);
    ...
    for (;;) {
        ...
        if (got < 0 && errno != EINTR) {
            LNFS_WARN("cluster: takeover hook {}: waitpid failed: {}", path, errno_name(errno_from(errno)));
