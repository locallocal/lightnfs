...
//    table (a published, snapshot-able set, plan 12 B1), the file-handle HMAC codec,
//    the boot epoch (write verifier and stateid epoch source).
//  - ProtocolStack: the v3/MOUNT engines, the v4.1/4.2 engine with its state manager,
//    the DRC and the per-object lock registry, all wired onto one RPC dispatcher.  Members are
//    declared in dependency order and live until run_server returns; main.cpp only builds and tears
//    this down.
...
// Durable identity: export table, handle HMAC key, boot epoch.
