...
Result<void> call_portmapper(uint32_t procedure, uint32_t program, uint32_t version, uint16_t port) {
    ...
    uint32_t xid = static_cast<uint32_t>(std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    ssize_t sent = sendto(fd, request.data(), n, 0, reinterpret_cast<sockaddr*>(&address), sizeof(address));
    ...
    if (get32(reply.data()) != xid || get32(reply.data() + 4) != 1 || get32(reply.data() + 8) != 0)
