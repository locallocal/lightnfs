...
CatalogApplier::CatalogApplier(Deps deps, uint64_t applied, core::Catalog catalog,
                               std::string digest)
    : deps_(std::move(deps)),
      applied_(applied),
...
void CatalogApplier::poll() {
  ...
  if (!doc) {
    ...
    if (poll_failures_++ % 60 == 0)
      LNFS_WARN("catalog: cannot read the catalog: {}", errno_name(doc.error()));
    return;
  }
  ...
  bool post_apply = false, post_retire = false;
  {
    ...
    if (pending_ && pending_ == failed_version_ && polls_until_retry_ > 0 &&
...
Result<uint64_t> CatalogApplier::apply_latest() {
  ...
  record.applied_at_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
  if (auto put = deps_.store.put_catalog_applied(record); !put)
    LNFS_WARN("catalog: cannot record the failure in catalog.{}: {}", deps_.node,
...
Result<uint64_t> CatalogApplier::apply_locked_pipeline(std::string& why) {
  ...
  if (!next)
    return fail(std::format("catalog v{} does not parse: {}", version, errno_name(next.error())),
                next.error());
  if (next->meta.version != version)
    return fail(
        std::format("catalog v{}: its [catalog] version says {}", version, next->meta.version),
        errno_from(EINVAL));
  std::string reason;
  ...
  if (auto ok = core::validate_config(merged); !ok) {
    ...
    for (const auto& exp : merged.exports) {
      ...
      if (auto one = core::validate_config(probe); !one) {
        blamed =
            std::format("export fsid={} ({}): {}", exp.fsid, exp.path, errno_name(one.error()));
        break;
      }
    }
  ...
  for (auto& cfg : plan.add) {
    ...
    if (!factory) {
      stop_started();
      return fail(std::format("catalog v{}: export fsid={}: backend \"{}\" is not in this build",
                              version, cfg.fsid, cfg.backend),
                  errno_from(ENODEV));
    }
    ...
    if (deps_.start_backend) {
      if (auto ok = deps_.start_backend(*made); !ok) {
        stop_started();
        return fail(std::format("catalog v{}: export fsid={} ({}): backend failed to start: {}",
                                version, cfg.fsid, cfg.path, errno_name(ok.error())),
                    ok.error());
      }
    }
  ...
  if (!published) {
    stop_started();
    return fail(std::format("catalog v{}: the export table refused the change: {}", version,
                            errno_name(published.error())),
                published.error());
  }
  ...
  LNFS_INFO("catalog v{} applied: {} added, {} updated, {} removed; {} export(s) served, digest {}",
            version, adds, updates, removes, (*published)->entries.size(), digest);
  CatalogApplied record{deps_.node, version, digest, 0, "ok"};
  record.applied_at_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
  if (auto put = deps_.store.put_catalog_applied(record); !put)
    LNFS_WARN("catalog: cannot record catalog.{} = {} ok: {}", deps_.node, version,
              errno_name(put.error()));
  return version;
}
