...
Result<backend::SetAttr> decode_sattr(xdr::XdrDec& dec) {
    ...
    auto decode_time_how = [&](backend::SetAttr::TimeHow& how, backend::Timespec& value) -> Result<void> {
...
void CreateArgs::encode(xdr::XdrEnc& enc) const {
    where.encode(enc);
    enc.u32(mode);
    if (mode == kCreateExclusive)
        enc.opaque_fixed(verf);
    else
        encode_sattr(enc, attrs);
}
...
void encode_wcc(xdr::XdrEnc& enc, const std::optional<WccPre>& pre, const std::optional<backend::Attr>& post,
                uint64_t fsid) {
...
void encode_post_attr(xdr::XdrEnc& enc, const std::optional<backend::Attr>& attr, uint64_t fsid) {
