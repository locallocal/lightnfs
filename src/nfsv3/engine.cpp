...
void begin_result(xdr::XdrEnc& enc, ConnCtx& ctx, const RpcCall& call, Status status) {
    ...
    if (status != Status::kOk) {
        obs::Metrics::instance().v3_errors[call.proc % obs::Metrics::kV3Procs].fetch_add(1, std::memory_order_relaxed);
        obs::record_error_reply(ctx.peer.to_string(), obs::v3_proc_name(call.proc), call.xid,
                                static_cast<uint32_t>(status));
    }
    if (log_enabled(LogLevel::kDebug))
        LNFS_DEBUG("v3 {} xid={:#x} peer={} st={}", obs::v3_proc_name(call.proc), call.xid, ctx.peer.to_string(),
                   static_cast<uint32_t>(status));
}
...
struct ProcTimer {
    ProcTimer(uint32_t p, const ConnCtx& c, uint32_t x) : proc(p % obs::Metrics::kV3Procs), ctx(&c), xid(x) {}
    ~ProcTimer() {
        ...
        if (auto thr = obs::slow_request_threshold_us(); thr != 0 && us >= thr)
            LNFS_WARN("v3 slow request proc={} xid={:#x} peer={} dur={}us", obs::v3_proc_name(proc), xid,
                      ctx->peer.to_string(), us);
    }
    uint32_t proc;
    const ConnCtx* ctx;
    uint32_t xid;
...
size_t dir_entry_size(std::string_view name) {
    return 24 + aligned4(name.size());
}
...
backend::Stability stability_from_wire(uint32_t stable) {
    switch (stable) {
        case kUnstable:
            return backend::Stability::kUnstable;
        case kDataSync:
            return backend::Stability::kDataSync;
        default:
            return backend::Stability::kFileSync;
    }
}
...
void encode_wcc_sample(xdr::XdrEnc& enc, const core::ChangeSample& sample, uint64_t fsid) {
    encode_wcc(enc, wcc_pre(sample.before), sample.after, fsid);
}

void encode_wcc_unchanged(xdr::XdrEnc& enc, const std::optional<backend::Attr>& attr, uint64_t fsid) {
    encode_wcc(enc, wcc_pre(attr), attr, fsid);
}

void encode_wcc_none(xdr::XdrEnc& enc) {
...
rt::Task<void> Engine::dispatch(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred) {
    ...
    if (drc_ && drc_cached(proc)) {
        ...
        if (!error && !cap.empty())
            co_await drc_->complete(key, std::move(cap));
        else
            co_await drc_->abort(key);
...
rt::Task<void> Engine::proc_getattr(ConnCtx& ctx, RpcCall& call, const rpc::Cred&, Capture* cap) {
    ...
    if (!resolved) {
        ...
        if (!attr)
            begin_result(enc, ctx, call, core::to_v3(attr.error(), Proc::kGetattr));
        else {
...
rt::Task<void> Engine::proc_access(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
...
rt::Task<void> Engine::proc_readlink(ConnCtx& ctx, RpcCall& call, const rpc::Cred&, Capture* cap) {
    auto args = FileHandle::decode(call.args);
    if (!args || !call.args.at_end()) {
        co_await rpc::Dispatcher::reply_garbage_args(ctx, call.xid);
        co_return;
...
rt::Task<void> Engine::proc_read(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    auto read = co_await resolved->obj->read(open, args->offset, std::span<std::byte>(data.data(), count), eof);
    auto attr = co_await resolved->obj->getattr();
    if (!read) {
...
rt::Task<void> Engine::proc_readdir(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    Proc proc = static_cast<Proc>(call.proc);
    ...
    if (!page) {
        Status status =
            cookie != 0 && page.error() == errno_from(EINVAL) ? Status::kBadCookie : core::to_v3(page.error(), proc);
...
rt::Task<void> Engine::proc_fs_query(ConnCtx& ctx, RpcCall& call, const rpc::Cred&, Capture* cap) {
    Proc proc = static_cast<Proc>(call.proc);
    auto args = FileHandle::decode(call.args);
    if (!args || !call.args.at_end()) {
        co_await rpc::Dispatcher::reply_garbage_args(ctx, call.xid);
...
rt::Task<void> Engine::proc_commit(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
...
rt::Task<void> Engine::proc_setattr(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    if (args->guard && before &&
        (before->ctime.sec != args->guard_ctime.sec || before->ctime.nsec != args->guard_ctime.nsec)) {
...
rt::Task<void> Engine::proc_write(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    uint32_t count =
        std::min<uint32_t>(std::min<uint64_t>(args->count, args->data_len), resolved->exp->backend->limits().max_write);
    ...
    auto written = co_await resolved->obj->write(open, args->offset, std::span<const iovec>(iov.data(), iov.size()),
...
rt::Task<void> Engine::proc_create(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    if (args->mode == kCreateExclusive) {
        ...
        if (!created && created.error() == errno_from(EEXIST) && args->mode == kCreateUnchecked) {
...
rt::Task<void> Engine::proc_mkdir(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
...
rt::Task<void> Engine::proc_symlink(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    co_await guard.enter({dir->obj, dir->oid});
    auto created = co_await dir->obj->symlink(guard.cred(), args->where.name, args->target, args->attrs);
    co_await guard.finish();
    if (!created) {
...
rt::Task<void> Engine::proc_mknod(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    bool special = device || args->type == backend::FType::kSock || args->type == backend::FType::kFifo;
    Status precheck = verdict_status(guard.precheck({args->where.name}));
    if (precheck == Status::kOk) {
        if (!special)
            precheck = Status::kBadtype;
    ...
    co_await guard.enter({dir->obj, dir->oid});
    auto created = co_await dir->obj->mknod(guard.cred(), args->where.name, args->type, args->dev, args->attrs);
    co_await guard.finish();
    if (!created) {
...
rt::Task<void> Engine::proc_remove(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    auto args = Diropargs::decode(call.args);
    if (!args || !call.args.at_end()) {
        co_await rpc::Dispatcher::reply_garbage_args(ctx, call.xid);
        co_return;
...
rt::Task<void> Engine::proc_rmdir(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    auto args = Diropargs::decode(call.args);
    if (!args || !call.args.at_end()) {
        co_await rpc::Dispatcher::reply_garbage_args(ctx, call.xid);
        co_return;
...
rt::Task<void> Engine::proc_rename(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    auto to_res = from ? co_await resolve(args->to.dir, ctx.peer.addr) : Result<Resolved>(Err(errno_from(ESTALE)));
    xdr::XdrEnc enc(ctx.pool);
    if (!from || !to_res) {
        begin_result(enc, ctx, call, core::to_v3(!from ? from.error() : to_res.error(), Proc::kRename));
        encode_wcc_none(enc);
        encode_wcc_none(enc);
        co_await reply(ctx, enc, cap);
        co_return;
...
rt::Task<void> Engine::proc_link(ConnCtx& ctx, RpcCall& call, const rpc::Cred& rpc_cred, Capture* cap) {
    ...
    auto dir_res = file ? co_await resolve(args->to.dir, ctx.peer.addr) : Result<Resolved>(Err(errno_from(ESTALE)));
    xdr::XdrEnc enc(ctx.pool);
    if (!file || !dir_res) {
        begin_result(enc, ctx, call, core::to_v3(!file ? file.error() : dir_res.error(), Proc::kLink));
        encode_post_attr(enc, std::nullopt, 0);
        encode_wcc_none(enc);
        co_await reply(ctx, enc, cap);
        co_return;
    ...
    if (precheck == Status::kOk && !dir.exp->backend->caps().has(backend::Cap::kHardlink)) precheck = Status::kNotsupp;
    if (precheck != Status::kOk) {
    ...
    auto file_attr = co_await core::sample_attr(file->obj);
    co_await guard.finish();
    begin_result(enc, ctx, call, linked ? Status::kOk : core::to_v3(linked.error(), Proc::kLink));
    encode_post_attr(enc, file_attr, file->exp->fsid);
    encode_wcc_sample(enc, guard.first(), dir.exp->fsid);
    co_await reply(ctx, enc, cap);
}
