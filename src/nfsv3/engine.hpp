...
class Engine {
 ...
 private:
    ...
    // (plan doc 10 §6.6).
    using Handler = rt::Task<void> (Engine::*)(transport::ConnCtx&, rpc::RpcCall&, const rpc::Cred&, Capture*);

    rt::Task<Result<Resolved>> resolve(const FileHandle&, const sockaddr_storage&);
    rt::Task<void> dispatch_proc(transport::ConnCtx&, rpc::RpcCall&, const rpc::Cred&, Capture* cap);
