...
rt::Task<Result<backend::DirPage>> readdir_page(const backend::ObjPtr& dir, const backend::Cred& cred, uint64_t cookie,
    ...
    if (cookie == 0) {
        out.ents.push_back({.name = ".", .cookie = 1, .fileid = dir_attr->fileid, .attr = *dir_attr, .oid = dir->id()});
    ...
    if (cookie <= 1) {
        ...
        out.ents.push_back(
            {.name = "..", .cookie = 2, .fileid = parent_attr->fileid, .attr = *parent_attr, .oid = (*parent)->id()});
        if (out.ents.size() == max_entries) co_return out;
    }
    uint64_t backend_cookie = cookie <= 2 ? 0 : cookie - 3;
    auto page = co_await dir->readdir(cred, backend_cookie, max_entries - static_cast<uint32_t>(out.ents.size()));
    if (!page) co_return Err(page.error());
    for (auto& ent : page->ents) {
        if (ent.cookie > std::numeric_limits<uint64_t>::max() - 3) co_return Err(errno_from(EOVERFLOW));
        ent.cookie += 3;
