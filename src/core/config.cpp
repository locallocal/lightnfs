...
Result<std::string> string_value(std::string_view value) {
    value = trim(value);
    if (value.size() < 2 || value.front() != '"' || value.back() != '"') return Err(errno_from(EINVAL));
    std::string out;
...
bool Cidr::contains(const sockaddr_storage& peer) const {
    ...
    if (peer.ss_family == family_) {
        actual = family_ == AF_INET
                     ? reinterpret_cast<const uint8_t*>(&reinterpret_cast<const sockaddr_in*>(&peer)->sin_addr)
                     : reinterpret_cast<const uint8_t*>(&reinterpret_cast<const sockaddr_in6*>(&peer)->sin6_addr);
    ...
    while (std::getline(input, raw_line)) {
        ...
        if (line.front() == '[' && line.back() == ']') {
            if (line == "[server]")
                ...
                section = Section::kLimits;
            else if (line == "[protocol]")
                section = Section::kProtocol;
            else if (line == "[tls]")
                section = Section::kTls;
        ...
        if (section == Section::kServer) {
            if (key == "reactors") {
                ...
                config.server.rpcbind = LNFS_TRY(bool_value(value));
            else if (key == "builtin_portmap")
                config.server.builtin_portmap = LNFS_TRY(bool_value(value));
            else if (key == "state_dir")
                config.server.state_dir = LNFS_TRY(string_value(value));
                ...
                if (lv != "debug" && lv != "info" && lv != "warn" && lv != "error") return Err(errno_from(EINVAL));
            ...
            if (key == "mode")
                config.server.tls_mode = LNFS_TRY(string_value(value));
            else if (key == "cert")
                config.server.tls_cert = LNFS_TRY(string_value(value));
            else if (key == "key")
                config.server.tls_key = LNFS_TRY(string_value(value));
            else if (key == "ca")
                config.server.tls_ca = LNFS_TRY(string_value(value));
            ...
            if (key == "enabled")
                c.enabled = LNFS_TRY(bool_value(value));
            else if (key == "id")
                c.id = LNFS_TRY(string_value(value));
            else if (key == "shared_dir")
                c.shared_dir = normalize_path(LNFS_TRY(string_value(value)));
            else if (key == "node")
                c.node = LNFS_TRY(string_value(value));
            else if (key == "role")
                c.role = LNFS_TRY(string_value(value));
            else if (key == "takeover")
                c.takeover = LNFS_TRY(string_value(value));
            else if (key == "takeover_hook")
                c.takeover_hook = LNFS_TRY(string_value(value));
            else if (key == "mode")
                c.mode = LNFS_TRY(string_value(value));
            else if (key == "node_address")
                c.node_address = LNFS_TRY(string_value(value));
            ...
            if (key == "v3")
                LNFS_TRY(bool_value(value));
            else if (key == "v4")
                config.server.enable_v4 = LNFS_TRY(bool_value(value));
            else if (key == "delegations")
                config.server.delegations = LNFS_TRY(bool_value(value));
                ...
                if (key == "lease")
                    ...
                    config.server.grace_seconds = static_cast<uint32_t>(n);
                ...
                if (suffix == "s" || suffix.empty())
                    config.server.drc_ttl_ms = n * 1000;
                else if (suffix == "ms")
                    config.server.drc_ttl_ms = n;
                else
                    return Err(errno_from(EINVAL));
            } else if (key == "drc_mem")
...
bool valid_cluster_node_name(std::string_view name) {
    return !name.empty() && name.size() <= 64 && std::all_of(name.begin(), name.end(), [](unsigned char ch) {
...
Result<void> validate_active_active(const Config& config) {
    ...
    if (!cluster_active_active(c)) {
        if (!c.node_address.empty()) LNFS_WARN("[cluster] node_address is ignored under mode = \"{}\"", c.mode);
        for (const auto& exp : config.exports)
            if (!exp.nodes.empty())
                LNFS_WARN("export fsid={}: nodes is ignored under [cluster] mode = \"{}\"", exp.fsid, c.mode);
        return {};
    }
    if (c.role != "auto") {
...
bool valid_export_nodes(const ExportConfig& exp, std::string& why) {
    ...
    for (const auto& node : exp.nodes) {
        if (!valid_cluster_node_name(node) || !seen.insert(node).second) {
            why = std::format("export fsid={}: nodes has an invalid or duplicate entry \"{}\"", exp.fsid, node);
            return false;
        }
    }
    return true;
...
std::map<std::string, std::vector<const ExportConfig*>> same_volume_groups(
    ...
    for (const ExportConfig* exp : exports) {
        const char* shared_key = exp->backend == "gluster" ? "volume" : exp->backend == "lustre" ? "mount" : nullptr;
        if (!shared_key) continue;
        auto it = exp->backend_config.values.find(shared_key);
        groups[exp->backend + ":" + (it == exp->backend_config.values.end() ? "" : it->second)].push_back(exp);
    }
    return groups;
}
...
Result<void> validate_config(const Config& config) {
    ...
    {  // Export source (design 11 §11.2, plan 12 A1): local [[export]] blocks or the
       ...
       // empty table before the first publish — so only "local" insists on exports here.
        ...
        for (const auto& [name, unused] : config.backend_defaults)
            if (!backend::find_backend(name)) LNFS_WARN("[backend_defaults.{}]: no such backend in this build", name);
    }
    ...
    {  // Multi-gateway failover (design 09 §9.3, plan 10 A1): the [cluster] section.
        ...
        if (c.enabled) {
            ...
            if (c.id.size() < 8 || c.id.size() > 64 || !std::all_of(c.id.begin(), c.id.end(), [](unsigned char ch) {
                    return std::isalnum(ch) || ch == '_' || ch == '-';
                }))
                return Err(errno_from(EINVAL));
            if (c.shared_dir.empty() || c.shared_dir.front() != '/') return Err(errno_from(EINVAL));
            ...
            if (!c.takeover_hook.empty()) {
                struct stat st{};
                if (stat(c.takeover_hook.c_str(), &st) < 0) return Err(errno_from(errno ? errno : ENOENT));
    ...
    for (const auto& exp : config.exports) {
        if (exp.fsid == 0 || exp.path.empty() || !fsids.insert(exp.fsid).second || !paths.insert(exp.path).second)
            return Err(errno_from(EINVAL));
...
std::string canonical_exports_text(const Config& config) {
    ...
    for (const ExportConfig* exp : sorted) {
        const char* squash = exp->squash == Squash::kNone ? "none" : exp->squash == Squash::kAll ? "all" : "root";
        out += std::format(
            "export path={} fsid={} backend={} readonly={} squash={} anon_uid={} "
            "anon_gid={}\n",
            exp->path, exp->fsid, exp->backend, exp->readonly ? 1 : 0, squash, exp->anon_uid, exp->anon_gid);
...
ExportEntry* ExportSet::for_mount_path(std::string_view raw, std::string& relative) const {
    ...
    for (const auto& entry : entries) {
        ...
        if (path.size() != entry->path.size() && !(entry->path == "/" || path[entry->path.size()] == '/')) continue;
...
Result<std::shared_ptr<const ExportSet>> ExportTable::apply(ExportSetPlan plan,
    ...
    for (const auto& cfg : plan.update) {
        ...
        if (!entry || removed.contains(cfg.fsid) || !updated.insert(cfg.fsid).second) return Err(errno_from(EINVAL));
    ...
    for (size_t i = 0; i < plan.update.size(); ++i)
        update_entry(*current->by_fsid(plan.update[i].fsid), plan.update[i], std::move(update_clients[i]));
...
std::string ExportTable::reload_dynamic(const Config& fresh) {
    ...
    for (const auto& cfg : fresh.exports) {
        ...
        if (normalize_path(cfg.path) != entry->path || cfg.squash != entry->squash || cfg.readonly != entry->readonly ||
            cfg.anon_uid != entry->anon_uid || cfg.anon_gid != entry->anon_gid) {
            report += std::format(
