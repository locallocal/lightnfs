...
void strip_per_node_keys(ExportConfig& cfg) {
  std::erase_if(cfg.backend_config.values,
                [](const auto& kv) { return per_node_backend_key(kv.first); });
}

// `inner` lives under `outer` ("/a" under "/", "/a/b" under "/a"; not "/ab" under "/a").
bool nested_path(const std::string& outer, const std::string& inner) {
  return inner.size() > outer.size() && inner.starts_with(outer) &&
         (outer == "/" || inner[outer.size()] == '/');
}
...
bool CatalogDiff::empty() const {
  return added.empty() && removed.empty() && disabled.empty() && enabled.empty() &&
         nodes_changed.empty() && dynamic_changed.empty() && rejected.empty();
}
...
std::string serialize_catalog(const Catalog& catalog) {
  ...
  for (const auto& exp : exports) {
    ...
    const char* squash = cfg.squash == Squash::kNone  ? "none"
                         : cfg.squash == Squash::kAll ? "all"
                                                      : "root";
    out += std::format(
        "\n[[export]]\npath = {}\nfsid = {}\nbackend = {}\nnodes = {}\nclients = {}\n"
        "readonly = {}\nsquash = \"{}\"\nanon_uid = {}\nanon_gid = {}\nread_bps = {}\n"
        "write_bps = {}\niops = {}\ndisabled = {}\n",
        quote(cfg.path), cfg.fsid, quote(cfg.backend), quote_array(cfg.nodes),
        quote_array(cfg.clients), cfg.readonly, squash, cfg.anon_uid, cfg.anon_gid, cfg.read_bps,
        cfg.write_bps, cfg.iops, exp.disabled);
...
Result<void> validate_catalog(const Catalog& catalog, bool active_active, std::string* why) {
  ...
  for (const auto& exp : catalog.exports) {
    ...
    if (!fsids.insert(cfg.fsid).second)
      return fail(std::format("export fsid={}: fsid is listed twice", cfg.fsid));
    if (cfg.path.empty() || cfg.path.front() != '/' || normalize_path(cfg.path) != cfg.path)
      return fail(
          std::format("export fsid={}: path must be absolute with no trailing slash", cfg.fsid));
    if (!backend::find_backend(cfg.backend))
      return fail(std::format("export fsid={}: no such backend \"{}\" in this build", cfg.fsid,
                              cfg.backend),
                  ENODEV);
    ...
    if (cfg.clients.empty())
      return fail(std::format("export fsid={}: clients must not be empty", cfg.fsid));
    ...
    for (const ExportConfig* other : exports) {
      if (other->path == cfg.path)
        return fail(std::format("export fsid={} and fsid={} share path {}", other->fsid, cfg.fsid,
                                cfg.path));
...
CatalogDiff diff_catalog(const Catalog& from, const Catalog& to) {
  ...
  for (const auto& [fsid, now] : after) {
    ...
    if (a.path != b.path || a.backend != b.backend ||
        cluster_backend_keys(a) != cluster_backend_keys(b)) {
    ...
    if (a.clients != b.clients || a.read_bps != b.read_bps || a.write_bps != b.write_bps ||
        a.iops != b.iops || a.readonly != b.readonly || a.squash != b.squash ||
