...
std::string serialize_catalog(const Catalog& catalog) {
    ...
    for (const auto& exp : exports) {
        ...
        out += std::format(
            "\n[[export]]\npath = {}\nfsid = {}\nbackend = {}\nnodes = {}\nclients = {}\n"
            "readonly = {}\nsecure_ports = {}\nstrict_readdir_cookies = {}\nsquash = \"{}\"\n"
            "anon_uid = {}\nanon_gid = {}\nread_bps = {}\nwrite_bps = {}\niops = {}\ndisabled = {}\n",
            quote(cfg.path), cfg.fsid, quote(cfg.backend), quote_array(cfg.nodes), quote_array(cfg.clients),
            cfg.readonly, cfg.secure_ports, cfg.strict_readdir_cookies, squash, cfg.anon_uid, cfg.anon_gid,
            cfg.read_bps, cfg.write_bps, cfg.iops, exp.disabled);
...
CatalogDiff diff_catalog(const Catalog& from, const Catalog& to) {
    ...
    for (const auto& [fsid, now] : after) {
        ...
        if (a.clients != b.clients || a.read_bps != b.read_bps || a.write_bps != b.write_bps || a.iops != b.iops ||
            a.readonly != b.readonly || a.secure_ports != b.secure_ports ||
            a.strict_readdir_cookies != b.strict_readdir_cookies || a.squash != b.squash || a.anon_uid != b.anon_uid ||
            a.anon_gid != b.anon_gid)
