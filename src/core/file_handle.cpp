...
uint64_t rotl(uint64_t value, int bits) {
...
uint32_t load_be32(const std::byte* p) {
    return (uint32_t(static_cast<uint8_t>(p[0])) << 24) | (uint32_t(static_cast<uint8_t>(p[1])) << 16) |
           (uint32_t(static_cast<uint8_t>(p[2])) << 8) | uint32_t(static_cast<uint8_t>(p[3]));
}
...
Result<std::array<std::byte, 16>> load_or_create_hmac_key(const std::string& path) {
    ...
    if (auto existing = read_existing())
        return existing;
    else if (existing.error() != errno_from(ENOENT))
        return existing;
...
uint64_t FileHandleCodec::tag(std::span<const std::byte> bytes) const {
    ...
    for (size_t i = 0; pos + i < bytes.size(); ++i) last |= uint64_t(static_cast<uint8_t>(bytes[pos + i])) << (8 * i);
    v3 ^= last;
    sip_round(v0, v1, v2, v3);
    sip_round(v0, v1, v2, v3);
    v0 ^= last;
...
std::vector<std::byte> FileHandleCodec::encode(const ExportEntry& exp, const backend::ObjId& oid) const {
...
Result<DecodedHandle> FileHandleCodec::decode(std::span<const std::byte> fh, const sockaddr_storage& peer,
                                              const ExportSet& exports) const {
    if (fh.size() < 14 || fh.size() > 64 || static_cast<uint8_t>(fh[0]) != kVersion) return Err(Errno::kBadHandle);
...
std::vector<std::byte> FileHandleCodec::encode_raw(uint32_t fsid, const backend::ObjId& oid) const {
    std::vector<std::byte> out;
    out.reserve(13 + oid.len);
    out.push_back(static_cast<std::byte>(kVersion));
...
Result<FileHandleCodec::DecodedV4> FileHandleCodec::decode_v4(std::span<const std::byte> fh,
                                                              ...
                                                              const ExportSet& exports) const {
    if (fh.size() < 14 || fh.size() > 64 || static_cast<uint8_t>(fh[0]) != kVersion) return Err(Errno::kBadHandle);
    if (tag(fh.first(fh.size() - 8)) != read64(fh.data() + fh.size() - 8)) return Err(Errno::kBadHandle);
    DecodedV4 out;
...
Result<FileHandleCodec::Inspection> FileHandleCodec::inspect(std::span<const std::byte> fh) const {
