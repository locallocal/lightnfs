...
std::shared_ptr<rt::AsyncSharedMutex> ObjLockRegistry::get(uint32_t fsid, const backend::ObjId& oid) {
    Key key{fsid, oid};
    ...
    if (shard.locks.size() > 1024) {
        ...
        for (int b = 0; b < 8 && ndead < 16; ++b) {
            ...
            for (auto it = shard.locks.begin(idx); it != shard.locks.end(idx) && ndead < 16; ++it) {
