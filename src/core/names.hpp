...
inline bool valid_component(std::string_view name, bool allow_dots = false, size_t max_len = kMaxNameLen) {
...
inline bool valid_utf8(std::string_view s) {
    return valid_utf8(std::span<const std::byte>(reinterpret_cast<const std::byte*>(s.data()), s.size()));
}
