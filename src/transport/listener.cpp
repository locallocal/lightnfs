...
Result<int> make_listen_socket(const std::string& bind_addr, uint16_t port, uint16_t& bound) {
    sockaddr_storage ss{};
    socklen_t slen = 0;
...
Result<std::unique_ptr<Listener>> Listener::create(uint16_t port, TransportConfig cfg, rpc::Dispatcher& disp,
                                                   rt::Runtime& rt, const std::string& bind_addr) {
    std::vector<int> fds;
    uint16_t bound = 0;
...
void Listener::request_stop() {
    ...
    for (size_t i = 0; i < fds_.size(); ++i) {
        ...
        spawn([](int fd) -> Task<void> { co_await uring_cancel_fd(fd); }(fds_[i]), rt_.reactor(i));
    }
}
