...
Result<std::unique_ptr<TlsContext>> TlsContext::create(const TlsConfig& cfg) {
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx, cfg.key.c_str(), SSL_FILETYPE_PEM) != 1) return fail(EINVAL);
    if (SSL_CTX_check_private_key(ctx) != 1) return fail(EINVAL);
    if (!cfg.ca.empty()) {
        if (SSL_CTX_load_verify_locations(ctx, cfg.ca.c_str(), nullptr) != 1) return fail(EINVAL);
...
rt::Task<Result<void>> TlsConn::feed_in(int fd) {
    for (;;) {
        int n = co_await uring_recv(fd, std::span<std::byte>(netbuf_.data(), netbuf_.size()));
        if (n == -EINTR || n == -EAGAIN) continue;
...
rt::Task<Result<void>> TlsConn::accept(int) {
