...
std::vector<ConnRegistry::Info> ConnRegistry::list() {
    ...
    for (const auto& [id, ent] : conns_)
        out.push_back(
            {id, ent.peer.to_string(), std::chrono::duration_cast<std::chrono::seconds>(now - ent.since).count()});
...
Task<void> ConnCtx::send(rt::SendBuf buf) {
    ...
    if (!r && !send_failed) {
        send_failed = true;
        LNFS_WARN("conn {}: send failed ({}), tearing down", peer.to_string(), errno_name(r.error()));
        cancel.request();
    }
}
...
std::optional<uint32_t> starttls_probe_xid(const BufferChain& rec) {
    ...
    if (*mtype != rpc::kCall || *proc != 0 || *cred_flavor != rpc::kAuthTls) return std::nullopt;
    return *xid;
}
...
Task<void> negotiate_starttls(ConnCtx* c, uint32_t xid) {
    ...
    if (auto ok = co_await tls->accept(c->fd); !ok) {
        LNFS_WARN("conn {}: TLS handshake failed ({}), tearing down", c->peer.to_string(), errno_name(ok.error()));
        c->cancel.request();
        co_return;
    }
    c->tls_active = true;
...
Task<void> handle_one(ConnCtx* c, rpc::Dispatcher* d, BufferChain rec) {
    ...
    try {
        ...
        LNFS_ERROR("conn {}: unrecoverable handler error ({}), tearing down", c->peer.to_string(), e.what());
        c->cancel.request();
    } catch (...) {
    ...
    for (;;) {
        ...
        if (!rec) {
            if (rec.error() != Errno::kEof) {
                LNFS_WARN("conn {}: framing error ({}), closing", c->peer.to_string(), errno_name(rec.error()));
            }
            break;
        }
