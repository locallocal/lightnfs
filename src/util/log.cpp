...
void ensure_default_logger() {
    std::call_once(g_once, [] {
        auto lg = std::make_shared<spdlog::logger>("lnfs", std::make_shared<spdlog::sinks::stderr_sink_mt>());
...
void init_async_logging(const LogSinkConfig& cfg) {
    ...
    if (cfg.file.empty()) {
        ...
        sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(cfg.file, cfg.rotate_size, cfg.rotate_keep);
    }
    auto lg = std::make_shared<spdlog::async_logger>("lnfs", std::move(sink), spdlog::thread_pool(),
