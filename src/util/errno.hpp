...
constexpr Errno errno_from_neg(int r) {
    return static_cast<Errno>(-r);
}
constexpr int raw(Errno e) {
...
inline std::string errno_name(Errno e) {
    switch (e) {
        ...
        case Errno::kEof:
            return "EOF";
        case Errno::kBadHandle:
            return "BADHANDLE";
        case Errno::kMoved:
            return "MOVED";
        default:
            return std::string(strerrorname_np(raw(e)) ? strerrorname_np(raw(e)) : "E?");
    }
}
