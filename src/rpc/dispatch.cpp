...
Task<void> Dispatcher::handle_request(transport::ConnCtx& ctx, rt::BufferChain rec) {
    ...
    if (!call) {
        ...
        LNFS_WARN("conn {}: unparseable RPC record ({} bytes), dropped", ctx.peer.to_string(), rec.size());
        co_return;
    }
    ...
    if (ctx.tls_policy == transport::TlsPolicy::kRequired && !ctx.tls_active && call->proc != 0) {
    ...
    try {
        ...
        LNFS_ERROR("conn {}: handler exception for prog={} proc={}: {}", ctx.peer.to_string(), call->prog, call->proc,
                   e.what());
        failed = true;
    } catch (...) {
        LNFS_ERROR("conn {}: handler exception for prog={} proc={}", ctx.peer.to_string(), call->prog, call->proc);
