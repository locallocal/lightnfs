...
Drc::Key Drc::Key::make(const sockaddr_storage& peer, uint32_t xid, uint32_t prog, uint32_t vers, uint32_t proc,
                        uint64_t args_hash) {
    Key k;
...
void Drc::purge(Shard& sh) {
    ...
    while (!sh.completed.empty()) {
        ...
        sh.bytes.fetch_sub(it->second.reply ? it->second.reply->size() : 0, std::memory_order_relaxed);
        sh.entries.erase(it);
