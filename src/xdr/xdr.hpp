...
inline uint32_t from_be32(uint32_t v) {
    return to_be32(v);
}
inline uint64_t from_be64(uint64_t v) {
    return to_be64(v);
}
constexpr uint32_t pad4(uint32_t n) {
    return (4 - (n & 3)) & 3;
}
...
class XdrEnc {
 public:
    ...
    Mark mark() const { return {out_.seg_count(), size_, tail_used_, tail_ ? tail_.data() : nullptr, overflow_}; }
    void rollback(const Mark& m) {
        if (tail_ && tail_.data() == m.tail_id) {
            ...
            while (out_.seg_count() > m.segs) {
                ...
                if (out_.seg_count() == m.segs && m.tail_id && seg.buf.data() == m.tail_id && seg.off == 0) {
 ...
 private:
    ...
    void close_tail() {
        if (tail_ && tail_used_ > 0) out_.append(std::move(tail_), 0, static_cast<uint32_t>(tail_used_));
        tail_ = rt::Buffer();
        tail_used_ = 0;
    }
    void check_limit() {
