...
const char* v3_proc_name(uint32_t proc) {
    static const char* names[] = {"NULL",   "GETATTR", "SETATTR",  "LOOKUP", "ACCESS",  "READLINK",
...
void append_histogram(std::string& out, std::string_view name, std::string_view labels,
                      std::span<const uint64_t> bounds_us, std::span<const uint64_t> buckets, uint64_t sum_us) {
    const char* sep = labels.empty() ? "" : ",";
    uint64_t cum = 0, count = 0;
    for (uint64_t b : buckets) count += b;
    for (size_t b = 0; b < bounds_us.size(); ++b) {
        cum += buckets[b];
        out += std::format("{}_bucket{{{}{}le=\"{}\"}} {}\n", name, labels, sep, seconds_text(bounds_us[b]), cum);
    }
    out += std::format("{}_bucket{{{}{}le=\"+Inf\"}} {}\n", name, labels, sep, count);
    if (labels.empty()) {
        out += std::format("{}_sum {}\n{}_count {}\n", name, seconds_text(sum_us), name, count);
    } else {
        out += std::format("{}_sum{{{}}} {}\n{}_count{{{}}} {}\n", name, labels, seconds_text(sum_us), name, labels,
                           count);
    }
}
...
void append_histogram(std::string& out, std::string_view name, std::string_view labels,
    ...
    append_histogram(out, name, labels, LatencyHistogram::kBoundsUs, snap.buckets, snap.sum_us);
}
...
std::string prometheus_text() {
    ...
    for (size_t i = 0; i < Metrics::kV3Procs; ++i) {
        ...
        append_histogram(out, "lightnfs_v3_duration_seconds", std::format("proc=\"{}\"", name),
                         m.v3_duration[i].snapshot());
    }
    ...
    for (size_t i = 0; i < Metrics::kV4Ops; ++i) {
        ...
        append_histogram(out, "lightnfs_v4_op_duration_seconds", label, m.v4_op_duration[i].snapshot());
    }
    ...
    out += std::format("lightnfs_v3_bad_cookie_total {}\n", m.v3_bad_cookie.load(std::memory_order_relaxed));
    out +=
        std::format("lightnfs_v4_readdir_not_same_total {}\n", m.v4_readdir_not_same.load(std::memory_order_relaxed));
