...
class ScopedIds {
 public:
    ScopedIds(const gfapi::Api& api, uint32_t uid, uint32_t gid, std::span<const uint32_t> gids) : api_(api) {
...
class GlusterOpenState final : public OpenState {
 public:
    GlusterOpenState(const gfapi::Api* api, glfs_fd* fd, bool writable) : api_(api), fd_(fd), writable_(writable) {}
...
Result<std::unique_ptr<GlusterBackend>> GlusterBackend::create(Config cfg, std::shared_ptr<const gfapi::Api> api) {
    ...
    if (cfg.transport != "tcp" && cfg.transport != "rdma" && cfg.transport != "unix") return Err(errno_from(EINVAL));
...
Result<GlusterBackend::ObjRef> GlusterBackend::handle_from_oid_sync(const ObjId& oid, struct stat* st) {
    auto gfid = gfid_from_oid(oid);
    if (!gfid) return Err(gfid.error());
    struct stat local{};
    glfs_object* obj =
        api_->glfs_h_create_from_handle(fs_, gfid->data(), static_cast<int>(gfid->size()), st ? st : &local);
    if (!obj) {
        int e = errno;
...
rt::Task<Result<void>> GlusterBackend::start() {
        ...
        [cfg, api]() -> Result<Started> {
            ...
            for (const auto& s : cfg.servers)
                if (api->glfs_set_volfile_server(fs, cfg.transport.c_str(), s.host.c_str(), s.port) < 0)
                    return fail(errno);
            if (!cfg.log_file.empty() && api->glfs_set_logging(fs, cfg.log_file.c_str(), cfg.log_level) < 0)
                return fail(errno);
    ...
    if (!started) {
        LNFS_ERROR("gluster volume {} ({}): connect failed: {}", cfg_.volume, cfg_.subdir, errno_name(started.error()));
    ...
    LNFS_INFO("gluster volume {} ({}) up: volume-id {} native-locks={} jukebox={}", cfg_.volume, cfg_.subdir,
              volume_id_.empty() ? "?" : volume_id_, cfg_.native_locks, cfg_.jukebox);
...
Result<Created> GlusterObject::created_sync(glfs_object* child, const struct stat& st, const Cred& cred,
...
rt::Task<Result<Created>> GlusterObject::create(const Cred& cred, std::string_view name, const SetAttr& attrs,
                                                ExclVerf* verf) {
    ...
    co_return co_await rt::offload([this, ref, oc, owned, attrs, verf_copy, exclusive]() -> Result<Created> {
        ...
        struct stat st{};
        glfs_object* child =
            api.glfs_h_creat(backend_.fs_, ref->obj, owned.c_str(), O_CREAT | O_EXCL | O_RDWR, mode, &st);
        ...
        if (exclusive) {
            ...
            if (api.glfs_h_setattrs(backend_.fs_, child, &sb, gfapi::kSetAtime | gfapi::kSetMtime) < 0) {
                int e = errno;
        ...
        if (attrs.size && api.glfs_h_truncate(backend_.fs_, child, static_cast<off_t>(*attrs.size)) < 0) {
            int e = errno;
            api.glfs_h_close(child);
            return Err(backend_.map_errno(e));
        }
...
rt::Task<Result<Created>> GlusterObject::mkdir(const Cred& cred, std::string_view name, const SetAttr& attrs) {
    if (!GlusterBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    auto dir_ok = co_await require_dir(cred);
    if (!dir_ok) co_return Err(dir_ok.error());
    std::string owned(name);
...
rt::Task<Result<Created>> GlusterObject::symlink(const Cred& cred, std::string_view name, std::string_view target,
                                                 const SetAttr&) {
    if (!GlusterBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    ...
    co_return co_await rt::offload([this, ref, oc, owned, owned_target]() -> Result<Created> {
        ...
        ScopedIds ids(api, oc.uid, oc.gid, oc.groups());
        struct stat st{};
        glfs_object* child = api.glfs_h_symlink(backend_.fs_, ref->obj, owned.c_str(), owned_target.c_str(), &st);
        if (!child) return Err(backend_.map_errno(errno));
        return created_sync(child, st, Cred{oc.uid, oc.gid, oc.groups()}, std::nullopt);
    });
}

rt::Task<Result<Created>> GlusterObject::mknod(const Cred& cred, std::string_view name, FType ftype, DevT dev,
                                               const SetAttr& attrs) {
    if (!GlusterBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    mode_t type_bits = 0;
    switch (ftype) {
    ...
    co_return co_await rt::offload([this, ref, oc, owned, mode, rdev]() -> Result<Created> {
        ...
        ScopedIds ids(api, oc.uid, oc.gid, oc.groups());
        struct stat st{};
        glfs_object* child = api.glfs_h_mknod(backend_.fs_, ref->obj, owned.c_str(), mode, rdev, &st);
        if (!child) return Err(backend_.map_errno(errno));
...
rt::Task<Result<void>> GlusterObject::rename(const Cred& cred, std::string_view from, Object& dst_dir,
                                             std::string_view to) {
    if (!GlusterBackend::valid_name(from) || !GlusterBackend::valid_name(to)) co_return Err(errno_from(EINVAL));
    ...
    co_return co_await rt::offload([this, sref, dref, oc, owned_from, owned_to]() -> Result<void> {
        const auto& api = *backend_.api_;
        ScopedIds ids(api, oc.uid, oc.gid, oc.groups());
        if (api.glfs_h_rename(backend_.fs_, sref->obj, owned_from.c_str(), dref->obj, owned_to.c_str()) < 0)
            return Err(backend_.map_errno(errno));
        return {};
    });
}

rt::Task<Result<void>> GlusterObject::link(const Cred& cred, Object& file, std::string_view name) {
    if (!GlusterBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
...
rt::Task<Result<DirPage>> GlusterObject::readdir(const Cred& cred, uint64_t cookie, uint32_t max_entries) {
    if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
...
rt::Task<Result<uint32_t>> GlusterObject::read(OpenCtx ctx, uint64_t off, std::span<std::byte> out, bool& eof) {
    if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
    if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
    glfs_fd* fd = nullptr;
    ...
    auto n = co_await rt::offload([this, fd, off, out]() -> Result<uint32_t> {
        ssize_t r = backend_.api_->glfs_pread(fd, out.data(), out.size(), static_cast<off_t>(off), 0, nullptr);
...
rt::Task<Result<uint32_t>> GlusterObject::write(OpenCtx ctx, uint64_t off, std::span<const std::byte> in,
                                                Stability stability) {
    iovec v{const_cast<std::byte*>(in.data()), in.size()};
    co_return co_await write(ctx, off, std::span<const iovec>(&v, 1), stability);
}

rt::Task<Result<uint32_t>> GlusterObject::write(OpenCtx ctx, uint64_t off, std::span<const iovec> iov,
                                                Stability stability) {
    if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
    if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
    glfs_fd* fd = nullptr;
    ...
    auto done = co_await rt::offload(
        [this, fd, off, vec, total, stability, short_write, sync_fault]() mutable -> Result<uint32_t> {
            const auto& api = *backend_.api_;
            size_t done = 0, idx = 0;
            while (done < total) {
                ssize_t n;
...
rt::Task<Result<void>> GlusterObject::commit(OpenCtx ctx, uint64_t, uint64_t) {
    ...
    co_return co_await rt::offload(
        [this, fd, sync_fault]() -> Result<void> {
...
rt::Task<Result<uint64_t>> GlusterObject::copy_range(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t src_off,
            ...
            while (done < want) {
                ...
                if (!native) {
                    ...
                    while (w < static_cast<size_t>(r)) {
                        ssize_t k = api.glfs_pwrite(dfd, buf.data() + w, static_cast<size_t>(r) - w, out + w, 0,
                                                    nullptr, nullptr);
...
Result<glfs_fd*> GlusterLockMgr::fd_for(GlusterObject& obj, const LockOwnerId& owner, bool create) {
...
rt::Task<Result<void>> GlusterLockMgr::lock(Object& object, const LockOwnerId& owner, LockRange range, bool exclusive,
                                            bool wait) {
...
rt::Task<Result<void>> GlusterLockMgr::unlock(Object& object, const LockOwnerId& owner, LockRange range) {
    auto* obj = dynamic_cast<GlusterObject*>(&object);
    if (!obj || &obj->backend_ != &backend_) co_return Err(errno_from(EXDEV));
    auto keep = obj->ref_;
...
rt::Task<Result<std::optional<LockConflict>>> GlusterLockMgr::test(Object& object, LockRange range, bool exclusive) {
    auto* obj = dynamic_cast<GlusterObject*>(&object);
    if (!obj || &obj->backend_ != &backend_) co_return Err(errno_from(EXDEV));
    if (obj->type() != FType::kReg) co_return Err(errno_from(EINVAL));
    auto keep = obj->ref_;
...
std::unique_ptr<Backend> make_gluster(const BackendConfig& cfg) {
    ...
    if (!made) {
        LNFS_ERROR("export {}: gluster backend config rejected: {}", cfg.path, errno_name(made.error()));
        return nullptr;
    }
