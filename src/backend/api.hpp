...
class Object {
 public:
    ...
    virtual rt::Task<Result<Created>> create(const Cred&, std::string_view, const SetAttr&, ExclVerf* = nullptr);
    virtual rt::Task<Result<Created>> mkdir(const Cred&, std::string_view, const SetAttr&);
    virtual rt::Task<Result<Created>> symlink(const Cred&, std::string_view, std::string_view, const SetAttr&);
    virtual rt::Task<Result<Created>> mknod(const Cred&, std::string_view, FType, DevT, const SetAttr&);
    ...
    virtual rt::Task<Result<void>> rename(const Cred&, std::string_view, Object&, std::string_view);
    virtual rt::Task<Result<void>> link(const Cred&, Object&, std::string_view);
    virtual rt::Task<Result<DirPage>> readdir(const Cred&, uint64_t cookie, uint32_t max_entries);
    ...
    virtual rt::Task<Result<uint32_t>> write(OpenCtx, uint64_t, std::span<const std::byte>, Stability);
    ...
    virtual rt::Task<Result<uint32_t>> write(OpenCtx, uint64_t, std::span<const iovec>, Stability);
    ...
    virtual rt::Task<Result<void>> clone(OpenCtx, Object& dst, OpenCtx, uint64_t src_off, uint64_t dst_off,
                                         uint64_t len);
    virtual rt::Task<Result<uint64_t>> copy_range(OpenCtx, Object& dst, OpenCtx, uint64_t src_off, uint64_t dst_off,
                                                  uint64_t len);
...
#define LNFS_REGISTER_BACKEND(name_literal, make_fn)                                                     \
    ...
    const bool LNFS_BACKEND_CAT(lnfs_backend_registered_, __COUNTER__) = [] {                            \
        ::lnfs::backend::register_backend({name_literal, ::lnfs::backend::kBackendApiVersion, make_fn}); \
        return true;                                                                                     \
    }();                                                                                                 \
    }
