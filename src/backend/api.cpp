...
rt::Task<Result<AccessMask>> Object::access(const Cred& cred, AccessMask want) {
    ...
    if (bits & 1) {
        if (a.type == FType::kDir)
            allowed.set(Access::kLookup);
        else
            allowed.set(Access::kExecute);
    }
...
rt::Task<Result<Created>> Object::create(const Cred&, std::string_view, const SetAttr&, ExclVerf*) {
    return unsupported<Created>();
}
rt::Task<Result<Created>> Object::mkdir(const Cred&, std::string_view, const SetAttr&) {
    return unsupported<Created>();
}
rt::Task<Result<Created>> Object::symlink(const Cred&, std::string_view, std::string_view, const SetAttr&) {
    return unsupported<Created>();
}
rt::Task<Result<Created>> Object::mknod(const Cred&, std::string_view, FType, DevT, const SetAttr&) {
    return unsupported<Created>();
}
...
rt::Task<Result<void>> Object::rename(const Cred&, std::string_view, Object&, std::string_view) {
    return unsupported<void>();
}
...
rt::Task<Result<std::string>> Object::readlink() {
    return unsupported<std::string>();
}
rt::Task<Result<OpenPtr>> Object::open(const Cred&, OpenFlags) {
    return unsupported<OpenPtr>();
}
rt::Task<Result<uint32_t>> Object::read(OpenCtx, uint64_t, std::span<std::byte>, bool&) {
    return unsupported<uint32_t>();
}
rt::Task<Result<uint32_t>> Object::write(OpenCtx, uint64_t, std::span<const std::byte>, Stability) {
    return unsupported<uint32_t>();
}
rt::Task<Result<uint32_t>> Object::write(OpenCtx ctx, uint64_t off, std::span<const iovec> iov, Stability stability) {
...
rt::Task<Result<void>> Object::clone(OpenCtx, Object&, OpenCtx, uint64_t, uint64_t, uint64_t) {
    return unsupported<void>();
}
rt::Task<Result<uint64_t>> Object::copy_range(OpenCtx, Object&, OpenCtx, uint64_t, uint64_t, uint64_t) {
    return unsupported<uint64_t>();
}
...
rt::Task<Result<void>> Backend::start() {
    co_return Result<void>{};
}
rt::Task<Result<void>> Backend::stop() {
    co_return Result<void>{};
}
rt::Task<Result<void>> Backend::takeover(const ClusterIdentity&) {
    co_return Result<void>{};
}
...
void register_backend(BackendFactory factory) {
    if (!factory.make || factory.name.empty() || factory.api_version != kBackendApiVersion) return;
    std::lock_guard lock(registry_mutex());
    auto& r = registry();
    auto it = std::find_if(r.begin(), r.end(), [&](const auto& f) { return f.name == factory.name; });
    if (it == r.end())
        r.push_back(std::move(factory));
    else
        *it = std::move(factory);
}
