...
inline constexpr unsigned kStatxMode = 0x1, kStatxNlink = 0x2, kStatxUid = 0x4, kStatxGid = 0x8, kStatxRdev = 0x10,
                          kStatxAtime = 0x20, kStatxMtime = 0x40, kStatxCtime = 0x80, kStatxIno = 0x100,
                          kStatxSize = 0x200, kStatxBlocks = 0x400, kStatxBasicStats = 0x7ff, kStatxBtime = 0x800,
                          kStatxVersion = 0x1000, kStatxAllStats = 0x1fff;
// ceph_ll_client.h CEPH_SETATTR_*
inline constexpr int kSetMode = 1 << 0, kSetUid = 1 << 1, kSetGid = 1 << 2, kSetMtime = 1 << 3, kSetAtime = 1 << 4,
                     kSetSize = 1 << 5, kSetCtime = 1 << 6, kSetMtimeNow = 1 << 7, kSetAtimeNow = 1 << 8;
...
struct Api {
    ...
    int (*ceph_ll_lookup)(ceph_mount_info*, Inode* parent, const char* name, Inode** out, struct ceph_statx* stx,
                          unsigned want, unsigned flags, const UserPerm* perms) = nullptr;
    int (*ceph_ll_put)(ceph_mount_info*, Inode* in) = nullptr;
    int (*ceph_ll_getattr)(ceph_mount_info*, Inode* in, struct ceph_statx* stx, unsigned want, unsigned flags,
                           const UserPerm* perms) = nullptr;
    int (*ceph_ll_setattr)(ceph_mount_info*, Inode* in, struct ceph_statx* stx, int mask,
                           const UserPerm* perms) = nullptr;
    int (*ceph_ll_open)(ceph_mount_info*, Inode* in, int flags, Fh** fh, const UserPerm* perms) = nullptr;
    int (*ceph_ll_create)(ceph_mount_info*, Inode* parent, const char* name, mode_t mode, int oflags, Inode** outp,
                          Fh** fhp, struct ceph_statx* stx, unsigned want, unsigned lflags,
                          const UserPerm* perms) = nullptr;
    int (*ceph_ll_mknod)(ceph_mount_info*, Inode* parent, const char* name, mode_t mode, dev_t rdev, Inode** out,
                         struct ceph_statx* stx, unsigned want, unsigned flags, const UserPerm* perms) = nullptr;
    int (*ceph_ll_mkdir)(ceph_mount_info*, Inode* parent, const char* name, mode_t mode, Inode** out,
                         struct ceph_statx* stx, unsigned want, unsigned flags, const UserPerm* perms) = nullptr;
    int (*ceph_ll_symlink)(ceph_mount_info*, Inode* in, const char* name, const char* value, Inode** out,
                           struct ceph_statx* stx, unsigned want, unsigned flags, const UserPerm* perms) = nullptr;
    int (*ceph_ll_link)(ceph_mount_info*, Inode* in, Inode* newparent, const char* name,
                        const UserPerm* perms) = nullptr;
    int (*ceph_ll_unlink)(ceph_mount_info*, Inode* in, const char* name, const UserPerm* perms) = nullptr;
    int (*ceph_ll_rmdir)(ceph_mount_info*, Inode* in, const char* name, const UserPerm* perms) = nullptr;
    int (*ceph_ll_rename)(ceph_mount_info*, Inode* parent, const char* name, Inode* newparent, const char* newname,
                          const UserPerm* perms) = nullptr;
    int (*ceph_ll_readlink)(ceph_mount_info*, Inode* in, char* buf, size_t bufsize, const UserPerm* perms) = nullptr;
    ...
    // file handles
    int (*ceph_ll_read)(ceph_mount_info*, Fh* filehandle, int64_t off, uint64_t len, char* buf) = nullptr;
    int (*ceph_ll_write)(ceph_mount_info*, Fh* filehandle, int64_t off, uint64_t len, const char* data) = nullptr;
    int64_t (*ceph_ll_writev)(ceph_mount_info*, Fh* fh, const struct iovec* iov, int iovcnt, int64_t off) = nullptr;
    ...
    int (*ceph_ll_fallocate)(ceph_mount_info*, Fh* fh, int mode, int64_t offset, int64_t length) = nullptr;
    ...
    int (*ceph_ll_setlk)(ceph_mount_info*, Fh* fh, struct flock* fl, uint64_t owner, int sleep) = nullptr;
    // directories
    int (*ceph_ll_opendir)(ceph_mount_info*, Inode* in, ceph_dir_result** dirpp, const UserPerm* perms) = nullptr;
    int (*ceph_ll_releasedir)(ceph_mount_info*, ceph_dir_result* dir) = nullptr;
    int (*ceph_readdirplus_r)(ceph_mount_info*, ceph_dir_result* dirp, struct dirent* de, struct ceph_statx* stx,
                              unsigned want, unsigned flags, Inode** out) = nullptr;
