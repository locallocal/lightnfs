...
class CephBackend::ObjCache {
 public:
    ...
    explicit ObjCache(size_t capacity) : per_shard_capacity_(std::max<size_t>((capacity + kShards - 1) / kShards, 1)) {}
...
class CephBackend::FdCache {
 public:
    ...
    rt::Task<Result<Ref>> acquire(CephObject& obj, bool write) {
        ...
        auto value = std::make_shared<Entry>(backend_.api_.get(), backend_.mount_, oid, *opened, flags);
        {
            std::lock_guard lock(shard.mu);
            auto [it, inserted] = shard.entries.emplace(oid, value);
            if (inserted) {
...
Result<std::unique_ptr<CephBackend>> CephBackend::create(Config cfg, std::shared_ptr<const cephapi::Api> api) {
...
Errno CephBackend::map_rc(int64_t rc) const {
    ...
    switch (e) {
        ...
        case kBlocklisted: {
            ...
            if ((n & (n - 1)) == 0)
                LNFS_ERROR(
                    "cephfs export {}: session blocklisted (EBLOCKLISTED, {} occurrences); "
                    "restart the gateway to reconnect",
                    cfg_.subdir, n);
            return errno_from(EIO);
        }
        case 0:
...
Result<Attr> CephBackend::attr_from_statx(const struct ceph_statx& st) const {
    ...
    a.rdev = DevT{static_cast<uint32_t>(major(st.stx_rdev)), static_cast<uint32_t>(minor(st.stx_rdev))};
    a.fileid = st.stx_ino;
...
Result<MountedRoot> mount_root(const cephapi::Api& api, ceph_mount_info* m, const std::string& subdir,
...
rt::Task<Result<void>> CephBackend::start() {
    ...
    auto started = co_await rt::offload(
        [cfg, api]() -> Result<Started> {
            ...
            return Started{m, root_perms, mounted->root, mounted->st, fsid, api->ceph_get_fs_cid(m), ver ? ver : "?"};
        },
    ...
    LNFS_INFO(
        ...
        cfg_.subdir, cluster_fsid_.empty() ? "?" : cluster_fsid_, cfg_.fs_name.empty() ? "<default>" : cfg_.fs_name,
        fscid_, started->version, cfg_.native_locks, cfg_.jukebox);
...
rt::Task<Result<void>> CephBackend::takeover(const ClusterIdentity& who) {
    ...
    if (!cephapi::reclaim_supported(*api_)) {
        LNFS_WARN(
            "cephfs export {}: libcephfs has no session reclaim (ceph_start_reclaim); "
            "the failed gateway's caps and locks expire on the MDS's own timeout",
            cfg_.subdir);
        co_return Err(errno_from(ENOTSUP));
    }
    const std::string uuid = cfg_.uuid.empty() ? who.cluster_id + "-" + std::to_string(cfg_.fsid) : cfg_.uuid;
    ...
    auto out = co_await rt::offload(
        [api, m, cfg, perms, uuid, already_ours, node = who.node, epoch = who.epoch]() -> Outcome {
            ...
            if (already_ours) {
                ...
                LNFS_INFO("cephfs export {}: session uuid {} already ours; remounting", cfg.subdir, uuid);
            } else {
                ...
                if (rc == 0) {
                    ...
                    LNFS_INFO(
                        "cephfs export {}: reclaimed session uuid {} for {} (epoch {}): the MDS "
                        "evicted the previous holder",
                        cfg.subdir, uuid, node, epoch);
                } else if (rc == -ENOENT) {
                    LNFS_INFO("cephfs export {}: no session with uuid {} to reclaim (epoch {})", cfg.subdir, uuid,
                              epoch);
                } else {
                    o.reclaim = Err(errno_from(-rc));
                    LNFS_WARN(
                        "cephfs export {}: reclaim of session uuid {} failed: {} (locks held "
    ...
    if (!out.root) {
        ...
        LNFS_ERROR("cephfs export {}: remount after reclaim failed: {}", cfg_.subdir, errno_name(out.root.error()));
        root_perms_ = nullptr;
        session_uuid_.clear();
        co_await rt::offload(
            [api, m, perms] {
...
rt::Task<Result<ObjPtr>> CephBackend::resolve(const ObjId& oid) {
    ...
    auto entry = obj_cache_->insert(oid, std::make_shared<ObjCache::Entry>(oid, got->first, got->second));
...
rt::Task<Result<ObjPtr>> CephObject::lookup(const Cred& cred, std::string_view name) {
    ...
    if (name == "." || (name == ".." && id() == backend_.root_oid_)) co_return backend_.wrap(ref_, id(), FType::kDir);
    ...
    co_return co_await rt::offload([this, ref, owned, oc]() -> Result<ObjPtr> {
        ...
        struct ceph_statx st{};
        Inode* child = nullptr;
        int rc = api.ceph_ll_lookup(backend_.mount_, ref->in, owned.c_str(), &child, &st, kWant, 0, perms.get());
...
rt::Task<Result<Attr>> CephObject::setattr(const Cred& cred, const SetAttr& s) {
    ...
    co_return co_await rt::offload([this, ref, oc, s]() -> Result<Attr> {
        ...
        if (mask) {
            ...
            if (rc < 0 && !(type() == FType::kLnk && (mask & cephapi::kSetMode))) return Err(backend_.map_rc(rc));
        }
...
Result<Created> CephObject::created_sync(Inode* child, struct ceph_statx st, const UserPerm* perms,
...
rt::Task<Result<Created>> CephObject::create(const Cred& cred, std::string_view name, const SetAttr& attrs,
                                             ExclVerf* verf) {
    ...
    OwnedCred oc(cred);
    auto ref = ref_;
    co_return co_await rt::offload([this, ref, oc, owned, attrs, verf_copy, exclusive]() -> Result<Created> {
        const auto& api = *backend_.api_;
        Perms perms(api, oc);
        ...
        Inode* child = nullptr;
        Fh* fh = nullptr;
        int rc = api.ceph_ll_create(backend_.mount_, ref->in, owned.c_str(), mode, O_CREAT | O_EXCL | O_RDWR, &child,
                                    &fh, &st, kWant, 0, perms.get());
        if (rc == -EEXIST && exclusive) {
            // Retransmitted EXCLUSIVE create: the verifier lives in atime/mtime.
            rc = api.ceph_ll_lookup(backend_.mount_, ref->in, owned.c_str(), &child, &st, kWant, 0, perms.get());
        ...
        if (exclusive) {
            ...
            rc = api.ceph_ll_setattr(backend_.mount_, child, &sb, cephapi::kSetAtime | cephapi::kSetMtime, perms.get());
...
rt::Task<Result<Created>> CephObject::mkdir(const Cred& cred, std::string_view name, const SetAttr& attrs) {
    if (!CephBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    auto dir_ok = co_await require_dir(cred);
    if (!dir_ok) co_return Err(dir_ok.error());
    std::string owned(name);
    ...
    co_return co_await rt::offload([this, ref, oc, owned, mode]() -> Result<Created> {
        ...
        struct ceph_statx st{};
        Inode* child = nullptr;
        int rc = api.ceph_ll_mkdir(backend_.mount_, ref->in, owned.c_str(), mode, &child, &st, kWant, 0, perms.get());
        if (rc < 0 || !child) return Err(backend_.map_rc(rc < 0 ? rc : -EIO));
        return created_sync(child, st, perms.get(), mode);
    });
}

rt::Task<Result<Created>> CephObject::symlink(const Cred& cred, std::string_view name, std::string_view target,
                                              const SetAttr&) {
    if (!CephBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    ...
    co_return co_await rt::offload([this, ref, oc, owned, owned_target]() -> Result<Created> {
        ...
        struct ceph_statx st{};
        Inode* child = nullptr;
        int rc = api.ceph_ll_symlink(backend_.mount_, ref->in, owned.c_str(), owned_target.c_str(), &child, &st, kWant,
                                     0, perms.get());
        if (rc < 0 || !child) return Err(backend_.map_rc(rc < 0 ? rc : -EIO));
        return created_sync(child, st, perms.get(), std::nullopt);
    });
...
rt::Task<Result<Created>> CephObject::mknod(const Cred& cred, std::string_view name, FType ftype, DevT dev,
                                            const SetAttr& attrs) {
    if (!CephBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    mode_t type_bits = 0;
    switch (ftype) {
    ...
    co_return co_await rt::offload([this, ref, oc, owned, mode, rdev]() -> Result<Created> {
        ...
        Inode* child = nullptr;
        int rc =
            api.ceph_ll_mknod(backend_.mount_, ref->in, owned.c_str(), mode, rdev, &child, &st, kWant, 0, perms.get());
        if (rc < 0 || !child) return Err(backend_.map_rc(rc < 0 ? rc : -EIO));
...
rt::Task<Result<void>> CephObject::rename(const Cred& cred, std::string_view from, Object& dst_dir,
                                          std::string_view to) {
    if (!CephBackend::valid_name(from) || !CephBackend::valid_name(to)) co_return Err(errno_from(EINVAL));
    ...
    co_return co_await rt::offload([this, sref, dref, oc, owned_from, owned_to]() -> Result<void> {
        ...
        Perms perms(api, oc);
        int rc =
            api.ceph_ll_rename(backend_.mount_, sref->in, owned_from.c_str(), dref->in, owned_to.c_str(), perms.get());
        if (rc < 0) return Err(backend_.map_rc(rc));
        return {};
    });
}

rt::Task<Result<void>> CephObject::link(const Cred& cred, Object& file, std::string_view name) {
    if (!CephBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
...
rt::Task<Result<DirPage>> CephObject::readdir(const Cred& cred, uint64_t cookie, uint32_t max_entries) {
    if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
    ...
    co_return co_await rt::offload([this, ref, oc, cookie, max_entries, enrich]() -> Result<DirPage> {
        ...
        while (page.ents.size() < max_entries) {
            ...
            // statx alone.
            rc = api.ceph_readdirplus_r(backend_.mount_, dir, &de, &st, kWant, cephapi::kStatxDontSync, nullptr);
            if (rc < 0) return Err(backend_.map_rc(rc));
            if (rc == 0) {
                page.eof = true;
                break;
...
rt::Task<Result<std::string>> CephObject::readlink() {
    ...
    co_return co_await rt::offload([this, ref]() -> Result<std::string> {
        ...
        int n = backend_.api_->ceph_ll_readlink(backend_.mount_, ref->in, buf.data(), buf.size(), backend_.root_perms_);
...
rt::Task<Result<OpenPtr>> CephObject::open(const Cred& cred, OpenFlags flags) {
    ...
    if (!opened) co_return Err(opened.error());
    co_return OpenPtr(std::make_shared<CephOpenState>(backend_.api_.get(), backend_.mount_, *opened, writable));
}
...
rt::Task<Result<uint32_t>> CephObject::read(OpenCtx ctx, uint64_t off, std::span<std::byte> out, bool& eof) {
    if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
    if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
    Fh* fh = nullptr;
    ...
    auto n = co_await rt::offload([this, fh, off, out]() -> Result<uint32_t> {
        ...
        int r = backend_.api_->ceph_ll_read(backend_.mount_, fh, static_cast<int64_t>(off), out.size(),
...
rt::Task<Result<uint32_t>> CephObject::write(OpenCtx ctx, uint64_t off, std::span<const std::byte> in,
                                             Stability stability) {
    iovec v{const_cast<std::byte*>(in.data()), in.size()};
    co_return co_await write(ctx, off, std::span<const iovec>(&v, 1), stability);
}

rt::Task<Result<uint32_t>> CephObject::write(OpenCtx ctx, uint64_t off, std::span<const iovec> iov,
                                             Stability stability) {
    if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
    if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
    Fh* fh = nullptr;
    ...
    auto done = co_await rt::offload(
        [this, fh, off, vec, total, stability, short_write, sync_fault]() mutable -> Result<uint32_t> {
            const auto& api = *backend_.api_;
            size_t done = 0, idx = 0;
            while (done < total) {
                int64_t n;
                ...
                if (short_write) {
                    ...
                    n = api.ceph_ll_writev(backend_.mount_, fh, vec.data() + idx, static_cast<int>(vec.size() - idx),
            ...
            if (stability != Stability::kUnstable) {
                int rc = sync_fault ? -EIO
                                    : api.ceph_ll_fsync(backend_.mount_, fh, stability == Stability::kDataSync ? 1 : 0);
                if (rc < 0) {
                    backend_.poison(id());
...
rt::Task<Result<void>> CephObject::commit(OpenCtx ctx, uint64_t, uint64_t) {
    ...
    co_return co_await rt::offload(
        [this, fh, sync_fault]() -> Result<void> {
    ...
    co_return co_await rt::offload(
        [this, fh, off, len]() -> Result<void> {
            int rc = backend_.api_->ceph_ll_fallocate(backend_.mount_, fh,
                                                      ...
                                                      static_cast<int64_t>(off), static_cast<int64_t>(len));
            if (rc < 0) return Err(backend_.map_rc(rc));
            return {};
        },
        rt::OffloadClass::kHeavy);
...
rt::Task<Result<uint64_t>> CephObject::copy_range(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t src_off,
    ...
    auto src_in = ref_;
    co_return co_await rt::offload(
        [this, src_in, sfh, dfh, src_off, dst_off, len]() -> Result<uint64_t> {
            ...
            while (done < want) {
                ...
                while (w < static_cast<size_t>(r)) {
                    int k =
                        api.ceph_ll_write(backend_.mount_, dfh, static_cast<int64_t>(dst_off + done + w),
                                          static_cast<size_t>(r) - w, reinterpret_cast<const char*>(buf.data()) + w);
...
rt::Task<Result<void>> CephLockMgr::lock(Object& object, const LockOwnerId& owner, LockRange range, bool exclusive,
                                         bool wait) {
...
rt::Task<Result<void>> CephLockMgr::unlock(Object& object, const LockOwnerId& owner, LockRange range) {
    auto* obj = dynamic_cast<CephObject*>(&object);
    if (!obj || &obj->backend_ != &backend_) co_return Err(errno_from(EXDEV));
    auto keep = obj->ref_;
...
rt::Task<Result<std::optional<LockConflict>>> CephLockMgr::test(Object& object, LockRange range, bool exclusive) {
    auto* obj = dynamic_cast<CephObject*>(&object);
    if (!obj || &obj->backend_ != &backend_) co_return Err(errno_from(EXDEV));
    if (obj->type() != FType::kReg) co_return Err(errno_from(EINVAL));
    auto keep = obj->ref_;
...
bool parse_options(const std::string& value, std::vector<std::pair<std::string, std::string>>& out) {
    size_t pos = 0;
...
std::unique_ptr<Backend> make_cephfs(const BackendConfig& cfg) {
    ...
    for (const auto& [key, value] : cfg.values) {
        if (key == "conf")
            c.conf = value;
        else if (key == "id" || key == "user" || key == "name")
            c.id = value;
        else if (key == "keyring")
            c.keyring = value;
        else if (key == "mon_host" || key == "monitors")
            c.mon_host = value;
        else if (key == "fs_name" || key == "filesystem")
            c.fs_name = value;
        else if (key == "subdir")
            c.subdir = value;
    ...
    if (!made) {
        LNFS_ERROR("export {}: cephfs backend config rejected: {}", cfg.path, errno_name(made.error()));
        return nullptr;
    }
