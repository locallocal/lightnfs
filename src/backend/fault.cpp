...
struct State {
    ...
    State() {
        static constexpr const char* kEnv[] = {
            "LNFS_FAULT_FSYNC_EIO",   "LNFS_FAULT_WRITE_ENOSPC", "LNFS_FAULT_WRITE_EDQUOT", "LNFS_FAULT_READ_EIO",
            "LNFS_FAULT_SHORT_WRITE", "LNFS_FAULT_SLOW_IO",      "LNFS_FAULT_JUKEBOX",
        };
...
void clear() {
    for (int i = 0; i < static_cast<int>(Kind::kCount); ++i) state().left[i].store(0, std::memory_order_relaxed);
}

int slow_ms() {
    return state().slow_ms.load(std::memory_order_relaxed);
}
void set_slow_ms(int ms) {
    state().slow_ms.store(ms, std::memory_order_relaxed);
}
