...
class LocalBackend : public Backend {
 ...
 protected:
    ...
    Result<DirPage> readdir_sync(const LocalObject& dir, int fd, std::mutex& dents_mu, uint64_t cookie,
...
class LocalObject final : public Object {
 public:
    ...
    rt::Task<Result<void>> clone(OpenCtx, Object& dst, OpenCtx, uint64_t src_off, uint64_t dst_off,
 ...
 private:
    ...
    LocalObject(LocalBackend& backend, ObjId id, FType type, int path_fd, std::string relative,
                std::shared_ptr<void> keeper = nullptr)
        : Object(std::move(id), type),
          backend_(backend),
          path_fd_(path_fd),
