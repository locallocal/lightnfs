...
class LocalBackend::FdCache {
 public:
    ...
    FdCache(LocalBackend& backend, size_t capacity)
        : backend_(backend), per_shard_capacity_(std::max<size_t>((capacity + kShards - 1) / kShards, 1)) {}
 ...
 private:
    ...
    void evict(Shard& shard) {
        ...
        if (shard.entries.size() > per_shard_capacity_) {
            ...
            if ((n & (n - 1)) == 0)
                LNFS_WARN(
                    "fd cache shard over capacity: {} entries all in use (cap {}), "
                    "{} overflows total",
...
LocalBackend::LocalBackend(Config cfg, int root_fd, int mount_fd)
    ...
    struct statx st{};
    if (::statx(root_fd_, "", AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW, STATX_BASIC_STATS | STATX_CHANGE_COOKIE, &st) == 0 &&
...
void LocalBackend::probe_v42_caps() {
    ...
    if (wrote) {
        if (::fallocate(a, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, 0, 1024) < 0 && errno == EOPNOTSUPP)
...
Result<ObjId> LocalBackend::oid_from_fd(int fd, std::string_view relative, bool remember) {
    ...
    struct statx sx{};
    if (::statx(fd, "", AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW, STATX_BTIME, &sx) == 0 && (sx.stx_mask & STATX_BTIME)) {
        uint64_t stamp = static_cast<uint64_t>(sx.stx_btime.tv_sec) * 1000000007ull + sx.stx_btime.tv_nsec;
    ...
    if (oid && remember) {
        ...
        if (fallback_paths_.size() >= cfg_.max_fallback_entries && !fallback_paths_.contains(*oid)) {
            ...
            if ((n & (n - 1)) == 0)
                LNFS_WARN(
                    "fallback handle table at cap {}: {} evictions total (old handles "
                    "go stale early)",
...
Result<Attr> LocalBackend::attr_from_fd(int fd) const {
    ...
    a.rdev = DevT{static_cast<uint32_t>(major(st.st_rdev)), static_cast<uint32_t>(minor(st.st_rdev))};
    a.fileid = st.st_ino;
    a.atime = convert_time(st.st_atim);
    a.mtime = convert_time(st.st_mtim);
    a.ctime = convert_time(st.st_ctim);
    a.change = static_cast<uint64_t>(std::max<int64_t>(a.ctime.sec, 0)) * 1000000000ull + a.ctime.nsec;
    return a;
}
...
Result<ObjPtr> LocalBackend::object_from_fd(int fd, std::string relative, bool remember) {
    ...
    return std::static_pointer_cast<Object>(
        std::shared_ptr<LocalObject>(new LocalObject(*this, *oid, attr->type, fd, std::move(relative))));
}
...
rt::Task<Result<ObjPtr>> LocalBackend::resolve(const ObjId& oid) {
    ...
    if (auto ref = path_cache_->find(oid)) {
        ...
        co_return std::static_pointer_cast<Object>(
            std::shared_ptr<LocalObject>(new LocalObject(*this, oid, ref->type, ref->fd, std::move(path), ref)));
    }
    ...
    auto ref = path_cache_->insert(oid, std::make_shared<PathCache::Entry>(oid, *opened, attr->type));
    co_return std::static_pointer_cast<Object>(
        std::shared_ptr<LocalObject>(new LocalObject(*this, oid, ref->type, ref->fd, std::move(path), ref)));
}
...
#endif
    int rc = co_await rt::uring_statx(path_fd_, "", AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW, mask, &st);
...
rt::Task<Result<ObjPtr>> LocalObject::lookup(const Cred& cred, std::string_view name) {
    ...
    int fd = co_await rt::uring_openat(path_fd_, owned.c_str(), O_PATH | O_NOFOLLOW | O_CLOEXEC, 0);
    if (fd < 0) co_return Err(errno_from_neg(fd));
    co_return backend_.object_from_fd(fd, child_path(relative_, name));
}

Result<DirPage> LocalBackend::readdir_sync(const LocalObject& dir, int fd, std::mutex& dents_mu, uint64_t cookie,
...
rt::Task<Result<DirPage>> LocalObject::readdir(const Cred& cred, uint64_t cookie, uint32_t max_entries) {
    if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
...
rt::Task<Result<OpenPtr>> LocalObject::open(const Cred&, OpenFlags flags) {
    ...
    auto opened =
        co_await rt::offload([this, writable] { return backend_.open_oid(id(), writable ? O_RDWR : O_RDONLY); });
    ...
    if (!opened) co_return Err(opened.error() == Errno::kJukebox ? Errno::kJukebox : errno_from(EOPNOTSUPP));
...
bool take_fsync_fault() {
    return fault::take(fault::Kind::kFsyncEio);
}
...
rt::Task<void> maybe_slow_io() {
    if (fault::take(fault::Kind::kSlowIo)) co_await rt::sleep_for(std::chrono::milliseconds(fault::slow_ms()));
}
}  // namespace

rt::Task<Result<uint32_t>> LocalObject::read(OpenCtx ctx, uint64_t off, std::span<std::byte> out, bool& eof) {
...
void LocalBackend::note_attr_error(const char* op, int err) {
    ...
    if ((n & (n - 1)) == 0)
        LNFS_WARN(
            "created-object {} failed: errno={} ({} occurrences total) — "
...
rt::Task<Result<AccessMask>> LocalObject::access(const Cred& cred, AccessMask want) {
    ...
    if (backend_.cfg_.identity != LocalBackend::Identity::kStrict || !base) co_return base;
...
rt::Task<Result<Attr>> LocalObject::setattr(const Cred& cred, const SetAttr& s) {
    ...
    if (!fsuid_mode) {
        ...
        if ((s.mode || s.atime_how != SetAttr::TimeHow::kOmit || s.mtime_how != SetAttr::TimeHow::kOmit) && !owner)
            co_return Err(errno_from(EPERM));
        if (s.uid && *s.uid != attr->uid && cred.uid != 0) co_return Err(errno_from(EPERM));
        if (s.gid && *s.gid != attr->gid && cred.uid != 0 && !(owner && cred.in_group(*s.gid)))
            co_return Err(errno_from(EPERM));
        if (s.size && !owner) {
    ...
    auto applied = co_await rt::offload([this, cred, s, fsuid_mode]() -> Result<void> {
        ...
        if (s.size) {
            ...
            if (::ftruncate(fd, static_cast<off_t>(*s.size)) < 0) return finish(Err(errno_from(errno)));
        }
        if (s.mode) {
            int rc = fd >= 0 ? ::fchmod(fd, *s.mode & 07777) : ::chmod(proc_fd_path(path_fd_).c_str(), *s.mode & 07777);
        ...
        if (s.uid || s.gid) {
            ...
            int rc = fd >= 0 ? ::fchown(fd, uid, gid) : ::fchownat(path_fd_, "", uid, gid, AT_EMPTY_PATH);
        ...
        if (s.atime_how != SetAttr::TimeHow::kOmit || s.mtime_how != SetAttr::TimeHow::kOmit) {
            ...
            if (s.atime_how == SetAttr::TimeHow::kServer)
                times[0].tv_nsec = UTIME_NOW;
            else if (s.atime_how == SetAttr::TimeHow::kClient)
                times[0] = to_timespec(s.atime);
            if (s.mtime_how == SetAttr::TimeHow::kServer)
                times[1].tv_nsec = UTIME_NOW;
            else if (s.mtime_how == SetAttr::TimeHow::kClient)
                times[1] = to_timespec(s.mtime);
            int rc = fd >= 0 ? ::futimens(fd, times) : ::utimensat(AT_FDCWD, proc_fd_path(path_fd_).c_str(), times, 0);
            if (rc < 0) return finish(Err(errno_from(errno)));
        }
        return finish({});
    });
...
rt::Task<Result<Created>> LocalObject::create(const Cred& cred, std::string_view name, const SetAttr& attrs,
                                              ExclVerf* verf) {
    ...
    bool fsuid_mode = backend_.cfg_.identity == LocalBackend::Identity::kSetFsuid;
    co_return co_await rt::offload([this, cred, owned, attrs, verf_copy, exclusive, fsuid_mode]() -> Result<Created> {
        ScopedFsIds ids(cred, fsuid_mode);
        mode_t mode = exclusive ? 0 : (attrs.mode.value_or(0644) & 07777);
        int fd = ::openat(path_fd_, owned.c_str(), O_CREAT | O_EXCL | O_RDWR | O_NOFOLLOW | O_CLOEXEC, mode);
        if (fd < 0 && errno == EEXIST && exclusive) {
            // Retransmitted EXCLUSIVE create: match the verifier stored in atime/mtime.
            struct stat st{};
            if (::fstatat(path_fd_, owned.c_str(), &st, AT_SYMLINK_NOFOLLOW) == 0 && S_ISREG(st.st_mode)) {
                int64_t lo = 0, hi = 0;
                verf_split(verf_copy, lo, hi);
                if (st.st_atim.tv_sec == lo && st.st_mtim.tv_sec == hi) return created_child_sync(owned);
            }
...
rt::Task<Result<Created>> LocalObject::mkdir(const Cred& cred, std::string_view name, const SetAttr& attrs) {
    if (!LocalBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    auto dir_ok = co_await require_dir_write(cred);
    if (!dir_ok) co_return Err(dir_ok.error());
    std::string owned(name);
...
rt::Task<Result<Created>> LocalObject::symlink(const Cred& cred, std::string_view name, std::string_view target,
                                               const SetAttr&) {
    if (!LocalBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    if (target.empty() || target.find('\0') != std::string_view::npos) co_return Err(errno_from(EINVAL));
    auto dir_ok = co_await require_dir_write(cred);
    if (!dir_ok) co_return Err(dir_ok.error());
    std::string owned(name), owned_target(target);
    bool fsuid_mode = backend_.cfg_.identity == LocalBackend::Identity::kSetFsuid;
    co_return co_await rt::offload([this, cred, owned, owned_target, fsuid_mode]() -> Result<Created> {
        ScopedFsIds ids(cred, fsuid_mode);
        if (::symlinkat(owned_target.c_str(), path_fd_, owned.c_str()) < 0) return Err(errno_from(errno));
        if (cred.uid != 0 && ::fchownat(path_fd_, owned.c_str(), cred.uid, cred.gid, AT_SYMLINK_NOFOLLOW) < 0)
            LocalBackend::note_attr_error("fchownat(symlink)", errno);
        return created_child_sync(owned);
    });
}

rt::Task<Result<Created>> LocalObject::mknod(const Cred& cred, std::string_view name, FType ftype, DevT dev,
                                             const SetAttr& attrs) {
    if (!LocalBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    mode_t type_bits = 0;
    switch (ftype) {
    ...
    bool fsuid_mode = backend_.cfg_.identity == LocalBackend::Identity::kSetFsuid;
    co_return co_await rt::offload([this, cred, owned, mode, rdev, fsuid_mode]() -> Result<Created> {
        ScopedFsIds ids(cred, fsuid_mode);
        if (::mknodat(path_fd_, owned.c_str(), mode, rdev) < 0) return Err(errno_from(errno));
        if (cred.uid != 0 && ::fchownat(path_fd_, owned.c_str(), cred.uid, cred.gid, AT_SYMLINK_NOFOLLOW) < 0)
...
rt::Task<Result<void>> LocalObject::rmdir(const Cred& cred, std::string_view name) {
    ...
    co_return co_await rt::offload([this, cred, owned, fsuid_mode]() -> Result<void> {
        ScopedFsIds ids(cred, fsuid_mode);
        if (::unlinkat(path_fd_, owned.c_str(), AT_REMOVEDIR) < 0) return Err(errno_from(errno));
        return {};
    });
}

rt::Task<Result<void>> LocalObject::rename(const Cred& cred, std::string_view from, Object& dst_dir,
                                           std::string_view to) {
    if (!LocalBackend::valid_name(from) || !LocalBackend::valid_name(to)) co_return Err(errno_from(EINVAL));
    ...
    bool fsuid_mode = backend_.cfg_.identity == LocalBackend::Identity::kSetFsuid;
    co_return co_await rt::offload([this, cred, owned_from, owned_to, dst_fd, fsuid_mode]() -> Result<void> {
        ScopedFsIds ids(cred, fsuid_mode);
        if (::renameat2(path_fd_, owned_from.c_str(), dst_fd, owned_to.c_str(), 0) < 0) return Err(errno_from(errno));
        return {};
    });
}

rt::Task<Result<void>> LocalObject::link(const Cred& cred, Object& file, std::string_view name) {
    if (!LocalBackend::valid_name(name)) co_return Err(errno_from(EINVAL));
    ...
    co_return co_await rt::offload([this, cred, owned, target_fd, fsuid_mode]() -> Result<void> {
        ...
        if (errno != EPERM && errno != EINVAL && errno != ENOENT) return Err(errno_from(errno));
        // AT_EMPTY_PATH linkat needs CAP_DAC_READ_SEARCH; the /proc alias works unprivileged.
        if (::linkat(AT_FDCWD, proc_fd_path(target_fd).c_str(), path_fd_, owned.c_str(), AT_SYMLINK_FOLLOW) < 0)
            return Err(errno_from(errno));
        return {};
    });
}

rt::Task<Result<uint32_t>> LocalObject::write(OpenCtx ctx, uint64_t off, std::span<const std::byte> in,
                                              Stability stability) {
    if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
    if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
    int fd = -1;
    ...
    while (done < in.size()) {
        ...
        if (n == 0)
            n = co_await rt::uring_write(
                fd, fault::take(fault::Kind::kShortWrite) ? in.subspan(done, 1) : in.subspan(done), off + done);
        if (n < 0) co_return Err(errno_from_neg(n));
        if (n == 0) co_return Err(errno_from(EIO));
        done += static_cast<size_t>(n);
    }
    if (stability != Stability::kUnstable) {
        int rc = take_fsync_fault() ? -EIO : co_await rt::uring_fsync(fd, stability == Stability::kDataSync);
        if (rc < 0) {
            backend_.poison(id());
...
rt::Task<Result<uint32_t>> LocalObject::write(OpenCtx ctx, uint64_t off, std::span<const iovec> iov,
                                              Stability stability) {
    int fd = -1;
    LocalBackend::FdCache::Ref ref;
    if (auto* os = open_state(ctx); os && os->writable()) {
    ...
    while (done < total) {
        ...
        if (n == 0 && fault::take(fault::Kind::kShortWrite)) {
            ...
            n = co_await rt::uring_write(fd, std::span(static_cast<const std::byte*>(vec[idx].iov_base), 1),
                                         off + done);
        } else if (n == 0) {
            n = co_await rt::uring_writev(fd, vec.data() + idx, static_cast<int>(vec.size() - idx), off + done);
        }
        if (n < 0) co_return Err(errno_from_neg(n));
        if (n == 0) co_return Err(errno_from(EIO));
        done += static_cast<size_t>(n);
...
rt::Task<Result<void>> LocalObject::allocate(OpenCtx ctx, uint64_t off, uint64_t len) {
    ...
    co_return co_await rt::offload(
        [fd, off, len, fsuid, cred]() -> Result<void> {
            ScopedFsIds ids(cred, fsuid);
            if (::fallocate(fd, 0, static_cast<off_t>(off), static_cast<off_t>(len)) < 0) return Err(errno_from(errno));
            return {};
        },
...
rt::Task<Result<void>> LocalObject::clone(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t src_off, uint64_t dst_off,
                                          uint64_t len) {
    ...
    Cred cred = dctx.cred;
    co_return co_await rt::offload(
        [sfd, dfd, src_off, dst_off, len, fsuid, cred]() -> Result<void> {
            ScopedFsIds ids(cred, fsuid);
            struct file_clone_range range{};
            range.src_fd = sfd;
...
rt::Task<Result<uint64_t>> LocalObject::copy_range(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t src_off,
...
std::unique_ptr<Backend> make_local(const BackendConfig& cfg) {
    ...
    for (const auto& [key, value] : cfg.values) {
        if (key == "fd_cache") {
            auto [ptr, ec] = std::from_chars(value.data(), value.data() + value.size(), local.fd_cache);
            ...
            if (value == "kernel")
                local.handles = LocalBackend::HandleMode::kKernel;
            else if (value == "fallback")
                local.handles = LocalBackend::HandleMode::kFallback;
            else if (value != "auto") {
            ...
            if (value != "true" && value != "false") {
                LNFS_ERROR("export {}: bad local backend readdir_enrich value '{}'", cfg.path, value);
                return nullptr;
            }
            local.enrich_readdir = value == "true";
        } else if (key == "identity") {
            if (value == "strict")
                local.identity = LocalBackend::Identity::kStrict;
            else if (value == "setfsuid")
                local.identity = LocalBackend::Identity::kSetFsuid;
            else if (value != "check") {
                LNFS_ERROR("export {}: bad local backend identity value '{}'", cfg.path, value);
                return nullptr;
            }
        } else if (key == "max_fallback_entries") {
            auto [ptr, ec] = std::from_chars(value.data(), value.data() + value.size(), local.max_fallback_entries);
            if (ec != std::errc{} || ptr != value.data() + value.size() || local.max_fallback_entries == 0) {
                LNFS_ERROR("export {}: bad local backend max_fallback_entries value '{}'", cfg.path, value);
                return nullptr;
            }
        } else {
