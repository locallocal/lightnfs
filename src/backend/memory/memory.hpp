...
class MemoryBackend final : public Backend {
 public:
    ...
    Result<ObjId> add_file(std::string_view path, std::span<const std::byte> data, uint32_t mode = 0644);
    Result<ObjId> add_file(std::string_view path, std::string_view data, uint32_t mode = 0644) {
        return add_file(path, std::span<const std::byte>(reinterpret_cast<const std::byte*>(data.data()), data.size()),
                        mode);
    }
 ...
 private:
    ...
    Result<ObjId> add_node(std::string_view path, FType type, uint32_t mode, std::span<const std::byte> data,
