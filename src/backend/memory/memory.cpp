...
class MemoryBackend::MemoryObject final : public Object {
 public:
    ...
    rt::Task<Result<ObjPtr>> lookup(const Cred& cred, std::string_view name) override {
        ...
        if (!allowed || !allowed->has(Access::kLookup)) co_return Err(allowed ? errno_from(EACCES) : allowed.error());
        std::lock_guard lock(backend_.mu_);
    ...
    rt::Task<Result<DirPage>> readdir(const Cred& cred, uint64_t cookie, uint32_t max_entries) override {
        if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
        auto allowed = co_await access(cred, Access::kRead);
        if (!allowed || !allowed->has(Access::kRead)) co_return Err(allowed ? errno_from(EACCES) : allowed.error());
        DirPage page;
        std::lock_guard lock(backend_.mu_);
        for (auto pos = node_->cookie_order.upper_bound(cookie); pos != node_->cookie_order.end(); ++pos) {
        ...
        page.eof = page.ents.empty() ||
                   page.ents.back().cookie == (node_->cookie_order.empty() ? 0 : node_->cookie_order.rbegin()->first);
        co_return page;
    }
    ...
    rt::Task<Result<uint32_t>> read(OpenCtx ctx, uint64_t off, std::span<std::byte> out, bool& eof) override {
    ...
    rt::Task<Result<Attr>> setattr(const Cred& cred, const SetAttr& s) override {
        ...
        auto apply_time = [&](SetAttr::TimeHow how, const Timespec& value, Timespec& out) {
            if (how == SetAttr::TimeHow::kServer)
                out = backend_.now();
            else if (how == SetAttr::TimeHow::kClient)
                out = value;
        };
    ...
    rt::Task<Result<Created>> create(const Cred& cred, std::string_view name, const SetAttr& attrs,
                                     ExclVerf* verf) override {
        if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
        ...
        auto node =
            backend_.new_child(node_, std::string(name), FType::kReg, verf ? 0 : attrs.mode.value_or(0644), cred);
        if (verf) {
    ...
    rt::Task<Result<Created>> mkdir(const Cred& cred, std::string_view name, const SetAttr& attrs) override {
        if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
        if (!valid_component(name)) co_return Err(errno_from(EINVAL));
        std::lock_guard lock(backend_.mu_);
        if (node_->children.contains(name)) co_return Err(errno_from(EEXIST));
        auto node = backend_.new_child(node_, std::string(name), FType::kDir, attrs.mode.value_or(0755), cred);
        node_->attr.nlink++;
        co_return Created{backend_.wrap(node), node->attr};
    }

    rt::Task<Result<Created>> symlink(const Cred& cred, std::string_view name, std::string_view target,
    ...
    rt::Task<Result<Created>> mknod(const Cred& cred, std::string_view name, FType ftype, DevT dev,
                                    const SetAttr& attrs) override {
        if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
        if (!valid_component(name)) co_return Err(errno_from(EINVAL));
        if (ftype != FType::kChr && ftype != FType::kBlk && ftype != FType::kSock && ftype != FType::kFifo)
            co_return Err(errno_from(EINVAL));
        std::lock_guard lock(backend_.mu_);
        if (node_->children.contains(name)) co_return Err(errno_from(EEXIST));
        auto node = backend_.new_child(node_, std::string(name), ftype, attrs.mode.value_or(0644), cred);
        node->attr.rdev = dev;
        co_return Created{backend_.wrap(node), node->attr};
    }
    ...
    rt::Task<Result<void>> rename(const Cred&, std::string_view from, Object& dst_dir, std::string_view to) override {
        if (type() != FType::kDir) co_return Err(errno_from(ENOTDIR));
    ...
    rt::Task<Result<uint32_t>> write(OpenCtx ctx, uint64_t off, std::span<const std::byte> in, Stability) override {
        if (type() == FType::kDir) co_return Err(errno_from(EISDIR));
        if (type() != FType::kReg) co_return Err(errno_from(EINVAL));
        ...
        if (fault::take(fault::Kind::kJukebox)) co_return Err(Errno::kJukebox);
        std::lock_guard lock(backend_.mu_);
        if (uint64_t cap = backend_.max_filesize_; cap && off + in.size() > cap) co_return Err(errno_from(EFBIG));
    ...
    rt::Task<Result<void>> allocate(OpenCtx ctx, uint64_t off, uint64_t len) override {
        ...
        if (!gate) co_return Err(gate.error());
        std::lock_guard lock(backend_.mu_);
        if (uint64_t cap = backend_.max_filesize_; cap && off + len > cap) co_return Err(errno_from(EFBIG));
    ...
    rt::Task<Result<void>> deallocate(OpenCtx ctx, uint64_t off, uint64_t len) override {
        ...
        if (off < size) {
            ...
            std::fill(node_->data.begin() + static_cast<size_t>(off), node_->data.begin() + static_cast<size_t>(end),
                      std::byte{0});
            touch();
        }
        co_return Result<void>{};
    ...
    rt::Task<Result<void>> clone(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t soff, uint64_t doff,
    ...
    rt::Task<Result<uint64_t>> copy_range(OpenCtx sctx, Object& dst, OpenCtx dctx, uint64_t soff, uint64_t doff,
                                          uint64_t len) override {
 ...
 private:
    ...
    rt::Task<Result<void>> io_gate(OpenCtx ctx, bool write) {
        ...
        if (!allowed->has(write ? Access::kModify : Access::kRead) && ctx.cred.uid != node_->attr.uid)
...
Timespec MemoryBackend::now() {
    return Timespec{tick_++, 0};
}

std::shared_ptr<MemoryBackend::Node> MemoryBackend::new_child(const std::shared_ptr<Node>& parent, std::string name,
...
rt::Task<Result<ObjPtr>> MemoryBackend::root() {
    co_return wrap(root_);
}
...
std::shared_ptr<MemoryBackend::Node> MemoryBackend::find_path(std::string_view path) {
    ...
    while (pos < path.size()) {
        ...
        std::string_view part = path.substr(pos, slash == std::string_view::npos ? path.size() - pos : slash - pos);
        if (part.empty() || part == ".") {
            // no-op
        } else {
...
Result<ObjId> MemoryBackend::add_node(std::string_view path, FType type, uint32_t mode, std::span<const std::byte> data,
                                      std::string_view link) {
    std::lock_guard lock(mu_);
    while (path.size() > 1 && path.back() == '/') path.remove_suffix(1);
    size_t slash = path.rfind('/');
    std::string_view parent_path = slash == std::string_view::npos ? std::string_view("/") : path.substr(0, slash);
...
Result<ObjId> MemoryBackend::add_file(std::string_view path, std::span<const std::byte> data, uint32_t mode) {
