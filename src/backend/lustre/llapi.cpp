...
class KernelOps final : public Ops {
 public:
    ...
    Result<HsmState> hsm_state(int fd) const override {
        ...
        return HsmState{hus.hus_states, hus.hus_archive_id, hus.hus_in_progress_state, hus.hus_in_progress_action};
    }
...
std::string fid_to_string(const Fid& fid) {
    char buf[64];
    std::snprintf(buf, sizeof buf, "0x%llx:0x%x:0x%x", static_cast<unsigned long long>(fid.seq), fid.oid, fid.ver);
    return buf;
}
