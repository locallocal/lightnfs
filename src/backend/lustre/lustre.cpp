...
void note_hsm_error(const char* op, Errno e) {
    ...
    uint64_t n = ++count;
    if ((n & (n - 1)) == 0) LNFS_WARN("lustre hsm {} failed: {} ({} occurrences total)", op, errno_name(e), n);
}
...
LustreBackend::LustreBackend(LocalBackend::Config base, int root_fd, int mount_fd, Config lcfg, const llapi::Ops& ops,
                             int lustre_fd, std::string mount_path)
    : LocalBackend(std::move(base), root_fd, mount_fd),
      lcfg_(std::move(lcfg)),
      ops_(ops),
      lustre_fd_(lustre_fd),
...
Result<std::unique_ptr<LustreBackend>> LustreBackend::create(Config cfg, const llapi::Ops* ops) {
    ...
    if (lfd < 0) {
        LNFS_ERROR("lustre export {}: cannot open mount root {}: errno={}", cfg.path, mount_path, errno);
        return fail(errno);
    }
    ...
    if (root_st.st_dev != mount_st.st_dev) {
        LNFS_ERROR("lustre export {}: not inside mount root {} (different device)", cfg.path, mount_path);
        ::close(lfd);
        return fail(EXDEV);
    }
    if (!o.is_lustre(lfd)) {
        LNFS_ERROR("lustre export {}: {} is not a Lustre mount (statfs magic)", cfg.path, mount_path);
        ::close(lfd);
    ...
    if (!root_fid) {
        LNFS_ERROR("lustre export {}: cannot derive the root FID: {}", cfg.path, errno_name(root_fid.error()));
    ...
    LNFS_INFO("lustre export {}: mount root {} root fid {} stripe-pref {} hsm={} locks={}", cfg.path, out->mount_path_,
              llapi::fid_to_string(*root_fid), out->limits_.pref_write, cfg.hsm, cfg.native_locks);
    return out;
}
...
Result<void> LustreBackend::hsm_gate(int fd, const llapi::Fid& fid) {
    ...
    if (state->in_progress_state == llapi::kHpsNone) {
        ...
        if (!kicked && kicked.error() != errno_from(EALREADY) && kicked.error() != errno_from(EINPROGRESS))
...
Result<int> LustreLockMgr::fd_for(const ObjId& oid, const LockOwnerId& owner, bool create) {
    ...
    if (!fd && (fd.error() == errno_from(EACCES) || fd.error() == errno_from(EROFS) || fd.error() == errno_from(EPERM)))
...
rt::Task<Result<void>> LustreLockMgr::lock(Object& object, const LockOwnerId& owner, LockRange range, bool exclusive,
                                           bool wait) {
...
rt::Task<Result<void>> LustreLockMgr::unlock(Object& object, const LockOwnerId& owner, LockRange range) {
    auto* obj = dynamic_cast<LocalObject*>(&object);
    if (!obj || &obj->backend() != &backend_) co_return Err(errno_from(EXDEV));
    ObjId oid = obj->id();
...
rt::Task<Result<std::optional<LockConflict>>> LustreLockMgr::test(Object& object, LockRange range, bool exclusive) {
    auto* obj = dynamic_cast<LocalObject*>(&object);
    if (!obj || &obj->backend() != &backend_) co_return Err(errno_from(EXDEV));
    if (obj->type() != FType::kReg) co_return Err(errno_from(EINVAL));
    ObjId oid = obj->id();
...
std::unique_ptr<Backend> make_lustre(const BackendConfig& cfg) {
    ...
    for (const auto& [key, value] : cfg.values) {
        if (key == "mount")
            ...
            if (value == "check")
                l.identity = LocalBackend::Identity::kCheck;
            else if (value == "strict")
                l.identity = LocalBackend::Identity::kStrict;
            else if (value == "setfsuid")
                l.identity = LocalBackend::Identity::kSetFsuid;
            else
                return bad("identity", value);
