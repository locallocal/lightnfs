...
class GatewayLockMgr : public backend::LockMgr {
 public:
    ...
    std::optional<backend::LockConflict> lock(const FileKey& file, const backend::LockOwnerId& owner,
                                              backend::LockRange range, bool exclusive);
    // Removes the owner's coverage of the range (POSIX split); unlocking nothing is fine.
    void unlock(const FileKey& file, const backend::LockOwnerId& owner, backend::LockRange range);
    // LOCKT: conflict test on behalf of `owner` (its own locks never conflict).
    std::optional<backend::LockConflict> test(const FileKey& file, const backend::LockOwnerId* owner,
                                              backend::LockRange range, bool exclusive);
    ...
    rt::Task<Result<void>> lock(backend::Object&, const backend::LockOwnerId&, backend::LockRange, bool exclusive,
                                bool wait) override;
    rt::Task<Result<void>> unlock(backend::Object&, const backend::LockOwnerId&, backend::LockRange) override;
    rt::Task<Result<std::optional<backend::LockConflict>>> test(backend::Object&, backend::LockRange,
 ...
 private:
    ...
    void carve(std::vector<LockSeg>& segs, const backend::LockOwnerId& owner, uint64_t start, uint64_t end);
