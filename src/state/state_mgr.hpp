...
class StateMgr {
 public:
    struct Config {
        ...
        struct NativeLocks {
            ...
            std::function<rt::Task<Result<backend::ObjPtr>>(uint32_t fsid, const backend::ObjId& oid)> resolve;
        } native_locks;
    ...
    rt::Task<ExchangeResult> exchange_id(std::string owner_id, Verifier verifier, std::string principal, bool update);
    ...
    rt::Task<CreateSessionResult> create_session(uint64_t clientid, uint32_t sequence, std::string principal,
                                                 const nfsv4::ChannelAttrs& fore_req,
                                                 const nfsv4::ChannelAttrs& back_req, uint64_t conn_id,
                                                 std::shared_ptr<transport::CbChannel> cb_chan = nullptr,
    ...
    rt::Task<uint32_t> bind_backchannel(const SessionId& id, std::shared_ptr<transport::CbChannel> chan);
    ...
    rt::Task<void> sequence_complete(const SessionId& id, uint32_t slotid, uint32_t seqid, bool cache,
    ...
    rt::Task<DelegGrant> maybe_grant_read_deleg(const SessionId& sessionid, uint64_t clientid, uint32_t fsid,
                                                const backend::ObjId& oid, std::vector<std::byte> fh, uint32_t access,
                                                bool reclaim);
    rt::Task<uint32_t> delegreturn(const Stateid& sid, uint64_t clientid, uint32_t fsid, const backend::ObjId& oid);
    ...
    void register_lock_waiter(uint32_t fsid, const backend::ObjId& oid, uint64_t clientid, std::string owner,
                              std::vector<std::byte> fh);
 ...
 private:
    ...
    rt::Task<backend::OpenPtr> unlink_state(const StateRef& rec, bool from_client, bool handover = false);
    ...
    rt::Task<Result<backend::ObjPtr>> native_lock_object(uint32_t fsid, const backend::ObjId& oid);
    rt::Task<LockResult> push_native_lock(backend::LockMgr& native, uint32_t fsid, const backend::ObjId& oid,
                                          const backend::LockOwnerId& lowner, backend::LockRange range, bool exclusive);
    static uint32_t native_lock_status(Errno error);
    void remember_lock_owner(const backend::LockOwnerId& id, std::shared_ptr<ClientRec> client, std::string owner);
    ...
    std::priority_queue<std::pair<int64_t, uint64_t>, std::vector<std::pair<int64_t, uint64_t>>, std::greater<>>
        lease_heap_;
    ...
    std::atomic<uint64_t> lease_expirations_{0}, reclaim_conflict_{0}, reclaim_timeout_{0}, reclaim_forced_{0},
        share_denied_{0}, open_merges_{0}, lock_denied_{0}, native_lock_denied_{0}, native_lock_errors_{0},
    ...
    rt::Task<bool> send_cb(std::shared_ptr<ClientRec> client,
                           std::vector<std::byte> (*build)(const nfsv4::cb::Target&, const void*), const void* args);
    ...
    std::atomic<uint64_t> deleg_grants_{0}, deleg_recalls_{0}, deleg_returns_{0}, deleg_revokes_{0},
        cb_lock_notifies_{0};
};
