...
constexpr int kReasonForced = 0, kReasonConflict = 1, kReasonTimeout = 2, kReasonReboot = 3;
...
std::string StateMgr::local_clients_dir(uint32_t fsid) const {
    return fsid == 0 ? cfg_.state_dir + "/clients" : cfg_.state_dir + "/fs/" + std::to_string(fsid) + "/clients";
}
...
void StateMgr::arm_grace(uint32_t fsid, std::vector<std::string> owners) {
    ...
    if (!window.pending.empty()) {
        ...
        if (fsid == 0)
            LNFS_INFO("grace period armed: {} clients expected to reclaim, {}s window", window.pending.size(),
                      grace_secs);
        else
            LNFS_INFO("grace period armed for fsid {}: {} clients expected to reclaim, {}s window", fsid,
...
bool StateMgr::end_grace(uint32_t fsid) {
    ...
    for (auto& [id, window] : grace_) {
        ...
        if (window.active) {
            ...
            window.active = false;
            if (id == 0)
                LNFS_INFO("grace period ended by operator (grace-end)");
            else
                LNFS_INFO("grace period for fsid {} ended", id);
        }
    }
...
bool StateMgr::window_live(uint32_t fsid, GraceWindow& window) const {
    ...
    if (window.pending.empty() || std::chrono::steady_clock::now() >= window.deadline) {
        ...
        if (fsid == 0)
            ...
            LNFS_INFO("grace period over for fsid {}", fsid);
        return false;
    }
    return true;
}
    ...
    for (auto& [id, window] : grace_) {
        ...
        if (window.pending.empty() && window.active) {
            ...
            if (id == 0)
                ...
                LNFS_INFO("all listed clients reclaimed for fsid {}: leaving its grace early", id);
        }
    }
}
...
void StateMgr::persist_client(const ClientRec& client) {
    // listed per export on its first state instead
    if (cfg_.per_fsid_reclaim) return;
    if (cfg_.stable.put) {
...
void StateMgr::persist_fs_client(uint32_t fsid, std::string_view owner_id) {
    if (cfg_.stable.put)
        cfg_.stable.put(fsid, owner_id);
    else
        write_client_record(local_clients_dir(fsid), owner_id);
}

void StateMgr::unpersist_fs_client(uint32_t fsid, std::string_view owner_id) {
    if (cfg_.stable.erase)
        cfg_.stable.erase(fsid, owner_id);
    else
        (void)::unlink(client_record_path(local_clients_dir(fsid), owner_id).c_str());
}
...
rt::Task<StateMgr::ExchangeResult> StateMgr::exchange_id(std::string owner_id, Verifier verifier, std::string principal,
                                                         bool update) {
    ExchangeResult out;
    ...
    // EXCHGID4_FLAG_UPD_CONFIRMED_REC_A (RFC 8881 §18.35 update cases)
    if (update) {
        if (!slot.confirmed) {
        ...
        if (slot.confirmed == nullptr && slot.unconfirmed == nullptr) shard.by_owner.erase(owner_id);
        co_return out;
    }
    ...
    if (slot.confirmed) {
        ...
        if (c.principal != principal) {
            // held state: owner is taken
            if (!c.sessions.empty() || !c.states.empty()) {
            ...
            // same incarnation: report confirmed record
            renew(c);
    ...
    // always replaced by a fresh registration
    if (slot.unconfirmed) {
    ...
    if (cfg_.max_clients != 0 &&
        client_count_.load(std::memory_order_relaxed) >= static_cast<int64_t>(cfg_.max_clients)) {
        if (!slot.confirmed) shard.by_owner.erase(owner_id);
        out.status = as_u32(Status::kResource);
        co_return out;
    }
    auto rec = std::make_shared<ClientRec>();
    rec->clientid = (cfg_.boot_epoch << 32) | next_client_.fetch_add(1, std::memory_order_relaxed);
...
rt::Task<StateMgr::CreateSessionResult> StateMgr::create_session(uint64_t clientid, uint32_t sequence,
                                                                 std::string principal,
                                                                 const nfsv4::ChannelAttrs& fore_req,
                                                                 const nfsv4::ChannelAttrs& back_req, uint64_t conn_id,
                                                                 std::shared_ptr<transport::CbChannel> cb_chan,
                                                                 uint32_t cb_program, nfsv4::cb::Cred cb_cred) {
    ...
    // client reboot: its state dies (§4.8)
    std::shared_ptr<ClientRec> old_incarnation;
    ...
    std::vector<StateOther> orphaned_states;
    {
        ...
        // replay
        if (sequence + 1 == client->cs_sequence && client->confirmed) {
            out.replay = true;
        ...
        // confirmation: this incarnation replaces the old one
        if (!client->confirmed) {
            ...
            if (slot.confirmed && slot.confirmed != client) {
                ...
                orphaned_states.assign(old_incarnation->states.begin(), old_incarnation->states.end());
        ...
        session->fore.max_response_cached = std::min(fore_req.max_response_cached, cfg_.max_cached_reply);
        ...
        // CB_SEQUENCE + one op is all we ever send
        session->back.max_ops = 2;
    ...
    // old incarnation's sessions die now
    ...
    // ...and so does its open state (RFC 8881 §8.4.2 / notes 4.8)
    if (old_incarnation) {
        ...
        LNFS_INFO("client {:#x} rebooted: released {} open states of the old incarnation", old_incarnation->clientid,
                  orphaned_states.size());
        // backend handles drop here, outside every shard lock
        released.clear();
    }
    co_return out;
}

rt::Task<void> StateMgr::confirm_create_session(uint64_t clientid, std::vector<std::byte> reply) {
    ...
    if (auto client = find_client_sync(clientid)) {
        ...
        if (shard.by_id.contains(clientid)) {
            ...
            if (!client->persisted) {
                // claimed under the lock; the write happens outside
...
rt::Task<uint32_t> StateMgr::destroy_session(const SessionId& id, uint64_t conn_id) {
    std::shared_ptr<SessionRec> session;
    {
        ...
        if (!it->second->bound_conns.contains(conn_id)) co_return as_u32(Status::kConnNotBound);
        session = std::move(it->second);
        shard.table.erase(it);
        session_count_.fetch_sub(1, std::memory_order_relaxed);
        // wake any in-flight duplicate waiters
        wake_all_slots(*session);
    }
...
rt::Task<uint32_t> StateMgr::destroy_clientid(uint64_t clientid) {
    ...
    if (!rec) co_return as_u32(Status::kStaleClientid);
    {
        ...
        if (it == shard.by_id.end() || it->second != rec)
            // raced with expiry/replacement
            co_return as_u32(Status::kStaleClientid);
        if (!rec->sessions.empty() || !rec->states.empty()) co_return as_u32(Status::kClientidBusy);
        ...
        if (slot_it != shard.by_owner.end()) {
            ...
            if (!slot_it->second.confirmed && !slot_it->second.unconfirmed) shard.by_owner.erase(slot_it);
        }
    ...
    std::erase_if(lock_owners_, [&](const auto& kv) { return kv.second.client->clientid == clientid; });
    co_return 0;
}
...
rt::Task<uint32_t> StateMgr::bind_backchannel(const SessionId& id, std::shared_ptr<transport::CbChannel> chan) {
    SessionShard& shard = session_shard(id);
    auto lock = co_await shard.mu.lock();
    auto it = shard.table.find(id);
    if (it == shard.table.end()) co_return as_u32(Status::kBadsession);
    ...
    // fresh path, fresh chance
    session.cb_down.store(false, std::memory_order_relaxed);
    co_return 0;
}

// ---- SEQUENCE --------------------------------------------------------------

rt::Task<StateMgr::SeqResult> StateMgr::sequence_begin(const SessionId& id, uint32_t slotid, uint32_t seqid,
    ...
    for (;;) {
        auto it = shard.table.find(id);
        if (it == shard.table.end() || it->second->client->expired.load(std::memory_order_relaxed)) {
            // gone, or reclaim chain in progress
            out.status = as_u32(Status::kBadsession);
            co_return out;
        }
        SessionRec& session = *it->second;
        // implicit bind (trunking-lenient)
        session.bound_conns.insert(conn_id);
        ...
        if (seqid == slot.seqid + 1) {
            ...
            if (session.client->delegs.load(std::memory_order_relaxed) > 0) {
                std::lock_guard cb(session.cb_mu);
                bool cb_alive =
                    session.cb_chan && session.cb_chan->alive() && !session.cb_down.load(std::memory_order_relaxed);
                if (!cb_alive) out.status_flags |= 0x1;
            }
            // SEQ4_STATUS_LEASE_MOVED (design 10 §10.4, plan 12 C3): an export this client
        ...
        // replay
        if (seqid == slot.seqid && slot.seqid != 0) {
            out.replay = true;
            if (slot.cached) {
...
rt::Task<void> StateMgr::sequence_complete(const SessionId& id, uint32_t slotid, uint32_t seqid, bool cache,
...
rt::Task<backend::OpenPtr> StateMgr::unlink_state(const StateRef& rec, bool from_client, bool handover) {
    backend::OpenPtr released;
    {
        StateShard& shard = state_shard(rec->other);
        auto lock = co_await shard.mu.lock();
        // raced with CLOSE / another reclaim
        if (rec->closed) co_return nullptr;
        rec->closed = true;
        shard.table.erase(rec->other);
        if (rec->type == StateType::kOpen) open_count_.fetch_sub(1, std::memory_order_relaxed);
        // under the state shard: IO-path copies are safe
        released = std::move(rec->bopen);
    }
    FileKey key{rec->fsid, rec->oid};
    // lock states derived from a closing open
    std::vector<StateRef> dependents;
    {
    ...
    if (rec->type == StateType::kLock) {
        // plain mutex table: no shard lock held
        locks_.release_owner(key, rec->lowner);
        ...
        if (backend::LockMgr* native = handover ? nullptr : native_lock_mgr(rec->fsid)) {
            ...
            Result<void> released = obj ? co_await native->release(**obj, rec->lowner) : Result<void>(Err(obj.error()));
            if (!released) {
                native_lock_errors_.fetch_add(1, std::memory_order_relaxed);
                LNFS_WARN("native lock release failed (fsid {}): {}", rec->fsid, errno_name(released.error()));
            }
        }
    ...
    if (from_client) {
        ...
        // from this very list.
        if (last_in_fs && cfg_.per_fsid_reclaim && !handover) unpersist_fs_client(rec->fsid, rec->client->owner_id);
    }
    // caller drops it with no lock held
    co_return released;
}
...
rt::Task<StateMgr::OpenResult> StateMgr::open(OpenArgs args, backend::OpenPtr bopen) {
    ...
    StateRef rec;
    backend::OpenPtr released;
    // write open vs. read delegations (§5.2)
    ...
    for (int attempt = 0;; ++attempt) {
        uint64_t courtesy_conflict = 0;
        {
            ...
            for (const auto& other : file.opens) {
                if (other->closed || other->client->expired.load(std::memory_order_relaxed))
                    // dead entries awaiting unlink never arbitrate
                    continue;
                ...
                if (share_conflict(args.access, args.deny, *other)) {
                    if (other->client->courtesy.load(std::memory_order_relaxed)) {
                        // conflict reclaims it (7.4)
                        courtesy_conflict = other->client->clientid;
                        break;
                    }
                    denied = true;
                    break;
            ...
            if (courtesy_conflict == 0 && !denied && (args.access & kShareWrite) && !args.deleg_claim) {
                for (const auto& d : file.delegs)
                    if (!d->closed && !d->client->expired.load(std::memory_order_relaxed)) deleg_recalls.push_back(d);
            }
            if (!deleg_recalls.empty()) {
                // Nothing created; fall out of the lock scope to start the recalls.
            } else if (courtesy_conflict == 0) {
                if (denied) {
                    if (file.opens.empty() && file.locks.empty() && file.delegs.empty()) shard.table.erase(key);
                    share_denied_.fetch_add(1, std::memory_order_relaxed);
                    out.status = as_u32(Status::kShareDenied);
                    co_return out;
                }
                // same owner + file: union, seqid++ (RFC 8881 §18.16.3)
                if (mine) {
                    mine->access |= args.access;
                    mine->deny |= args.deny;
                    mine->seqid += 1;
                    // the record keeps its original handle
                    released = std::move(bopen);
                    rec = mine;
                    out.merged = true;
        ...
        // pathological: keep the invariant rather than spin
        if (attempt >= 8) {
            out.status = as_u32(Status::kDelay);
            co_return out;
        }
    ...
    if (!out.merged) {
        ...
        // reclaim chain ran between arbitration and registration
        if (dead) {
    ...
    // superseded backend handle: dropped with no lock held
    released.reset();
    co_return out;
}

rt::Task<Stateid> StateMgr::open_read(uint64_t clientid, uint32_t fsid, const backend::ObjId& oid) {
    OpenArgs args;
...
rt::Task<StateMgr::DelegGrant> StateMgr::maybe_grant_read_deleg(const SessionId& sessionid, uint64_t clientid,
    ...
    // never reach the client.
    {
        ...
        SessionRec& session = *it->second;
        std::lock_guard cb(session.cb_mu);
        if (!session.cb_chan || !session.cb_chan->alive() || session.cb_down.load(std::memory_order_relaxed))
            co_return out;
    }
    auto client = co_await find_client(clientid);
    if (!client || client->expired.load(std::memory_order_relaxed)) co_return out;
    {
        ClientShard& shard = owner_shard(client->owner_id);
        auto lock = co_await shard.mu.lock();
        if (cfg_.max_states_per_client != 0 && client->states.size() >= cfg_.max_states_per_client) co_return out;
    }
    ...
    FileKey key{fsid, oid};
    {
        ...
        for (const auto& o : file.opens) {
            ...
            if (o->client != client && (o->access.load(std::memory_order_relaxed) & kShareWrite))
                // live write open elsewhere: not coherent to delegate
                co_return out;
        }
        for (const auto& d : file.delegs) {
            if (d->closed) continue;
            if (d->client == client || d->recalled.load(std::memory_order_relaxed))
                // one per client; nothing new while a recall is running
                co_return out;
        }
    ...
    // reclaim chain ran mid-grant: retract
    if (dead) {
...
rt::Task<uint32_t> StateMgr::delegreturn(const Stateid& sid, uint64_t clientid, uint32_t fsid,
    ...
    if (rec->type != StateType::kDeleg || rec->client->clientid != clientid || rec->fsid != fsid || !(rec->oid == oid))
...
rt::Task<uint32_t> StateMgr::deleg_conflict(uint32_t fsid, const backend::ObjId& oid) {
    // fast path
    if (deleg_count_.load(std::memory_order_relaxed) == 0) co_return 0;
    ...
    FileKey key{fsid, oid};
    {
        ...
        if (it != shard.table.end()) {
            for (const auto& d : it->second.delegs)
                if (!d->closed && !d->client->expired.load(std::memory_order_relaxed)) recalls.push_back(d);
        }
    }
...
rt::Task<uint32_t> StateMgr::check_deleg_claim(const Stateid& sid, uint64_t clientid, uint32_t fsid,
                                               const backend::ObjId& oid) {
    if (epoch_of(sid.other) != static_cast<uint32_t>(cfg_.boot_epoch)) co_return as_u32(Status::kStaleStateid);
    StateRef rec;
    {
...
void StateMgr::start_recall(const StateRef& deleg) {
    // once only
    if (deleg->recalled.exchange(true, std::memory_order_relaxed)) return;
    deleg->recall_deadline.store(now_coarse() + cfg_.lease_seconds, std::memory_order_relaxed);
    {
...
rt::Task<void> StateMgr::recall_task(StateRef deleg) {
    ...
    if (!sent)
        LNFS_WARN(
            "CB_RECALL undeliverable (client {:#x}): delegation will be revoked at "
            "the deadline",
            deleg->client->clientid);
}

rt::Task<bool> StateMgr::send_cb(std::shared_ptr<ClientRec> client,
                                 std::vector<std::byte> (*build)(const nfsv4::cb::Target&, const void*),
                                 const void* args) {
    ...
    std::shared_ptr<transport::CbChannel> chan;
    {
        ...
        if (session->cb_inflight) {
            ...
            if (now - session->cb_inflight_since > static_cast<int64_t>(cfg_.lease_seconds))
    ...
    bool ok = false;
    {
        ...
        if (!reply) {
            ...
            // any well-formed RPC answer proves the path is alive
            ok = rs.rpc_ok;
...
rt::Task<void> StateMgr::sweep_recalls() {
    ...
    int64_t now = now_coarse();
    {
        ...
        for (size_t i = 0; i < recall_watch_.size();) {
            ...
            // DELEGRETURN / client expiry beat the deadline
            if (rec->closed) {
    ...
    for (auto& rec : due) {
        LNFS_WARN("delegation recall deadline passed: revoking (client {:#x})", rec->client->clientid);
        (void)co_await unlink_state(rec, true);
...
void StateMgr::register_lock_waiter(uint32_t fsid, const backend::ObjId& oid, uint64_t clientid, std::string owner,
    ...
    for (auto& w : waiters) {
        if (w.clientid == clientid && w.owner == owner) {
            // re-armed by the retry
            w.expires = expires;
            return;
        }
    }
    // bounded, best effort
    if (waiters.size() >= 16) return;
...
void StateMgr::notify_lock_waiters(const FileKey& key) {
    ...
    int64_t now = now_coarse();
    for (auto& w : waiters) {
        // the waiter gave up a lease ago
        if (w.expires < now) continue;
        rt::spawn(notify_task(w.clientid, std::move(w.owner), std::move(w.fh)), rt::current_reactor());
    }
}

rt::Task<void> StateMgr::notify_task(uint64_t clientid, std::string owner, std::vector<std::byte> fh) {
    auto client = co_await find_client(clientid);
    ...
    bool ok = co_await send_cb(
        ...
        [](const nfsv4::cb::Target& t, const void* p) {
            ...
            return nfsv4::cb::build_cb_notify_lock(
                t, *a->fh, a->clientid,
                std::span<const std::byte>(reinterpret_cast<const std::byte*>(a->owner->data()), a->owner->size()));
        },
        &args);
    if (ok) cb_lock_notifies_.fetch_add(1, std::memory_order_relaxed);
}

rt::Task<StateMgr::IoCheck> StateMgr::check_io(const Stateid& sid, uint64_t clientid, uint32_t fsid,
                                               const backend::ObjId& oid, uint32_t need) {
    IoCheck out;
    if (sid.is_special()) {
        out.special = true;
        // stateless writes wait for reclaim
        if (need == kShareWrite && in_grace(fsid)) {
            out.status = as_u32(Status::kGrace);
            co_return out;
        }
        // READ bypass: ignores share deny
        if (sid.is_all_one()) co_return out;
    ...
    if (epoch_of(sid.other) != static_cast<uint32_t>(cfg_.boot_epoch)) {
        // pre-restart stateid: no table walk
...
rt::Task<StateMgr::StateLookup> StateMgr::lookup_stateid(const Stateid& sid) {
    ...
    if (it == shard.table.end()) {
        ...
        out.rec =
            OpenRec{rec.client->clientid, rec.fsid, rec.oid, rec.seqid.load(), rec.access.load(), rec.deny.load()};
    }
    co_return out;
}
...
rt::Task<uint32_t> StateMgr::close_state(const Stateid& sid, uint64_t clientid, Stateid* out) {
    ...
    // backend handle: after the last lock (07 §6.1)
    released.reset();
    co_return 0;
}
...
rt::Task<uint32_t> StateMgr::open_downgrade(const Stateid& sid, uint64_t clientid, uint32_t access, uint32_t deny,
    ...
    if (access == 0 || (access & ~rec->access) != 0 || (deny & ~rec->deny) != 0) co_return as_u32(Status::kInval);
    {
        FileShard& shard = file_shard(FileKey{rec->fsid, rec->oid});
        // serializes with merges on this file
        auto lock = co_await shard.mu.lock();
        rec->access = access;
        rec->deny = deny;
        rec->seqid += 1;
...
rt::Task<uint32_t> StateMgr::free_stateid(const Stateid& sid) {
    ...
    if (locks_.count_owner(FileKey{rec->fsid, rec->oid}, rec->lowner) > 0) co_return as_u32(Status::kLocksHeld);
    (void)co_await unlink_state(rec, true);
    co_return 0;
}
...
void StateMgr::remember_lock_owner(const backend::LockOwnerId& id, std::shared_ptr<ClientRec> client,
                                   std::string owner) {
    std::lock_guard g(lock_owner_mu_);
...
std::optional<StateMgr::LockOwnerRec> StateMgr::find_lock_owner(const backend::LockOwnerId& id) {
    std::lock_guard g(lock_owner_mu_);
    auto it = lock_owners_.find(std::string(reinterpret_cast<const char*>(id.bytes.data()), id.len));
...
rt::Task<StateMgr::LockResult> StateMgr::lock(LockArgs args) {
    ...
    // same grace gate as OPEN(CLAIM_PREVIOUS)
    if (args.reclaim) {
    ...
    if (args.new_owner) {
        if (args.open_stateid.is_special() ||
            ...
            out.status = args.open_stateid.is_special() ? as_u32(Status::kBadStateid) : as_u32(Status::kStaleStateid);
            co_return out;
        }
        {
        ...
        if (args.lock_stateid.is_special() ||
            ...
            out.status = args.lock_stateid.is_special() ? as_u32(Status::kBadStateid) : as_u32(Status::kStaleStateid);
            co_return out;
        }
        {
    ...
    if (args.new_owner && cfg_.max_states_per_client != 0 && held_states >= cfg_.max_states_per_client) {
        out.status = as_u32(Status::kResource);
        co_return out;
    }
    if (cfg_.max_lock_segments_per_owner != 0 && locks_.count_owner(key, lowner) >= cfg_.max_lock_segments_per_owner) {
        out.status = as_u32(Status::kResource);
        co_return out;
    }
    ...
    if (native) {
        auto pushed = co_await push_native_lock(*native, args.fsid, args.oid, lowner, range, args.exclusive);
        if (pushed.status != 0) {
            locks_.release_owner(key, lowner);
            for (const auto& seg : before)
                (void)locks_.lock(key, lowner, GatewayLockMgr::to_range(seg.start, seg.end), seg.exclusive);
    ...
    bool created = false;
    {
        ...
        if (!lock_rec) {
            ...
            // bumped below
            lock_rec->seqid = 0;
            if (file.opens.empty() && file.locks.empty()) file_count_.fetch_add(1, std::memory_order_relaxed);
    ...
    if (created) {
        ...
        if (first_in_fs && cfg_.per_fsid_reclaim) persist_fs_client(lock_rec->fsid, client->owner_id);
    }
...
rt::Task<StateMgr::LockResult> StateMgr::lockt(uint64_t clientid, uint32_t fsid, const backend::ObjId& oid,
                                               std::string owner, bool exclusive, uint64_t offset, uint64_t length) {
    LockResult out;
    auto client = co_await find_client(clientid);
    if (!client || !client->confirmed) {
        out.status = as_u32(Status::kStaleClientid);
...
rt::Task<Result<backend::ObjPtr>> StateMgr::native_lock_object(uint32_t fsid, const backend::ObjId& oid) {
...
uint32_t StateMgr::native_lock_status(Errno error) {
    if (error == Errno::kJukebox || error == errno_from(ENOTCONN) || error == errno_from(ETIMEDOUT) ||
        error == errno_from(EAGAIN))
        return as_u32(Status::kDelay);
    if (error == errno_from(ESTALE) || error == errno_from(ENOENT)) return as_u32(Status::kStale);
    return as_u32(Status::kServerfault);
}

rt::Task<StateMgr::LockResult> StateMgr::push_native_lock(backend::LockMgr& native, uint32_t fsid,
                                                          const backend::ObjId& oid, const backend::LockOwnerId& lowner,
...
rt::Task<uint32_t> StateMgr::locku(const Stateid& sid, uint64_t clientid, uint64_t offset, uint64_t length,
    ...
    if (backend::LockMgr* native = native_lock_mgr(rec->fsid)) {
        ...
        Result<void> released =
            obj ? co_await native->unlock(**obj, rec->lowner, {offset, length}) : Result<void>(Err(obj.error()));
        if (!released) {
            native_lock_errors_.fetch_add(1, std::memory_order_relaxed);
    ...
    {
        FileShard& shard = file_shard(FileKey{rec->fsid, rec->oid});
        // serializes seqid bumps with LOCK
        auto lock = co_await shard.mu.lock();
        rec->seqid += 1;
    }
    if (out) {
...
rt::Task<void> StateMgr::scan_leases() {
    ...
    int64_t now = now_coarse();
    int64_t courtesy_window =
        static_cast<int64_t>(cfg_.lease_seconds) * std::max<uint32_t>(1, cfg_.courtesy_multiplier);
    std::vector<std::shared_ptr<ClientRec>> due;
    {
        ...
        while (!lease_heap_.empty() && lease_heap_.top().first <= now) {
            ...
            auto it = client_idx_.find(clientid);
            // client already gone
            if (it == client_idx_.end()) continue;
            // superseded duplicate
            if (it->second->lease_heap_deadline != when) continue;
    ...
    for (const auto& client : due) {
        ...
        if (client->courtesy.load(std::memory_order_relaxed)) {
            if (now - client->courtesy_since >= courtesy_window) {
                // reclaim drops the record
                timed_out.push_back(client->clientid);
            } else {
        ...
        // renewed since the entry was armed
        if (expiry > now) {
            arm_lease_check(*client, expiry);
            continue;
        }
        ...
        // replaced/gone
        if (it == shard.by_id.end() || it->second != client) continue;
        ClientRec& c = *client;
        ...
        lease_expirations_.fetch_add(1, std::memory_order_relaxed);
        LNFS_INFO("client {:#x} ({} sessions, {} states) lease expired: courtesy", c.clientid, c.sessions.size(),
                  c.states.size());
...
rt::Task<uint32_t> StateMgr::expire_client_impl(uint64_t clientid, int reason) {
    ...
    switch (reason) {
        case kReasonConflict:
            reclaim_conflict_.fetch_add(1, std::memory_order_relaxed);
            break;
        case kReasonTimeout:
            reclaim_timeout_.fetch_add(1, std::memory_order_relaxed);
            break;
        case kReasonForced:
            reclaim_forced_.fetch_add(1, std::memory_order_relaxed);
            break;
        default:
            break;
    }
    LNFS_INFO("client {:#x} reclaimed ({}): {} states, {} sessions released", clientid,
...
rt::Task<size_t> StateMgr::release_fsid(uint32_t fsid) {
    ...
    for (const auto& rec : victims) {
        // an open already took this lock with it
    ...
    (void)end_grace(fsid);
    LNFS_INFO("fsid {} handed over: {} states dropped, {} clients to be told LEASE_MOVED", fsid, dropped, moved.size());
    // backend handles drop here, outside every shard lock
    released.clear();
    co_return dropped;
}
...
rt::Task<std::string> StateMgr::dump() {
    std::string out;
    int64_t now = now_coarse();
    out += std::format("grace={} remaining={}s boot_epoch={}\n", in_grace() ? 1 : 0, grace_remaining_seconds(),
                       cfg_.boot_epoch);
    for (const auto& [fsid, left] : stats().fs_grace) out += std::format("grace fsid={} remaining={}s\n", fsid, left);
    for (size_t si = 0; si < shard_count_; ++si) {
        ...
        for (const auto& [id, c] : shard.by_id) {
            ...
            out += std::format(
                ...
                id, owner_hex, c->principal, c->confirmed ? 1 : 0, c->courtesy.load(std::memory_order_relaxed) ? 1 : 0,
                c->lease_expiry.load(std::memory_order_relaxed) - now, c->sessions.size(), c->states.size(),
                c->reclaim_complete ? 1 : 0, c->lease_moved_until.load(std::memory_order_relaxed) > now ? 1 : 0);
        }
    }
    for (size_t si = 0; si < shard_count_; ++si) {
        ...
        for (const auto& [id, s] : shard.table) {
            ...
            out += std::format("session {} client={:#x} slots={} cached={} in_flight={} conns={}\n", sid_hex,
                               s->client->clientid, s->slot_count, cached, in_flight, s->bound_conns.size());
        }
    }
    for (size_t si = 0; si < shard_count_; ++si) {
        ...
        for (const auto& [other, rec] : shard.table) {
            std::string oid_hex;
            for (std::byte b : rec->oid.view()) oid_hex += std::format("{:02x}", static_cast<unsigned>(b));
            std::string owner_hex;
                ...
                for (const auto& seg : locks_.segments(FileKey{rec->fsid, rec->oid}))
                    if (same_owner(seg.owner, rec->lowner))
                        ranges += std::format("[{},{}){} ", seg.start,
                                              seg.end == UINT64_MAX ? std::string("EOF") : std::to_string(seg.end),
                                              seg.exclusive ? "W" : "R");
                out += std::format("lock {} seqid={} client={:#x} fsid={} oid={} owner={} parent={} ranges={}\n",
            ...
            out += std::format("open {} seqid={} client={:#x} fsid={} oid={} owner={} access={} deny={}\n",
                               hex_other(other), rec->seqid.load(), rec->client->clientid, rec->fsid, oid_hex,
