...
void GatewayLockMgr::carve(std::vector<LockSeg>& segs, const backend::LockOwnerId& owner, uint64_t start,
                           uint64_t end) {
...
void GatewayLockMgr::coalesce(std::vector<LockSeg>& segs) {
    std::sort(segs.begin(), segs.end(), [](const LockSeg& a, const LockSeg& b) { return a.start < b.start; });
    std::vector<LockSeg> out;
    for (const LockSeg& s : segs) {
        bool merged = false;
        // Adjacent/overlapping same-owner same-type segments fuse (POSIX coalescing).
        for (auto it = out.rbegin(); it != out.rend(); ++it) {
            if (same_owner(it->owner, s.owner) && it->exclusive == s.exclusive && it->end >= s.start &&
                s.end >= it->start) {
    ...
    std::sort(out.begin(), out.end(), [](const LockSeg& a, const LockSeg& b) { return a.start < b.start; });
    segs.swap(out);
}

void GatewayLockMgr::account(Shard& s, const FileKey& key, std::vector<LockSeg>& segs, size_t before) {
    size_t after = segs.size();
    if (after > before)
        total_.fetch_add(after - before, std::memory_order_relaxed);
    else if (before > after)
        total_.fetch_sub(before - after, std::memory_order_relaxed);
    if (after == 0) {
        s.files.erase(key);
...
std::optional<backend::LockConflict> GatewayLockMgr::lock(const FileKey& file, const backend::LockOwnerId& owner,
...
void GatewayLockMgr::unlock(const FileKey& file, const backend::LockOwnerId& owner, backend::LockRange range) {
    uint64_t start = range.offset, end = range_end(range);
    Shard& s = shard(file);
    std::lock_guard g(s.mu);
...
std::optional<backend::LockConflict> GatewayLockMgr::test(const FileKey& file, const backend::LockOwnerId* owner,
                                                          backend::LockRange range, bool exclusive) {
    uint64_t start = range.offset, end = range_end(range);
    Shard& s = shard(file);
    std::lock_guard g(s.mu);
...
size_t GatewayLockMgr::release_owner(const FileKey& file, const backend::LockOwnerId& owner) {
    ...
    auto& segs = it->second;
    segs.erase(
        std::remove_if(segs.begin(), segs.end(), [&](const LockSeg& seg) { return same_owner(seg.owner, owner); }),
        segs.end());
...
rt::Task<Result<void>> GatewayLockMgr::lock(backend::Object& obj, const backend::LockOwnerId& owner,
                                            backend::LockRange range, bool exclusive, bool /*wait*/) {
    if (lock(FileKey{0, obj.id()}, owner, range, exclusive)) co_return Err(errno_from(EAGAIN));
    co_return Result<void>{};
}

rt::Task<Result<void>> GatewayLockMgr::unlock(backend::Object& obj, const backend::LockOwnerId& owner,
                                              backend::LockRange range) {
    unlock(FileKey{0, obj.id()}, owner, range);
    co_return Result<void>{};
}

rt::Task<Result<std::optional<backend::LockConflict>>> GatewayLockMgr::test(backend::Object& obj,
                                                                            backend::LockRange range, bool exclusive) {
