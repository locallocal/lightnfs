...
uint32_t st(Status s) {
    return static_cast<uint32_t>(s);
}

uint64_t conn_id_of(ConnCtx& conn) {
    return reinterpret_cast<uint64_t>(&conn);
}
...
constexpr uint32_t kAccessRead = 0x01, kAccessLookup = 0x02, kAccessModify = 0x04, kAccessExtend = 0x08,
                   kAccessDelete = 0x10, kAccessExecute = 0x20;
...
bool is_current_placeholder(const Stateid& sid) {
    ...
    return std::all_of(sid.other.begin(), sid.other.end(), [](std::byte b) { return b == std::byte{0}; });
}
...
void Engine::register_with(rpc::Dispatcher& dispatcher) {
    dispatcher.add(
        {kProgram, kVersion, kVersion, this, [](void* self, ConnCtx& ctx, RpcCall& call, const rpc::Cred& cred) {
...
Engine::FhBytes Engine::export_fh(const core::ExportEntry& exp, const backend::ObjId& oid) const {
...
rt::Task<void> Engine::compound(ConnCtx& conn, RpcCall& call, const rpc::Cred& cred) {
    ...
    auto log_summary = [&](uint32_t status, uint32_t ops_done) {
        ...
        if (dbg) {
            LNFS_DEBUG("v4 xid={:#x} peer={} tag=\"{}\" minor={} ops={}/[{}] st={} dur={}us", call.xid,
                       conn.peer.to_string(),
                       std::string_view(reinterpret_cast<const char*>(tag_bytes.data()), tag_ok ? tag_bytes.size() : 0),
                       *minor, ops_done, dbg_ops, status, us);
        }
        if (status != 0)
            obs::record_error_reply(conn.peer.to_string(), fail_op ? op_name(fail_op) : "COMPOUND", call.xid, status);
        ...
        if (auto thr = obs::slow_request_threshold_us(); thr != 0 && us >= thr) {
            ...
            LNFS_WARN("v4 slow compound xid={:#x} peer={} dur={}us st={} ops={}/[{}]", call.xid, conn.peer.to_string(),
                      us, status, ops_done, spans);
        }
    };
    ...
    if (static_cast<Op>(*first_op) == Op::kSequence) {
        ...
        auto seq = co_await state_.sequence_begin(ctx.sessionid, *sl, *sq, *hi, *ct, conn_id_of(conn));
        if (seq.replay && !seq.replay_bytes.empty()) {
            ...
            if (dbg)
                LNFS_DEBUG("v4 xid={:#x} peer={} slot={} seq={} replay (cached compound)", call.xid,
                           conn.peer.to_string(), ctx.slotid, ctx.seqid);
        ...
        ctx.max_response = ctx.cachethis ? std::min(seq.max_response, seq.max_response_cached) : seq.max_response;
        enc.u32(static_cast<uint32_t>(Op::kSequence));
        ...
        for (uint32_t i = 1; i < *numops && status == st(Status::kOk); ++i) {
            ...
            if (enc.size() + 16 > ctx.max_response) {
                status = ctx.cachethis && enc.size() + 16 <= seq.max_response ? st(Status::kRepTooBigToCache)
        ...
        co_await state_.sequence_complete(ctx.sessionid, ctx.slotid, ctx.seqid, ctx.cachethis, std::move(cache_bytes));
...
rt::Task<uint32_t> Engine::exec_op(Ctx& ctx, uint32_t opcode, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    auto t0 = std::chrono::steady_clock::now();
    ...
    if (ctx.span_count < Ctx::kMaxSpans)
        ctx.spans[ctx.span_count++] = {opcode, static_cast<uint32_t>(std::min<uint64_t>(us, UINT32_MAX))};
    co_return status;
}

rt::Task<uint32_t> Engine::exec_op_impl(Ctx& ctx, uint32_t opcode, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    switch (static_cast<Op>(opcode)) {
        ...
        case Op::kReadPlus: {
            ...
            switch (static_cast<Op>(opcode)) {
                case Op::kSeek:
                    co_return co_await op_seek(ctx, dec, enc);
                case Op::kAllocate:
                    co_return co_await op_allocate(ctx, dec, enc, false);
                case Op::kDeallocate:
                    co_return co_await op_allocate(ctx, dec, enc, true);
                case Op::kCopy:
                    co_return co_await op_copy(ctx, dec, enc);
                case Op::kReadPlus:
                    co_return co_await op_read_plus(ctx, dec, enc);
                default:
                    co_return co_await op_clone(ctx, dec, enc);
            }
        }
        default:
            break;
...
rt::Task<uint32_t> Engine::absent_attr_reply(Ctx& ctx, core::ExportEntry& exp, const core::PseudoFs::Node* crossing,
...
rt::Task<uint32_t> Engine::attr_reply(Ctx& ctx, const Resolved& resolved, const Bitmap& wanted, xdr::XdrEnc& enc) {
    AttrSource src;
...
rt::Task<uint32_t> Engine::op_access(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    uint32_t supported = kAccessRead | kAccessLookup | kAccessModify | kAccessExtend | kAccessDelete | kAccessExecute;
    uint32_t granted = 0;
    if (resolved->pseudo()) {
        ...
        if (resolved->exp->readonly)
            allowed->clear(backend::Access::kModify).clear(backend::Access::kExtend).clear(backend::Access::kDelete);
...
rt::Task<uint32_t> Engine::op_read(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    auto check = co_await state_.check_io(*sid, ctx.clientid, resolved->exp->fsid, resolved->oid, state::kShareRead);
    if (check.status != 0) {
    ...
    auto n = co_await resolved->obj->read(open, *offset, std::span<std::byte>(data.data(), len), eof);
    if (!n) {
...
rt::Task<uint32_t> Engine::op_read_plus(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (sparse) {
        ...
        if (hole && *hole > *offset) len = static_cast<uint32_t>(std::min<uint64_t>(len, *hole - *offset));
    }
    auto data = ctx.conn.pool.alloc(std::max<uint32_t>(len, 1));
    bool eof = false;
    auto n = co_await resolved->obj->read(open, *offset, std::span<std::byte>(data.data(), len), eof);
...
rt::Task<uint32_t> Engine::op_readdir(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    size_t slack = 256;
    size_t budget =
        ctx.max_response > enc.size() + slack ? std::min<size_t>(*maxcount, ctx.max_response - enc.size() - slack) : 0;
    if (budget < 128) {
    ...
    auto emit = [&](uint64_t entry_cookie, std::string_view name, const backend::Attr& attr, const FhBytes& fh,
                    uint64_t fsid_val, uint64_t mounted_on, const core::FsProps* ent_fs,
                    const AbsentEntry* absent = nullptr) -> bool {
    ...
    if (resolved->pseudo()) {
        ...
        for (const auto& [name, child] : resolved->node->children) {
            ...
            if (child->exp && role_of(child->exp->fsid) != core::FsRole::kActive) {
                ...
                ok = emit(this_cookie, name, attr, pseudo_fh(*child), child->exp->fsid, child->id, nullptr, &absent);
            } else if (child->exp) {
                if (!exports_.check_client(ctx.conn.peer.addr, *child->exp)) continue;
                auto obj = co_await child->exp->backend->root();
                if (!obj) continue;
                ...
                ok = emit(this_cookie, name, *attr, export_fh(*child->exp, (*obj)->id()), child->exp->fsid, child->id,
                          &child_fs);
            } else {
                auto attr = ctx.set->pseudo->attr_of(*child);
        ...
        for (;;) {
            ...
            for (auto& ent : page.ents) {
                ...
                if (!emit(ent.cookie, ent.name, attr, export_fh(*resolved->exp, oid), resolved->exp->fsid, attr.fileid,
                          &fs))
                    break;
            }
            if (truncated) break;
...
rt::Task<uint32_t> Engine::op_open(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (*opentype == kOpenCreate) {
        ...
        if (create_mode == kCreateUnchecked || create_mode == kCreateGuarded || create_mode == kCreateExclusive41) {
    ...
    if (*claim == kClaimNull) {
        ...
        if (dir->pseudo()) {
            ...
            uint32_t code = dir->node->children.contains(name) ? st(Status::kIsdir) : st(Status::kNoent);
            enc.u32(code);
            co_return code;
        }
        ...
        if (!create) {
            ...
            if (create_mode == kCreateExclusive || create_mode == kCreateExclusive41) {
                ...
                created_new = made.has_value();
                if (!made && made.error() == errno_from(EEXIST) && create_mode == kCreateUnchecked) {
    ...
    if (deleg_claim) {
        ...
        uint32_t code = co_await state_.check_deleg_claim(deleg_claim_sid, ctx.clientid, exp->fsid, file->id());
        if (code != 0) {
            enc.u32(code);
            co_return code;
        }
    ...
    if (!deleg_claim && *claim != kClaimDelegPrevFh && want != 0x0400 && want != 0x0500) {
        grant = co_await state_.maybe_grant_read_deleg(ctx.sessionid, ctx.clientid, exp->fsid, file->id(), ctx.cfh,
                                                       access, is_reclaim);
    }
    enc.u32(st(Status::kOk));
...
rt::Task<uint32_t> Engine::op_delegreturn(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    uint32_t code = co_await state_.delegreturn(*sid, ctx.clientid, resolved->exp->fsid, resolved->oid);
    enc.u32(code);
    co_return code;
}
...
rt::Task<uint32_t> Engine::op_open_downgrade(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    auto sid = Stateid::decode(dec);
    auto seqid = dec.u32();
    auto access = dec.u32();
    auto deny = dec.u32();
...
rt::Task<uint32_t> Engine::op_write(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    auto check = co_await state_.check_io(*sid, ctx.clientid, resolved->exp->fsid, resolved->oid, state::kShareWrite);
    if (check.status != 0) {
        enc.u32(check.status);
        co_return check.status;
    }
    ...
    uint32_t count = static_cast<uint32_t>(std::min<size_t>(*data_len, resolved->exp->backend->limits().max_write));
    ...
    auto written =
        co_await resolved->obj->write(open, *offset, std::span<const iovec>(iov.data(), iov.size()), stability);
    if (!written) {
...
rt::Task<uint32_t> Engine::op_setattr(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (attrs.size) {
        ...
        if (resolved->obj->type() != backend::FType::kReg) co_return fail(st(Status::kWrongType));
        // Truncation is a write: the stateid must grant WRITE (or be special).
        if (uint32_t cur = resolve_current(ctx, *sid); cur != st(Status::kOk)) co_return fail(cur);
        auto check =
            co_await state_.check_io(*sid, ctx.clientid, resolved->exp->fsid, resolved->oid, state::kShareWrite);
        if (check.status != 0) co_return fail(check.status);
    }
    MutateGuard guard(locks_, *resolved->exp, ctx.cred);
    if (auto verdict = guard.precheck({}); !verdict) co_return fail(verdict_status4(verdict));
    // Attribute changes invalidate read delegations (plan doc 10 §5.2): recall + DELAY.
    if (uint32_t dc = co_await state_.deleg_conflict(resolved->exp->fsid, resolved->oid); dc != 0) co_return fail(dc);
    co_await guard.enter({resolved->obj, resolved->oid, /*sample=*/false});
...
rt::Task<uint32_t> Engine::op_verify(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc, bool nverify) {
    ...
    if (nverify)
        code = same ? st(Status::kSame) : st(Status::kOk);
    else
        code = same ? st(Status::kOk) : st(Status::kNotSame);
    enc.u32(code);
    co_return code;
}
...
rt::Task<uint32_t> Engine::op_create(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if ((*type == kNf4Lnk && !caps.has(backend::Cap::kSymlink)) ||
        ((*type == kNf4Blk || *type == kNf4Chr || *type == kNf4Sock || *type == kNf4Fifo) &&
    ...
    switch (*type) {
        case kNf4Dir:
            made = co_await dir->obj->mkdir(cred, *name, attrs);
            break;
        case kNf4Lnk:
            made = co_await dir->obj->symlink(cred, *name, linkdata, attrs);
            break;
        case kNf4Blk:
    ...
    enc.u32(st(Status::kOk));
    encode_change_info(enc, true, guard.first().change_before(), guard.first().change_after());
    attrset.encode(enc);
    co_return st(Status::kOk);
}
...
rt::Task<uint32_t> Engine::op_remove(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (uint32_t dc = co_await state_.deleg_conflict(dir->exp->fsid, (*target)->id()); dc != 0) {
        enc.u32(dc);
        co_return dc;
    }
    Result<void> removed = (*target)->type() == backend::FType::kDir ? co_await dir->obj->rmdir(cred, *name)
...
rt::Task<uint32_t> Engine::op_rename(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (auto victim = co_await to->obj->lookup(cred, *newname); victim) {
        if (uint32_t dc = co_await state_.deleg_conflict(to->exp->fsid, (*victim)->id()); dc != 0) {
            enc.u32(dc);
            co_return dc;
        }
    }
    ...
    enc.u32(st(Status::kOk));
    encode_change_info(enc, true, guard.first().change_before(), guard.first().change_after());
    encode_change_info(enc, true, guard.second().change_before(), guard.second().change_after());
    co_return st(Status::kOk);
}
...
rt::Task<uint32_t> Engine::op_secinfo_no_name(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    auto style = dec.u32();
    if (!style || *style > 1) {
        enc.u32(st(Status::kBadxdr));
        co_return st(Status::kBadxdr);
...
rt::Task<uint32_t> Engine::op_reclaim_complete(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
...
void encode_lock_denied(xdr::XdrEnc& enc, const state::StateMgr::LockDenied& d) {
    ...
    enc.u64(d.clientid);
    enc.opaque(std::span<const std::byte>(reinterpret_cast<const std::byte*>(d.owner.data()), d.owner.size()));
}
...
rt::Task<uint32_t> Engine::op_lock(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (result.status == st(Status::kDenied)) {
        ...
        if (ctx.session && waiter_known)
            state_.register_lock_waiter(target->exp->fsid, target->oid, ctx.clientid, std::move(waiter_owner), ctx.cfh);
...
rt::Task<uint32_t> Engine::op_lockt(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    auto result = co_await state_.lockt(ctx.clientid, target->exp->fsid, target->oid,
...
rt::Task<Result<Engine::Resolved>> Engine::resolve_regular(Ctx& ctx, const FhBytes& fh, Op op, uint32_t* status) {
    *status = 0;
    if (fh.empty()) {
        *status = st(Status::kNofilehandle);
        co_return Err(errno_from(EINVAL));
...
rt::Task<uint32_t> Engine::op_allocate(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc, bool deallocate) {
...
rt::Task<uint32_t> Engine::op_copy(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (!src_sid || !dst_sid || !src_off || !dst_off || !count || !consecutive || !synchronous || !nservers) {
        enc.u32(st(Status::kBadxdr));
        co_return st(Status::kBadxdr);
    }
    uint32_t status = 0;
    ...
    auto scheck = co_await state_.check_io(*src_sid, ctx.clientid, src->exp->fsid, src->oid, state::kShareRead);
    if (scheck.status != 0) {
        enc.u32(scheck.status);
        co_return scheck.status;
    }
    auto dcheck = co_await state_.check_io(*dst_sid, ctx.clientid, dst->exp->fsid, dst->oid, state::kShareWrite);
    if (dcheck.status != 0) {
    ...
    auto second = same ? nullptr : locks_.get(dst->exp->fsid, src->oid < dst->oid ? dst->oid : src->oid);
    ...
    while (done < length) {
        ...
        std::optional<decltype(held1)> held2;
        if (second) held2.emplace(co_await second->lock());
        auto part = co_await src->obj->copy_range(sopen, *dst->obj, dopen, *src_off + done, *dst_off + done, chunk);
        if (!part) {
...
rt::Task<uint32_t> Engine::op_clone(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    co_await guard.enter({src->obj, src->oid, /*sample=*/false}, {dst->obj, dst->oid, /*sample=*/false});
...
rt::Task<uint32_t> Engine::op_exchange_id(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    auto result =
        co_await state_.exchange_id(std::string(reinterpret_cast<const char*>(ownerid->data()), ownerid->size()),
...
rt::Task<uint32_t> Engine::op_create_session(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    ...
    if (!clientid || !sequence || !flags || !fore || !back || !cb_program || !sec_count || *sec_count > 8) {
        enc.u32(st(Status::kBadxdr));
        co_return st(Status::kBadxdr);
    }
    ...
    if (fore->max_request < 128 || fore->max_response < 128 || back->max_request < 128 || back->max_response < 128) {
    ...
    auto result =
        co_await state_.create_session(*clientid, *sequence, ctx.cred.principal(), *fore, *back, conn_id_of(ctx.conn),
...
rt::Task<uint32_t> Engine::op_destroy_session(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    auto sid = dec.opaque_fixed(16);
    if (!sid) {
        enc.u32(st(Status::kBadxdr));
        co_return st(Status::kBadxdr);
...
rt::Task<uint32_t> Engine::op_destroy_clientid(Ctx& ctx, xdr::XdrDec& dec, xdr::XdrEnc& enc) {
    auto clientid = dec.u64();
    if (!clientid) {
        enc.u32(st(Status::kBadxdr));
        co_return st(Status::kBadxdr);
