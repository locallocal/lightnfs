...
bool Stateid::is_all_zero() const {
    ...
    return std::all_of(other.begin(), other.end(), [](std::byte b) { return b == std::byte{0}; });
}

bool Stateid::is_all_one() const {
    if (seqid != 0xffffffffu) return false;
    return std::all_of(other.begin(), other.end(), [](std::byte b) { return b == std::byte{0xff}; });
}
...
const char* op_name(uint32_t op) {
    switch (static_cast<Op>(op)) {
        ...
        case Op::kIoAdvise:
            return "IO_ADVISE";
        case Op::kLayouterror:
            return "LAYOUTERROR";
        case Op::kLayoutstats:
            return "LAYOUTSTATS";
        case Op::kOffloadCancel:
            return "OFFLOAD_CANCEL";
        case Op::kOffloadStatus:
