...
class Engine {
 public:
    ...
    Engine(core::ExportTable& exports, core::FileHandleCodec& handles, core::ObjLockRegistry& locks,
           state::StateMgr& state, std::string server_owner = "lightnfs", std::string server_scope = "lightnfs",
 ...
 private:
    ...
    // (plan doc 10 §3.1/§3.6).
    rt::Task<uint32_t> exec_op(Ctx& ctx, uint32_t opcode, xdr::XdrDec& dec, xdr::XdrEnc& enc);
    rt::Task<uint32_t> exec_op_impl(Ctx& ctx, uint32_t opcode, xdr::XdrDec& dec, xdr::XdrEnc& enc);
    ...
    rt::Task<uint32_t> op_delegreturn(Ctx&, xdr::XdrDec&, xdr::XdrEnc&);
    ...
    rt::Task<Result<Resolved>> resolve_regular(Ctx&, const FhBytes& fh, Op op, uint32_t* status);
    ...
    rt::Task<uint32_t> absent_attr_reply(Ctx&, core::ExportEntry& exp, const core::PseudoFs::Node* crossing,
                                         const Bitmap& wanted, xdr::XdrEnc&);
