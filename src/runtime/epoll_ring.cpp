...
void EpollRing::prep_writev(OpHandle* op, int fd, const iovec* iov, int iovcnt, uint64_t off) {
...
void EpollRing::prep_statx(OpHandle* op, int dirfd, const char* path, int flags, unsigned mask, struct statx* out) {
...
size_t EpollRing::wait(std::span<Completion> out, std::optional<std::chrono::nanoseconds> timeout) {
    {
        std::lock_guard lk(rmu_);
...

    {
        ...
        int ms;
        if (!ready_.empty() || (timeout && timeout->count() == 0))
            ms = 0;
        else if (!timeout)
            ms = -1;
        else
            ms = static_cast<int>((timeout->count() + 999999) / 1000000);
