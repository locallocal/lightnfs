...
Runtime::Runtime(Config cfg) {
    int n = cfg.reactors > 0 ? cfg.reactors : static_cast<int>(std::thread::hardware_concurrency());
    if (n <= 0) n = 1;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (cfg.ring == "uring" || cfg.ring == "auto") {
            auto r = UringRing::create(UringRing::Setup{
                .sq_entries = cfg.ring_entries, .cq_entries = cfg.ring_cq_entries, .sqpoll = cfg.ring_sqpoll});
            if (r) {
                if (uring_mode.empty()) uring_mode = (*r)->multishot_accept() ? "multishot-accept" : "single-accept";
                ring = std::move(*r);
                ring_kind_ = "uring";
    ...
    offload_ = std::make_unique<OffloadPool>(OffloadPool::Config{.threads = cfg.offload_threads,
    ...
    LNFS_INFO("runtime: {} reactors, ring={}, offload_threads={}", reactors_.size(), ring_kind_, cfg.offload_threads);
    if (ring_kind_ == "uring")
        LNFS_INFO(
            "uring probe: required opcodes supported "
