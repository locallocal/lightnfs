...
class BufferPool {
 public:
    ...
    static constexpr size_t kClassCaps[] = {4 * 1024, 64 * 1024, 128 * 1024, 256 * 1024, 1024 * 1024};
...
class BufferChain {
 public:
    ...
    std::vector<std::byte> to_bytes() const {
        ...
        out.reserve(size_);
        for (const auto& s : segs_) out.insert(out.end(), s.buf.data() + s.off, s.buf.data() + s.off + s.len);
        return out;
    }
