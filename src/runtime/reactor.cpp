...
ReactorGuard::ReactorGuard(Reactor* r) : prev_(t_current) {
    t_current = r;
}
ReactorGuard::~ReactorGuard() {
    t_current = prev_;
}
...
void Reactor::run() {
    ...
    for (;;) {
        ...
        if (stop_.load(std::memory_order_acquire) && live_tasks() == 0 && pending_ops_ == 0 && remote_.empty() &&
            local_ready_.empty()) {
            break;
        }
...
struct SpawnRoot {
    struct promise_type {
        SpawnRoot get_return_object() { return {std::coroutine_handle<promise_type>::from_promise(*this)}; }
