...
class EpollRing final : public RingOps {
 public:
    ...
    void prep_statx(OpHandle* op, int dirfd, const char* path, int flags, unsigned mask, struct statx* out) override;
 ...
 private:
    ...
    FdQ* fdq(int fd) { return fd >= 0 && static_cast<size_t>(fd) < socks_.size() ? socks_[fd].get() : nullptr; }
    FdQ& fdq_make(int fd) {
