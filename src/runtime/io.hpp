...
inline auto uring_read(int fd, std::span<std::byte> buf, uint64_t off) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_read(op, fd, buf, off); }};
}
inline auto uring_write(int fd, std::span<const std::byte> buf, uint64_t off) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_write(op, fd, buf, off); }};
}
inline auto uring_writev(int fd, const iovec* iov, int iovcnt, uint64_t off) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_writev(op, fd, iov, iovcnt, off); }};
}
inline auto uring_fsync(int fd, bool datasync) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_fsync(op, fd, datasync); }};
}
...
inline auto uring_sendv(int fd, const iovec* iov, int iovcnt) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_sendv(op, fd, iov, iovcnt); }};
}
inline auto uring_accept(int fd, sockaddr* addr, socklen_t* alen) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_accept(op, fd, addr, alen); }};
}
inline auto uring_statx(int dirfd, const char* path, int flags, unsigned mask, struct statx* out) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_statx(op, dirfd, path, flags, mask, out); }};
}
inline auto uring_openat(int dirfd, const char* path, int flags, mode_t mode) {
    return detail::IoAwaiter{[=](RingOps& ring, OpHandle* op) { ring.prep_openat(op, dirfd, path, flags, mode); }};
}
...
inline SleepAwaiter sleep_for(std::chrono::nanoseconds d) {
    return {d};
}
