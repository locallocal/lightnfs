...
OffloadPool::OffloadPool(Config cfg) {
    ...
    for (int i = 0; i < light; ++i) threads_.emplace_back([this, i] { worker(groups_[0], static_cast<size_t>(i)); });
    for (int i = 0; i < heavy; ++i) threads_.emplace_back([this, i] { worker(groups_[1], static_cast<size_t>(i)); });
}
...
void OffloadPool::submit(MoveOnlyFn job, OffloadClass cls) {
    ...
    if (g.depth.load(std::memory_order_relaxed) >= g.queue_cap && !stopping_.load(std::memory_order_relaxed)) {
