...
constexpr std::pair<int, const char*> kRequiredOps[] = {
    {IORING_OP_READ, "read"},           {IORING_OP_WRITE, "write"},   {IORING_OP_WRITEV, "writev"},
    {IORING_OP_RECV, "recv"},           {IORING_OP_ACCEPT, "accept"}, {IORING_OP_OPENAT, "openat"},
    {IORING_OP_CLOSE, "close"},         {IORING_OP_FSYNC, "fsync"},   {IORING_OP_STATX, "statx"},
    {IORING_OP_ASYNC_CANCEL, "cancel"},
};
...
Result<std::unique_ptr<UringRing>> UringRing::create(const Setup& setup) {
...
#if defined(IORING_SETUP_SINGLE_ISSUER) && defined(IORING_SETUP_DEFER_TASKRUN)
    ladder[ladder_n++] = {IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN | IORING_SETUP_R_DISABLED, true};
#endif
#if defined(IORING_SETUP_SINGLE_ISSUER) && defined(IORING_SETUP_COOP_TASKRUN)
    ladder[ladder_n++] = {IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_COOP_TASKRUN | IORING_SETUP_R_DISABLED, true};
#endif
#ifdef IORING_SETUP_COOP_TASKRUN
    ...
    for (;;) {
        for (int i = 0; i < ladder_n; ++i) {
            ...
            if (ladder[i].flags & IORING_SETUP_SQPOLL) params.sq_thread_idle = setup.sqpoll_idle_ms;
            ...
            if (ladder[i].flags & IORING_SETUP_SQPOLL)
                LNFS_WARN("io_uring SQPOLL requested but unavailable (errno={}); falling back", -rc);
        }
    ...
    if (cq_entries != cq_target)
        LNFS_WARN("io_uring CQ reduced {} -> {} (memlock/memory pressure)", cq_target, cq_entries);
    r->ring_init_ = true;
    r->enabled_ = !r->needs_enable_;
    if (auto missing = missing_opcodes(&r->ring_); !missing.empty()) {
        LNFS_WARN("io_uring lacks required opcodes: {} (kernel too old for the uring ring)", missing);
    ...
    LNFS_DEBUG("uring setup: mode={} sq={} cq={} multishot_accept={}", flags_name(r->setup_flags_), setup.sq_entries,
               cq_entries, r->multishot_accept_);
...
int UringRing::submit_all() {
    ...
    if (rc < 0) {
        ...
        if ((n & (n - 1)) == 0)
            LNFS_WARN(
                "io_uring_submit failed: errno={} ({} failures total, {} SQEs queued "
                "for retry)",
...
void UringRing::prep_writev(OpHandle* op, int fd, const iovec* iov, int iovcnt, uint64_t off) {
    auto* sqe = get_sqe();
...
void UringRing::prep_statx(OpHandle* op, int dirfd, const char* path, int flags, unsigned mask, struct statx* out) {
    auto* sqe = get_sqe();
...
size_t UringRing::wait(std::span<Completion> out, std::optional<std::chrono::nanoseconds> timeout) {
    ...
    if (!rearm_pending_.empty()) {
        ...
        for (int fd : fds) {
            auto it = accept_streams_.find(fd);
            if (it != accept_streams_.end() && !it->second.armed && it->second.waiter) arm_multishot(fd, it->second);
        }
    }
