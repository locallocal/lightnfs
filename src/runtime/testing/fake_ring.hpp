...
class FakeRing final : public RingOps {
 public:
    enum class Kind { kRead, kWrite, kWritev, kFsync, kRecv, kSendv, kAccept, kStatx, kOpenat, kClose, kCancelFd };
    ...
    void complete_with_data(const Op& o, std::string_view s) {
        complete_with_data(o, std::span<const std::byte>(reinterpret_cast<const std::byte*>(s.data()), s.size()));
    }
    ...
    void prep_fsync(OpHandle* op, int fd, bool) override { pending.push_back(Op{op, Kind::kFsync, fd}); }
    ...
    void prep_statx(OpHandle* op, int dirfd, const char* path, int, unsigned, struct statx*) override {
    ...
    void prep_close(OpHandle* op, int fd) override { pending.push_back(Op{op, Kind::kClose, fd}); }
    ...
    size_t wait(std::span<Completion> out, std::optional<std::chrono::nanoseconds> timeout) override {
        ...
        if (ready.empty()) {
            if (!timeout) {
                ...
                std::fprintf(stderr, "FakeRing: reactor blocked forever (%zu pending ops, nothing ready)\n",
                             pending.size());
                std::abort();
            }
            return 0;
