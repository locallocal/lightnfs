...
struct FidHash {
    size_t operator()(const Fid& f) const noexcept {
        return std::hash<uint64_t>{}(f.seq) ^ (std::hash<uint64_t>{}(f.oid) << 1) ^ (std::hash<uint64_t>{}(f.ver) << 2);
    }
};
...
Result<Fid> derive_fid(int fd) {
    struct statx sx{};
    if (::statx(fd, "", AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW, STATX_BASIC_STATS | STATX_BTIME, &sx) < 0)
