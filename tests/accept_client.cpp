...
std::vector<std::byte> build_call(BufferPool& pool, uint32_t xid, uint32_t prog, uint32_t proc,
...
struct Client {
    ...
    std::vector<std::byte> call(uint32_t prog, uint32_t proc, const std::vector<std::byte>& body,
...
void enc_fh(XdrEnc& e, const Fh& fh) {
    e.opaque(fh);
}
...
std::pair<Fh, std::optional<Fattr>> lookup(Client& c, const Fh& dir, const std::string& name) {
    XdrEnc args(c.pool);
    enc_fh(args, dir);
    args.string(name);
...
std::pair<std::vector<std::byte>, bool> read_range(Client& c, const Fh& fh, uint64_t offset, uint32_t count) {
    XdrEnc args(c.pool);
    enc_fh(args, fh);
    args.u64(offset);
    args.u32(count);
...
std::vector<std::byte> read_all(Client& c, const Fh& fh, uint64_t size, uint32_t chunk) {
    ...
    while (out.size() < size) {
        ...
        if (data.empty()) fatal("READ: zero-length data before size %llu reached", (unsigned long long)size);
        out.insert(out.end(), data.begin(), data.end());
        if (eof) break;
    }
    if (out.size() != size) fatal("READ: got %zu bytes, expected %llu", out.size(), (unsigned long long)size);
    return out;
}
...
std::vector<DirEntry> readdirplus_all(Client& c, const Fh& dir) {
    ...
    for (;;) {
        ...
        auto dec = make_dec(reply);
        uint32_t status = ru32(dec);
        if (status != kNfs3Ok) fatal("READDIRPLUS cookie=%llu: status %u", (unsigned long long)cookie, status);
        decode_post_attr(dec);
        ...
        while (rbool(dec)) {
            ...
            if (!seen.insert(ent.name).second) fatal("READDIRPLUS: duplicate entry '%s'", ent.name.c_str());
    ...
    if (!seen.count(".") || !seen.count("..")) fatal("READDIRPLUS: missing synthesized . or ..");
    return out;
}
...
std::vector<std::string> readdir_all(Client& c, const Fh& dir) {
    ...
    for (;;) {
        ...
        auto dec = make_dec(reply);
        uint32_t status = ru32(dec);
        if (status != kNfs3Ok) fatal("READDIR cookie=%llu: status %u", (unsigned long long)cookie, status);
        decode_post_attr(dec);
...
void walk_dir(Client& c, const Fh& dir_fh, const fs::path& local, uint32_t chunk, WalkStats& st) {
    ++st.dirs;
    auto entries = readdirplus_all(c, dir_fh);

    std::unordered_set<std::string> local_names;
    for (const auto& e : fs::directory_iterator(local)) local_names.insert(e.path().filename().string());
    ...
    for (auto& ent : entries) {
        ...
        if (!local_names.count(ent.name))
            fatal("%s: server lists '%s' which is absent locally", local.c_str(), ent.name.c_str());
    }
    if (remote_count != local_names.size())
        fatal("%s: server lists %zu entries, backing dir has %zu", local.c_str(), remote_count, local_names.size());
        ...
        switch (attr.type) {
            ...
            case 1: {
                ...
                if (attr.size != local_size)
                    fatal("%s: size mismatch server=%llu local=%llu", child.c_str(), (unsigned long long)attr.size,
                ...
                if (local_size > 0) {
                    ...
                    if (!data.empty() || !eof)
                        fatal("%s: empty file READ returned %zu bytes eof=%d", child.c_str(), data.size(), (int)eof);
                    ++st.empty_checked;
                }
                ++st.files;
                break;
...
void negative_checks(Client& c, const Fh& root, bool readonly_export) {
    ...
    for (uint32_t proc : {2u, 7u, 8u, 9u, 10u, 11u, 12u, 13u, 14u, 15u, 21u}) {
        ...
        if (stat != 4) fatal("write proc %u with empty args: expected GARBAGE_ARGS(4), got %u", proc, stat);
    }

    uint32_t granted = access_bits(c, root, 0x3f);
    if (!(granted & 0x01) || !(granted & 0x02)) fatal("ACCESS did not grant read+lookup on root: 0x%x", granted);
    if (readonly_export) {
        // ACCESS on a read-only export must never grant modify/extend/delete, and a
        // well-formed CREATE must answer ROFS.
        if (granted & (0x04 | 0x08 | 0x10)) fatal("ACCESS on readonly export granted write bits: 0x%x", granted);
        XdrEnc cr(c.pool);
        enc_fh(cr, root);
...
int cmd_walk(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
             const fs::path& backing, bool readonly_export) {
    ...
    std::printf(
        "accept_client walk OK: %zu dirs, %zu files (%zu bytes verified), "
        "%zu symlinks, negative checks passed\n",
        st.dirs, st.files, st.bytes, st.symlinks);
    return 0;
}

int cmd_bigdir(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
               const fs::path& backing, const std::string& subdir, size_t expected) {
    auto root = mnt(host, mount_port, export_path);
    Client c(host, nfs_port);
    ...
    if (names.size() != expected)
        fatal("bigdir %s: READDIR returned %zu entries, expected %zu", subdir.c_str(), names.size(), expected);
    ...
    for (const auto& e : fs::directory_iterator(backing / subdir)) {
        if (!remote.count(e.path().filename().string()))
            fatal("bigdir: local entry '%s' missing from READDIR", e.path().filename().c_str());
        ++local_count;
    }
    if (local_count != expected) fatal("bigdir: backing dir has %zu entries, expected %zu", local_count, expected);
    std::printf(
        "accept_client bigdir OK: %zu entries, no duplicates, no omissions "
        "(%.2fs full pagination)\n",
        names.size(), secs);
    return 0;
}

int cmd_stress(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
               const fs::path& backing, const std::string& rel_file, int conns, int pipeline, int seconds) {
    auto root = mnt(host, mount_port, export_path);
    Fh file_fh;
    {
        Client c(host, nfs_port);
    ...
    for (int t = 0; t < conns; ++t) {
        threads.emplace_back([&, t] {
            ...
            auto reap = [&] {
                ...
                if (n != expect)
                    fatal("stress: READ off=%llu len=%u returned %u (expected %u)", (unsigned long long)p.off, p.len, n,
                          expect);
    ...
    std::printf(
        "accept_client stress OK: %llu ops over %d conns x %d pipeline in %ds "
        "(%.0f ops/s), all payloads verified\n",
        (unsigned long long)total_ops.load(), conns, pipeline, seconds, (double)total_ops.load() / seconds);
    return 0;
}
...
uint32_t nfs_create(Client& c, const Fh& dir, const std::string& name, uint32_t mode, std::optional<uint32_t> file_mode,
                    const std::array<std::byte, 8>* verf, Fh* fh_out) {
    XdrEnc args(c.pool);
    enc_fh(args, dir);
    args.string(name);
    args.u32(mode);
    if (mode == 2)
        ...
        enc_sattr(args, file_mode, std::nullopt);
...
uint32_t nfs_write(Client& c, const Fh& fh, uint64_t off, std::span<const std::byte> data, uint32_t stable,
                   std::array<std::byte, 8>* verf_out) {
    XdrEnc args(c.pool);
    enc_fh(args, fh);
    args.u64(off);
...
uint32_t nfs_setattr(Client& c, const Fh& fh, std::optional<uint32_t> mode, std::optional<uint64_t> size) {
    XdrEnc args(c.pool);
    enc_fh(args, fh);
...
uint32_t nfs_rename(Client& c, const Fh& from_dir, const std::string& from, const Fh& to_dir, const std::string& to) {
    XdrEnc args(c.pool);
...
uint32_t nfs_symlink(Client& c, const Fh& dir, const std::string& name, const std::string& target, Fh* fh_out) {
    XdrEnc args(c.pool);
    enc_fh(args, dir);
    args.string(name);
...
int cmd_wtest(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
    ...
    Fh file;
    if (nfs_create(c, work, "data.bin", 0, 0644u, nullptr, &file) != kNfs3Ok) fatal("create data.bin failed");
    auto blob = pattern_bytes(256 * 1024 + 13, 42);
    std::array<std::byte, 8> verf{}, verf2{};
    size_t third = blob.size() / 3;
    if (nfs_write(c, file, 0, std::span(blob).subspan(0, third), 0, &verf) != kNfs3Ok) fatal("unstable write failed");
    ...
    if (nfs_write(c, file, 2 * third, std::span(blob).subspan(2 * third), 2, &verf2) != kNfs3Ok)
    ...
    if (read_local(backing / "wtest/data.bin") != blob) fatal("backing file bytes differ after write");
    ...
    if (attr.size != 100 || (attr.mode & 07777) != 0600)
        fatal("setattr not applied: size=%llu mode=%o", (unsigned long long)attr.size, attr.mode);
    ...
    if (nfs_create(c, work, "data.bin", 1, 0644u, nullptr, nullptr) != 17) fatal("guarded create should EEXIST");
    Fh again;
    ...
    Fh excl;
    if (nfs_create(c, work, "excl", 2, std::nullopt, &everf, &excl) != kNfs3Ok) fatal("exclusive create failed");
    ...
    if (getattr(c, file).size != fs::file_size(backing / "wtest/hard")) fatal("hard link contents differ");
    ...
    std::printf(
        "accept_client wtest OK: create/write(3 levels)/commit/setattr/"
        "exclusive-replay/namespace ops/DRC all verified\n");
    return 0;
}

// Crash-recovery phase 1: unstable writes, record the verifier.
int cmd_crash_write(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
                    const std::string& state_file) {
    auto root = mnt(host, mount_port, export_path);
    Client c(host, nfs_port);
    Fh file;
    if (nfs_create(c, root, "crash.bin", 0, 0644u, nullptr, &file) != kNfs3Ok) fatal("create crash.bin failed");
...
int cmd_crash_recover(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path,
                      const fs::path& backing, const std::string& state_file) {
    ...
    char now_hex[17];
    for (int i = 0; i < 8; ++i) std::snprintf(now_hex + 2 * i, 3, "%02x", (unsigned)verf[i]);
    if (old_hex == now_hex) fatal("write verifier did not change across restart (%s)", now_hex);
    ...
    if (read_local(backing / "crash.bin") != blob) fatal("post-recovery on-disk content differs");
    auto remote = read_all(c, file, blob.size(), 65536);
    if (remote != blob) fatal("post-recovery READ-back differs");
    if (nfs_dirop(c, 12, root, "crash.bin") != kNfs3Ok) fatal("cleanup remove failed");
    std::printf(
        "accept_client crash-recover OK: verifier changed %s -> %s, data "
...
int cmd_connstorm(const char* host, uint16_t nfs_port, int count, int pipeline_depth) {
    ...
    // answered (backpressure delays parsing, never drops).
    {
        ...
        for (int i = 0; i < pipeline_depth; ++i) {
            ...
            v = lnfs::xdr::from_be32(v);
            std::vector<std::byte> body(v & 0x7fffffffu);
            if (!read_exact(first_alive, body.data(), body.size())) fatal("connstorm: pipeline reply body %d lost", i);
        }
    }
    for (int fd : fds) close(fd);
    std::printf(
        "accept_client connstorm OK: %d alive of %d attempted (%d shed by "
...
int cmd_fsync_eio(const char* host, uint16_t nfs_port, uint16_t mount_port, const std::string& export_path) {
    auto root = mnt(host, mount_port, export_path);
    Client c(host, nfs_port);
    Fh work;
    ...
    std::printf(
        "accept_client fsync-eio OK: injected EIO surfaced as NFS3ERR_IO, stayed "
...
constexpr uint32_t kOpClose = 4, kOpGetfh = 10, kOpLookup = 15, kOpOpen = 18, kOpPutfh = 22, kOpPutrootfh = 24,
                   kOpRead = 25, kOpReaddir = 26, kOpReadlink = 27, kOpExchangeId = 42, kOpCreateSession = 43,
                   kOpDestroySession = 44, kOpSequence = 53, kOpDestroyClientid = 57, kOpReclaimComplete = 58;
...
struct V4Client {
    ...
    Res run(const std::vector<std::byte>& body, uint32_t expect = 0, bool check = true) {
        Res out;
...
std::vector<std::byte> lookup_path(V4Client& c, const std::vector<std::string>& components) {
...
std::vector<std::byte> v4_read_all(V4Client& c, const std::vector<std::byte>& dir_fh, const std::string& name,
                                   uint64_t size) {
...
void v4_walk_dir(V4Client& c, const std::vector<std::byte>& dir_fh, const fs::path& local, size_t& dirs, size_t& files,
                 size_t& bytes) {
    ++dirs;
    std::unordered_set<std::string> local_names;
    for (const auto& e : fs::directory_iterator(local)) local_names.insert(e.path().filename().string());
    ...
    if (entries.size() != local_names.size())
        fatal("%s: v4 lists %zu entries, backing has %zu", local.c_str(), entries.size(), local_names.size());
    for (auto& ent : entries) {
        ...
        if (ent.type == 2) {
            ...
            if (!target || *target != fs::read_symlink(child).string()) fatal("%s: v4 symlink mismatch", child.c_str());
        }
    }
}
...
constexpr uint32_t kOpCommit = 5, kOpCreate = 6, kOpLock = 12, kOpLockt = 13, kOpLocku = 14, kOpLink = 11,
                   kOpOpenDowngrade = 21, kOpRemove = 28, kOpRename = 29, kOpSavefh = 32, kOpSetattr = 34,
                   kOpWrite = 38;
constexpr uint32_t kShareDenied = 10015, kOpenmode = 10038, kGrace = 10013, kStaleStateid = 10023, kNoGrace = 10033;
...
OpenOut v4_open(V4Client& c, const std::vector<std::byte>& fh, const std::string& name, uint32_t access, uint32_t deny,
                const std::string& owner, bool create, uint32_t claim = 0,
...
uint32_t v4_write(V4Client& c, const std::vector<std::byte>& fh, const Stateid4& sid, uint64_t offset,
                  std::span<const std::byte> data, uint32_t stable, std::array<std::byte, 8>* verf = nullptr) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 2);
    ops.u32(kOpPutfh);
    ops.opaque(fh);
...
std::vector<std::byte> v4_read(V4Client& c, const std::vector<std::byte>& fh, const Stateid4& sid, uint64_t size,
...
uint32_t v4_setattr_size(V4Client& c, const std::vector<std::byte>& fh, const Stateid4& sid, uint64_t size) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 2);
    ops.u32(kOpPutfh);
    ops.opaque(fh);
...
uint32_t v4_dir_op(V4Client& c, const std::vector<std::byte>& dir_fh, const std::function<void(XdrEnc&)>& body,
                   uint32_t extra = 0, const std::function<void(XdrEnc&)>& prefix = {}) {
    XdrEnc ops(c.rpc.pool);
...
std::vector<std::string> split_path(const std::string& path) {
    ...
    while (pos < path.size()) {
        ...
        if (pos < path.size())
            components.push_back(path.substr(pos, end == std::string::npos ? std::string::npos : end - pos));
...
int cmd_v4rw(const char* host, uint16_t nfs_port, const std::string& export_path, const fs::path& backing) {
    using namespace v4;
    ...
    for (size_t off = 0; off < payload.size(); off += 65536) {
        ...
        uint32_t st =
            v4_write(a, o.fh, o.stateid, off, std::span<const std::byte>(payload.data() + off, len), 0, &wverf);
    ...
    if (v4_read(a, o.fh, o.stateid, payload.size()) != payload) fatal("v4rw: read-back mismatch");

    // Truncate via SETATTR(size) with the open stateid.
    if (uint32_t st = v4_setattr_size(a, o.fh, o.stateid, 1u << 20); st != 0) fatal("v4rw: SETATTR size status %u", st);
    ...
    for (auto [fh, name] :
         {std::pair{dir_fh, "moved.bin"}, std::pair{root, "hardlink.bin"}, std::pair{root, "v4dir"}}) {
    ...
    a.destroy();
    std::printf(
        "accept_client v4rw OK: %zu bytes written/committed/read back, truncate, "
...
int cmd_v4lock(const char* host, uint16_t nfs_port, const std::string& export_path, const fs::path& backing) {
    using namespace v4;
    V4Client a(host, nfs_port);
    a.establish();
    auto root = lookup_path(a, split_path(export_path));
    ...
    auto do_lock = [&](v4::V4Client& c, const std::vector<std::byte>& fh, uint32_t type, uint64_t off, uint64_t len,
                       bool new_owner, const v4::Stateid4& sid, const std::string& owner, v4::Stateid4* out,
    ...
    // a: exclusive [0,100).
    v4::Stateid4 lsid;
    if (do_lock(a, oa.fh, 2, 0, 100, true, oa.stateid, "lo-a", &lsid, nullptr) != 0) fatal("v4lock: a WRITE_LT failed");
    if (lsid.seqid != 1 || (lsid.other[4] != std::byte{2})) fatal("v4lock: lock stateid malformed");
    ...
    if (denied.clientid != a.clientid || denied.owner != "lo-a" || denied.offset != 0 || denied.length != 100)
        fatal("v4lock: DENIED holder info wrong (client=%llx owner=%s [%llu,%llu))",
              (unsigned long long)denied.clientid, denied.owner.c_str(), (unsigned long long)denied.offset,
              (unsigned long long)denied.length);
    v4::Stateid4 lsb;
    ...
    if (v4_read(a, oa.fh, oa.stateid, payload.size()) != payload) fatal("v4lock: data changed under locking");
    ...
    c.destroy();
    std::printf(
        "accept_client v4lock OK: LOCK new/existing owner, LOCKT probe, DENIED with "
...
int cmd_v4reclaim(const char* host, uint16_t nfs_port, const std::string& export_path, const fs::path& backing,
                  const std::string& restart_cmd) {
    using namespace v4;
    std::vector<std::byte> fh;
    Stateid4 old_sid;
    std::string before = "before-restart";
    std::span<const std::byte> before_bytes(reinterpret_cast<const std::byte*>(before.data()), before.size());
    {
    ...
    if (plain.status != kGrace) fatal("v4reclaim: plain OPEN in grace: expected GRACE, got %u", plain.status);
    ...
    if (v4_write(c, re.fh, re.stateid, before.size(),
                 std::span<const std::byte>(reinterpret_cast<const std::byte*>(after.data()), after.size()), 2) != 0)
    ...
    if (late.status != kNoGrace) fatal("v4reclaim: late reclaim: expected NO_GRACE, got %u", late.status);
    ...
    c.destroy();
    std::printf(
        "accept_client v4reclaim OK: CLAIM_PREVIOUS inside grace, data intact, "
...
uint32_t v4_lock_op(V4Client& c, const std::vector<std::byte>& fh, uint32_t type, uint64_t off, uint64_t len,
                    bool reclaim, bool new_owner, const Stateid4& sid, const std::string& owner, Stateid4* out) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 2);
    ops.u32(kOpPutfh);
    ops.opaque(fh);
...
int cmd_v4failover(const char* host, uint16_t port_a, uint16_t port_b, const std::string& export_path,
                   const fs::path& backing, const std::string& takeover_cmd) {
    using namespace v4;
    ...
    std::string owner_a, scope_a;
    {
        ...
        if (v4_write(a, fh, lock_sid, 0, payload, 0, &verf_a) != 0) fatal("v4failover: WRITE(UNSTABLE) on A failed");
        std::printf(
            "v4failover: A holds open+lock+unstable write: clientid=%#llx owner=%s "
            "scope=%s\n",
    ...
    if ((b.clientid >> 32) == (clientid_a >> 32))
        fatal("v4failover: clientid epoch did not advance (%#llx → %#llx)", (unsigned long long)clientid_a,
              (unsigned long long)b.clientid);
    std::printf("v4failover: B identity kept, clientid %#llx → %#llx (epoch %llu → %llu)\n",
                (unsigned long long)clientid_a, (unsigned long long)b.clientid, (unsigned long long)(clientid_a >> 32),
                (unsigned long long)(b.clientid >> 32));
    ...
    if (plain.status != kGrace) fatal("v4failover: plain OPEN in grace: expected GRACE, got %u", plain.status);
    ...
    if (read_local(backing / "failover.bin") != payload) fatal("v4failover: backing content differs from the payload");
    ...
    b.destroy();
    std::printf(
        "accept_client v4failover OK: takeover %lld ms, BADSESSION/STALE_STATEID/GRACE "
...
int cmd_v4moved(const char* host, uint16_t port_a, uint16_t port_b, uint16_t port_c, const std::string& export_path,
                const fs::path& backing, const std::string& migrate_cmd) {
    using namespace v4;
    ...
    std::string owner_a;
    {
        ...
        a.establish();
        if ((a.eir_flags & 0x3u) != 0x3u) fatal("v4moved: A eir_flags %#x lack SUPP_MOVED_REFER|MIGR", a.eir_flags);
        owner_a = a.server_owner;
    ...
    std::array<std::byte, 8> verf_b{};
    {
        ...
        if (v4_write(b, fh, lock_sid, 0, payload, 0, &verf_b) != 0) fatal("v4moved: WRITE(UNSTABLE) on B failed");
        std::printf("v4moved: B serves the export: owner=%s scope=%s (A owner=%s)\n", owner_b.c_str(), scope_b.c_str(),
                    owner_a.c_str());
    ...
    // ---- C reclaims: CLAIM_PREVIOUS + LOCK(reclaim), verifier changed, data re-sent ----
    {
        ...
        for (;;) {
            ...
            if (plain.status != kMoved && plain.status != kDelay)
                fatal("v4moved: plain OPEN on C: expected MOVED/DELAY (not yet owner) or GRACE, got %u", plain.status);
        ...
        if (read_local(backing / "moved.bin") != payload) fatal("v4moved: backing content differs from the payload");
        if (v4_reclaim_complete(c) != 0) fatal("v4moved: RECLAIM_COMPLETE on C failed");
        plain = v4_open(c, root, "moved.bin", 3, 0, owner, false);
        if (plain.status != 0) fatal("v4moved: OPEN after RECLAIM_COMPLETE on C: status %u", plain.status);
...
int cmd_v4courtesy(const char* host, uint16_t nfs_port, const std::string& export_path, unsigned lease_seconds) {
    using namespace v4;
    ...
    if (denied.status != kShareDenied) fatal("v4courtesy: inside lease: expected SHARE_DENIED, got %u", denied.status);
    ...
    if (ok.status != 0) fatal("v4courtesy: after lease: expected OK (conflict reclaim), got %u", ok.status);
    ...
    b.destroy();
    std::printf(
        "accept_client v4courtesy OK: SHARE_DENIED inside lease, conflict reclaim "
...
int cmd_v4walk(const char* host, uint16_t nfs_port, const std::string& export_path, const fs::path& backing) {
    ...
    std::vector<std::string> components;
    {
        ...
        while (pos < rest.size()) {
            ...
            if (pos < rest.size())
                components.push_back(rest.substr(pos, end == std::string::npos ? std::string::npos : end - pos));
            if (end == std::string::npos) break;
            pos = end + 1;
        }
    }
...
constexpr uint32_t kOpAllocate = 59, kOpCopy = 60, kOpDeallocate = 62, kOpSeek = 69, kOpClone = 71;
...
SeekOut v4_seek(V4Client& c, const std::vector<std::byte>& fh, const Stateid4& sid, uint64_t offset, uint32_t what) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 2);
    ops.u32(kOpPutfh);
    ops.opaque(fh);
...
uint32_t v4_alloc(V4Client& c, const std::vector<std::byte>& fh, const Stateid4& sid, uint32_t op, uint64_t offset,
                  uint64_t length) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 2);
    ops.u32(kOpPutfh);
...
uint32_t v4_copy(V4Client& c, const std::vector<std::byte>& src, const Stateid4& ssid,
                 const std::vector<std::byte>& dst, const Stateid4& dsid, uint64_t soff, uint64_t doff, uint64_t count,
                 bool clone, uint64_t* copied = nullptr) {
    XdrEnc ops(c.rpc.pool);
    c.seq_header(ops, 4);
    ops.u32(kOpPutfh);
    ops.opaque(src);
...
// at minorversion 1.
int cmd_v42(const char* host, uint16_t nfs_port, const std::string& export_path, const fs::path& backing) {
    using namespace v4;
    ...
    // SEEK agrees with lseek(2) on the backing file.
    {
        ...
        if (hole.eof || hole.offset != (uint64_t)local_hole)
            fatal("v42: SEEK hole %llu eof=%d vs lseek %lld", (unsigned long long)hole.offset, hole.eof,
                  (long long)local_hole);
        ...
        if (data.offset != (uint64_t)local_data)
            fatal("v42: SEEK data %llu vs lseek %lld", (unsigned long long)data.offset, (long long)local_data);
        ...
        if (v4_seek(a, src.fh, src.stateid, 3 * blk, 0).status != kNxio) fatal("v42: SEEK data past EOF expected NXIO");
    }
    ...
    if (v4_alloc(a, src.fh, src.stateid, kOpAllocate, 0, 0) != kInval) fatal("v42: ALLOCATE length 0 expected INVAL");
    ...
    uint64_t copied = 0;
    if (uint32_t st = v4_copy(a, src.fh, src.stateid, dst.fh, dst.stateid, 0, 0, 0, false, &copied); st != 0)
        fatal("v42: COPY status %u", st);
    if (copied != 3 * blk) fatal("v42: COPY count %llu", (unsigned long long)copied);
    (void)v4_commit(a, dst.fh);
    if (read_local(backing / "v42dst.bin") != read_local(backing / "v42src.bin")) fatal("v42: COPY content mismatch");
    if (uint32_t st = v4_copy(a, src.fh, src.stateid, dst.fh, dst.stateid, 2 * blk, 3 * blk, 100, false, &copied);
        st != 0 || copied != 100)
        fatal("v42: ranged COPY status %u count %llu", st, (unsigned long long)copied);
    {
        auto d = read_local(backing / "v42dst.bin");
        if (d.size() != 3 * blk + 100 || !std::equal(d.begin() + 3 * blk, d.end(), block.begin()))
    ...
    if (v4_alloc(a, src.fh, ro.stateid, kOpAllocate, 0, 1) != kOpenmode) fatal("v42: ro ALLOCATE expected OPENMODE");
    ...
    std::printf(
        "accept_client v42 OK: DEALLOCATE/SEEK/ALLOCATE mirrored on backing, COPY "
...
int main(int argc, char** argv) {
    ...
    if (cmd == "walk" && (argc == 7 || argc == 8))
        return cmd_walk(host, nfs_port, mount_port, export_path, argv[6], argc == 8 && std::string(argv[7]) == "ro");
    if (cmd == "bigdir" && argc == 9)
        return cmd_bigdir(host, nfs_port, mount_port, export_path, argv[6], argv[7], strtoull(argv[8], nullptr, 10));
    if (cmd == "stress" && argc == 11)
        return cmd_stress(host, nfs_port, mount_port, export_path, argv[6], argv[7], atoi(argv[8]), atoi(argv[9]),
                          atoi(argv[10]));
    if (cmd == "wtest" && argc == 7) return cmd_wtest(host, nfs_port, mount_port, export_path, argv[6]);
    if (cmd == "crash-write" && argc == 7) return cmd_crash_write(host, nfs_port, mount_port, export_path, argv[6]);
    if (cmd == "crash-recover" && argc == 8)
        return cmd_crash_recover(host, nfs_port, mount_port, export_path, argv[6], argv[7]);
    if (cmd == "connstorm" && argc == 7) return cmd_connstorm(host, nfs_port, atoi(argv[5]), atoi(argv[6]));
    if (cmd == "v4walk" && argc == 7) return cmd_v4walk(host, nfs_port, export_path, argv[6]);
