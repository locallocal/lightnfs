...
namespace {
rt::BufferChain enc_to_chain(XdrEnc& enc) {
    return enc.take();
}

rt::BufferChain make_chain(rt::BufferPool& pool, std::span<const std::byte> bytes, size_t split) {
...
TEST(Xdr, OpaqueSpansZeroCopyAcrossSegments) {
    ...
    raw.insert(raw.end(), reinterpret_cast<const std::byte*>(payload), reinterpret_cast<const std::byte*>(payload) + 9);
