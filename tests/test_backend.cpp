...
T run_immediate(rt::Reactor& reactor, rt::Task<T> task) {
    ...
    rt::spawn([](rt::Task<T> work, std::optional<T>* out) -> rt::Task<void> { out->emplace(co_await std::move(work)); }(
...
TEST(Backend, MemoryLookupReadAndStableCookies) {
    ...
    bool eof = false;
    auto n = run_immediate(reactor, (*file)->read(backend::OpenCtx{cred}, 0, data, eof));
...
TEST(Backend, HundredThousandEntryTraversalHasNoDuplicatesOrOmissions) {
    ...
    for (uint32_t i = 0; i < kEntries; ++i) ASSERT_TRUE(memory.add_file("/big/f" + std::to_string(i), "").has_value());
    rt::testing::FakeRing ring;
    rt::Reactor reactor(ring);
    backend::Cred cred{0, 0, {}};
    auto root = run_immediate(reactor, memory.root());
...
TEST(Backend, LocalFallbackReadDirectoryAndStaleHandle) {
    ...
    backend::LocalBackend::Config cfg{
        .path = path, .fsid = 31, .fd_cache = 8, .handles = backend::LocalBackend::HandleMode::kFallback};
    ...
    bool eof = false;
    auto read = run_runtime(runtime, (*file)->read(backend::OpenCtx{cred}, 0, bytes, eof));
...
TEST(Core, LocalBackendConfigKeysReachFactory) {
    ...
    auto parsed =
        core::parse_config(config_text("handles = \"fallback\"\nfd_cache = 123\n"
...
TEST(Backend, LocalOpenStateFdSemantics) {
    ...
    backend::LocalBackend::Config cfg{
        .path = path, .fsid = 32, .fd_cache = 8, .handles = backend::LocalBackend::HandleMode::kFallback};
    auto made = backend::LocalBackend::create(cfg);
    ASSERT_TRUE(made.has_value());
    auto backend = std::move(*made);
    rt::Runtime runtime({.reactors = 1, .offload_threads = 2, .ring = "epoll"});
    ...
    auto data = std::span<const std::byte>(reinterpret_cast<const std::byte*>("ABC"), 3);
    auto wrote = run_runtime(
        runtime, (*file)->write(backend::OpenCtx{owner, opened->get()}, 0, data, backend::Stability::kUnstable));
    ...
    bool eof = false;
    auto anon = run_runtime(runtime, (*file)->read(backend::OpenCtx{stranger}, 0, buf, eof));
    EXPECT_FALSE(anon.has_value());
    EXPECT_EQ(raw(anon.error()), EACCES);
    auto via_open = run_runtime(runtime, (*file)->read(backend::OpenCtx{stranger, opened->get()}, 0, buf, eof));
    ...
    auto owner_wr = run_runtime(
        runtime, (*file)->write(backend::OpenCtx{owner, ro_open->get()}, 3, data, backend::Stability::kUnstable));
    EXPECT_TRUE(owner_wr.has_value());
    auto stranger_wr = run_runtime(
        runtime, (*file)->write(backend::OpenCtx{stranger, ro_open->get()}, 3, data, backend::Stability::kUnstable));
...
TEST(Backend, KernelHandleModeStableAcrossRestart) {
    ...
    backend::LocalBackend::Config cfg{
        .path = path, .fsid = 33, .fd_cache = 8, .handles = backend::LocalBackend::HandleMode::kKernel};
    auto made = backend::LocalBackend::create(cfg);
    if (!made.has_value()) {
        ...
        std::printf(
            "  note: kernel handles unavailable on %s (fs support or "
    ...
    if (resolved.has_value()) {
        std::array<std::byte, 16> buf{};
        bool eof = false;
        auto n = run_runtime(runtime, (*resolved)->read(backend::OpenCtx{cred}, 0, buf, eof));
        ASSERT_TRUE(n.has_value());
        EXPECT_EQ(*n, 7u);
    } else {
        EXPECT_EQ((int)resolved.error(), EPERM);
        std::printf(
            "  note: open_by_handle_at needs CAP_DAC_READ_SEARCH — resolve gave "
...
TEST(Backend, IdentityStrictDeniesForeignCred) {
    ...
    runtime.start();
    {
        ...
        auto w = run_runtime(
            runtime, created->obj->write(backend::OpenCtx{self}, 0,
                                         std::span<const std::byte>(reinterpret_cast<const std::byte*>("top"), 3),
                                         backend::Stability::kUnstable));
        ASSERT_TRUE(w.has_value());

        std::array<std::byte, 8> buf{};
        bool eof = false;
        auto own = run_runtime(runtime, created->obj->read(backend::OpenCtx{self}, 0, buf, eof));
        EXPECT_TRUE(own.has_value());
        auto denied = run_runtime(runtime, created->obj->read(backend::OpenCtx{foreign}, 0, buf, eof));
        ASSERT_TRUE(!denied.has_value());
        EXPECT_EQ((int)denied.error(), EACCES);
        auto wdenied = run_runtime(
            runtime, created->obj->write(backend::OpenCtx{foreign}, 0,
...
TEST(Backend, IdentitySetFsuidUnprivilegedIsDocumentedNoop) {
    if (geteuid() == 0) {
        std::printf(
            "  note: running as root — the unprivileged-degraded assertion does not "
    ...
    runtime.start();
    {
        ...
        auto w = run_runtime(runtime,
                             created->obj->write(backend::OpenCtx{foreign}, 0,
                                                 std::span<const std::byte>(reinterpret_cast<const std::byte*>("k"), 1),
                                                 backend::Stability::kUnstable));
...
TEST(Backend, FdCacheConcurrentStressFlushAndNoFdLeak) {
    ...
    int fds_baseline = count_open_fds();
    {
        ...
        for (int i = 0; i < kFiles; ++i) {
            auto created = run_runtime(runtime, (*root)->create(cred, "f" + std::to_string(i), {}, nullptr));
            ASSERT_TRUE(created.has_value());
        ...
        for (int t = 0; t < kTasks; ++t) {
            rt::spawn(
                [](int task, std::vector<backend::ObjPtr>* files, backend::Cred cred, std::atomic<int>* failures,
                   std::mutex* mu, std::condition_variable* cv, int* done) -> rt::Task<void> {
                    std::byte buf[16];
                    for (int i = 0; i < kIters; ++i) {
                        ...
                        auto w =
                            co_await obj.write(open, static_cast<uint64_t>(task) * 64,
