...
TEST(TokenBucket, ShapesToConfiguredRate) {
    ...
    auto waited = std::chrono::duration_cast<std::chrono::milliseconds>(after_shaped - start).count();
...
TEST(TokenBucket, DebtModePassesOverBurstRequests) {
    ...
    auto repaid = std::chrono::duration_cast<std::chrono::milliseconds>(after_small - start).count();
