...
struct WriteFixture {
    ...
    std::vector<std::byte> request(nfsv3::Proc proc, rt::BufferChain args, uint32_t xid = 0x1234) {
...
TEST(WritePath, DrcReplaysIdenticalReply) {
    ...
    size_t dropped = 0;
    rt::spawn([](rpc::Drc* d, size_t* out) -> rt::Task<void> { *out = co_await d->flush(); }(&drc, &dropped),
              f.reactor);
...
TEST(WritePath, ErrmapWhitelistForWriteProcs) {
    ...
    EXPECT_EQ((uint32_t)core::to_v3(errno_from(ENOTEMPTY), P::kRmdir), (uint32_t)S::kNotempty);
...
TEST(WritePath, WritePayloadSpanningSegments) {
    ...
    wenc.u32(nfsv3::kFileSync);
    wenc.opaque(std::span<const std::byte>(reinterpret_cast<const std::byte*>(payload.data()), payload.size()));
