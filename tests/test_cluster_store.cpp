...
TEST(ClusterStore, Sha256Vectors) {
    ...
    EXPECT_STREQ(util::sha256_hex(std::string_view("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq")),
...
TEST(ClusterStore, CatalogAppliedPerNode) {
    ...
    ASSERT_TRUE(a->put_catalog_applied(
                     {"gw1", 2, "sha256:aaaa", t0 + 5, "error:merge failed: fsid 3 has no [backend_defaults.cephfs]"})
                    .has_value());
...
TEST(ClusterStore, MemCatalogMirrorsPosix) {
    ...
    EXPECT_TRUE(mem.write_catalog(server::kCatalogHistoryKeep + 5, catalog_text(99)).error() == errno_from(EIO));
