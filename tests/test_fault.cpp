...
TEST(Fault, ShortWriteContinuationBothOverloads) {
    ...
    runtime.start();
    {
        ...
        auto w = run_runtime(runtime, obj.write(open, 0, bytes("0123456789"), backend::Stability::kUnstable));
        ASSERT_TRUE(w.has_value());
        EXPECT_EQ(*w, 10u);
        ...
        auto wv = run_runtime(runtime, obj.write(open, 0, std::span<const iovec>(iov), backend::Stability::kUnstable));
...
TEST(Fault, SlowIoDelaysWithoutFailing) {
    ...
    runtime.start();
    {
        ...
        auto w = run_runtime(runtime, created->obj->write(open, 0, bytes("z"), backend::Stability::kUnstable));
...
TEST(Fault, InjectedFsyncEioPoisonsFile) {
    ...
    runtime.start();
    {
        ...
        auto w = run_runtime(runtime, created->obj->write(open, 0, bytes("data"), backend::Stability::kFileSync));
        ASSERT_TRUE(!w.has_value());
