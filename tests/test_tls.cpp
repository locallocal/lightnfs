...
struct TempCert {
    ...
    TempCert() {
        ...
        X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, reinterpret_cast<const unsigned char*>("lightnfs-test"),
                                   -1, -1, 0);
...
std::vector<std::byte> build_call(uint32_t xid, uint32_t proc, uint32_t cred_flavor, std::string_view payload) {
    BufferPool pool;
    xdr::XdrEnc enc(pool);
    enc.u32(xid);
    // CALL
...
bool do_starttls_probe(int fd) {
    ...
    if (!read_record([&](std::byte* p, size_t n) { return raw_read(fd, p, n); }, rep)) return false;
...
TEST(Tls, RequiredRejectsCleartextOps) {
    ...
    ASSERT_TRUE(read_record([&](std::byte* p, size_t n) { return ssl_read_all(ssl, p, n); }, rep2));
