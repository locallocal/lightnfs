...
TEST(Offload, RoundTripAffinity) {
    ...
    spawn(
        [](std::thread::id* rt_id, std::thread::id* off_id, std::thread::id* res_id, Runtime* r) -> Task<void> {
...
TEST(Offload, HeavyClassDoesNotBlockLight) {
    ...
    pool.submit(
        ...
        OffloadClass::kHeavy);
    for (int i = 0; i < 8; ++i) pool.submit([&] { light_done.fetch_add(1); }, OffloadClass::kLight);
