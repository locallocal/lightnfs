...
struct Mount {
    ...
    explicit Mount(bool jukebox = true, bool locks = true) : Mount(MountOpts{.jukebox = jukebox, .locks = locks}) {}
...
TEST(Cephfs, NamespaceOpsAndReaddirCookies) {
    ...
    auto fifo = run(m.runtime, dir->obj->mknod(m.root_cred, "ff", backend::FType::kFifo, {}, backend::SetAttr{}));
...
TEST(Cephfs, AnonymousAndOpenStateIo) {
    ...
    auto w2 = run(m.runtime, obj.write(anon, 6, std::span<const iovec>(iov, 2), backend::Stability::kFileSync));
    ...
    auto w_short = run(m.runtime, obj.write(anon, 12, std::span<const iovec>(iov, 2), backend::Stability::kUnstable));
...
TEST(Cephfs, ReclaimLockDelayUntilTakeover) {
    ...
    test::ReclaimProbe probe(
        m.runtime, state_dir.path, /*fsid=*/9, oid, [mgr](uint32_t) -> backend::LockMgr* { return mgr; },
        [be = m.be.get()](uint32_t, const backend::ObjId& o) -> rt::Task<Result<backend::ObjPtr>> {
...
TEST(Cephfs, TakeoverReclaimsStaleLocks) {
    ...
    {
        auto root = m.root();
        ASSERT_TRUE(run(m.runtime, root->create(m.root_cred, "lk", backend::SetAttr{}, nullptr)).has_value());
        ASSERT_TRUE(run(m.runtime, root->create(m.root_cred, "other", backend::SetAttr{}, nullptr)).has_value());
    }
...
TEST(Cephfs, TakeoverIsScopedToOneExport) {
    ...
    {
        auto root = m.root();
        ASSERT_TRUE(run(m.runtime, root->create(m.root_cred, "lk9", backend::SetAttr{}, nullptr)).has_value());
        ASSERT_TRUE(run(m.runtime, root->create(m.root_cred, "lk10", backend::SetAttr{}, nullptr)).has_value());
    }
...
TEST(Cephfs, PerFsidTakeoverReclaimIsolation) {
    ...
    {
        auto root9 = m.root();
        ASSERT_TRUE(run(m.runtime, root9->create(m.root_cred, "lk9", backend::SetAttr{}, nullptr)).has_value());
        auto root10 = run(m.runtime, be10->root());
        ASSERT_TRUE(root10.has_value());
...

    {
        TmpDir state_dir;
        test::ReclaimProbe probe(
            m.runtime, state_dir.path, /*fsid=*/10, oid10, [mgr10](uint32_t) -> backend::LockMgr* { return mgr10; },
...
TEST(Cephfs, TakeoverWithoutReclaimSymbolsOrMount) {
    ...
    EXPECT_TRUE(run(m.runtime, root->create(m.root_cred, "f", backend::SetAttr{}, nullptr)).has_value());
...
TEST(Cephfs, StopRestartAndLeakFree) {
    ...
    int perms_before = testing::FakeCephApi::live_perms();
    {
        ...
        ASSERT_TRUE(f.has_value());
        ASSERT_TRUE(
            run(m.runtime, f->obj->write({m.root_cred}, 0, m.bytes("abc"), backend::Stability::kUnstable)).has_value());
        auto oid = f->obj->id();
        f->obj.reset();
        root.reset();
        ASSERT_TRUE(run(m.runtime, m.be->stop()).has_value());
