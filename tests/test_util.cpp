...
TEST(Metrics, V4OpSeriesInPrometheusText) {
    ...
    EXPECT_TRUE(text.find("lightnfs_v4_op_calls_total{op=\"GETATTR\"} 3") != std::string::npos);
    EXPECT_TRUE(text.find("lightnfs_v4_op_errors_total{op=\"GETATTR\"} 1") != std::string::npos);
    EXPECT_TRUE(text.find("lightnfs_v4_op_duration_seconds_bucket{op=\"GETATTR\"") != std::string::npos);
    EXPECT_TRUE(text.find("lightnfs_v4_compound_duration_seconds_count") != std::string::npos);
