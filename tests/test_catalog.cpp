...
TEST(Catalog, ParseAndRoundTrip) {
  ...
  e.cfg.backend_config.values = {{"volume", "007"},
                                 {"servers", "a,b"},
                                 {"port", "24007"},
                                 {"acl", "true"},
                                 {"path with space", "v"}};
...
TEST(Catalog, ValidateRules) {
  ...
  EXPECT_TRUE(
      contains(reason(with(make_export(3, "/a")), true), "fsid=1 and fsid=3 share path /a"));
  EXPECT_TRUE(contains(reason(with(make_export(3, "/a/sub")), true),
                       "fsid=3 (/a/sub) is nested in fsid=1 (/a)"));
  EXPECT_TRUE(
      contains(reason(with(make_export(3, "/")), true), "fsid=1 (/a) is nested in fsid=3 (/)"));
  EXPECT_STREQ(reason(with(make_export(3, "/ab")), true), "(accepted)");  // not a prefix
  EXPECT_TRUE(contains(reason(with(make_export(3, "/z", "bogus")), true, ENODEV),
                       "no such backend \"bogus\""));
  EXPECT_TRUE(contains(reason(with(make_export(3, "/z", "local", {})), true),
                       "nodes is required under active-active"));
...
TEST(Catalog, Diff) {
  ...
  for (auto mutate : {+[](core::ExportConfig& c) { c.readonly = !c.readonly; },
                      ...
                      +[](core::ExportConfig& c) { c.anon_uid = 1; },
                      +[](core::ExportConfig& c) { c.anon_gid = 1; },
                      +[](core::ExportConfig& c) { c.read_bps = 1; },
                      +[](core::ExportConfig& c) { c.write_bps = 1; },
