...
struct Options {
    ...
    Options& at(std::string m) {
        mount = std::move(m);
        return *this;
    }
    Options& reusing(std::string dir) {
        reuse = std::move(dir);
        return *this;
    }
    Options& striped(uint32_t bytes) {
        stripe = bytes;
        return *this;
    }
};
...
struct Mount {
    ...
    backend::ObjPtr create_file(std::string_view name, std::string_view content) {
        ...
        if (!f) return nullptr;
        if (!content.empty()) {
            auto n = run(runtime, f->obj->write({root_cred}, 0, bytes(content), backend::Stability::kFileSync));
...
TEST(Lustre, AnonymousAndOpenStateIo) {
    ...
    auto n = run(m.runtime, f->write(octx, 0, m.bytes("hello lustre"), backend::Stability::kUnstable));
...
TEST(Lustre, ReclaimLockDelayUntilClientEviction) {
    ...
    test::ReclaimProbe probe(
        m.runtime, state_dir.path, /*fsid=*/11, oid, [mgr](uint32_t) -> backend::LockMgr* { return mgr; },
