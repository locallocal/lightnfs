...
struct Fixture {
    ...
    void dispatch(BufferChain rec) {
        spawn([](Fixture* f, BufferChain rr) -> Task<void> { co_await f->disp.handle_request(f->ctx, std::move(rr)); }(
                                                 this, std::move(rec)),
              r);
        pump();
    }
...
BufferChain make_call(BufferPool& pool, uint32_t xid, uint32_t prog, uint32_t vers, uint32_t proc, uint32_t rpcvers = 2,
                      uint32_t cred_flavor = 0, std::span<const std::byte> cred_body = {},
