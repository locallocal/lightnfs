...
TEST(Gluster, NamespaceOpsAndReaddirCookies) {
    ...
    auto fifo = run(v.runtime, dir->obj->mknod(v.root_cred, "ff", backend::FType::kFifo, {}, backend::SetAttr{}));
...
TEST(Gluster, AnonymousAndOpenStateIo) {
    ...
    auto w2 = run(v.runtime, obj.write(anon, 6, std::span<const iovec>(iov, 2), backend::Stability::kFileSync));
...
TEST(Gluster, StopRestartAndLeakFree) {
    ...
    int fds_before = testing::FakeGfapi::live_fds();
    {
        ...
        ASSERT_TRUE(f.has_value());
        ASSERT_TRUE(
            run(v.runtime, f->obj->write({v.root_cred}, 0, v.bytes("abc"), backend::Stability::kUnstable)).has_value());
        auto oid = f->obj->id();
        f->obj.reset();
        root.reset();
...
TEST(Gluster, ReclaimLockDelayUntilBrickTimeout) {
    ...
    test::ReclaimProbe probe(
        v.runtime, state_dir.path, /*fsid=*/9, oid, [mgr](uint32_t) -> backend::LockMgr* { return mgr; },
        [be = v.be.get()](uint32_t, const backend::ObjId& o) -> rt::Task<Result<backend::ObjPtr>> {
