...
TEST(BackendWrite, CreateWriteCommitAndSetattr) {
    ...
    runtime.start();
    {
        ...
        auto w1 = run_runtime(runtime, created->obj->write(open, 0, data, backend::Stability::kUnstable));
        ASSERT_TRUE(w1.has_value());
        EXPECT_EQ(*w1, 9u);
        auto w2 = run_runtime(runtime, created->obj->write(open, 9, data, backend::Stability::kDataSync));
        ASSERT_TRUE(w2.has_value());
        auto w3 = run_runtime(runtime, created->obj->write(open, 18, data, backend::Stability::kFileSync));
...
TEST(BackendWrite, NamespaceOpsAndFdCacheUpgrade) {
    ...
    runtime.start();
    {
        ...
        auto renamed = run_runtime(runtime, (*root)->rename(cred, "a", *dir->obj, "b"));
...
TEST(BackendWrite, FdCacheHardCapAndOverflow) {
    ...
    runtime.start();
    {
        ...
        auto made = backend::LocalBackend::create({.path = tree.path, .fsid = 5, .fd_cache = 16});
        ASSERT_TRUE(made.has_value());
        auto& be = **made;
        auto root = run_runtime(runtime, be.root());
        ASSERT_TRUE(root.has_value());
        ...
        for (int i = 0; i < 64; ++i) {
            auto f = run_runtime(runtime, (*root)->create(cred, "f" + std::to_string(i), {}, nullptr));
            ASSERT_TRUE(f.has_value());
            auto w = run_runtime(runtime, f->obj->write(open, 0, one, backend::Stability::kUnstable));
            ASSERT_TRUE(w.has_value());
        ...
        auto again = run_runtime(runtime, files.back()->write(open, 0, one, backend::Stability::kUnstable));
        ...
        for (size_t i = 0; i < files.size() && !dst; ++i)
            for (size_t j = i + 1; j < files.size() && !dst; ++j)
                if (backend::ObjIdHash{}(files[i]->id()) % 16 == backend::ObjIdHash{}(files[j]->id()) % 16) {
                    src = files[i];
                    dst = files[j];
                }
        ASSERT_TRUE(src && dst);
        auto copied = run_runtime(runtime, src->copy_range(open, *dst, open, 0, 0, 1));
...
TEST(BackendWrite, V42SparseCopyClone) {
    ...
    runtime.start();
    {
        ...
        ASSERT_TRUE(
            run_runtime(runtime, src->obj->write(open, 2 << 16, block, backend::Stability::kFileSync)).has_value());
...
TEST(BackendWrite, ScatterWriteVectored) {
    ...
    runtime.start();
    {
        ...
        ASSERT_TRUE(root.has_value());
        auto cred = self_cred();
        auto created = run_runtime(runtime, (*root)->create(cred, "vfile", backend::SetAttr{}, nullptr));
        ASSERT_TRUE(created.has_value());
        backend::OpenCtx open{cred, nullptr};
        ...
        const char c[] = "world";
        iovec iov[3] = {{const_cast<char*>(a), 6}, {const_cast<char*>(b), 8}, {const_cast<char*>(c), 5}};
        auto w = run_runtime(
            runtime, created->obj->write(open, 0, std::span<const iovec>(iov, 3), backend::Stability::kFileSync));
        ASSERT_TRUE(w.has_value());
        EXPECT_EQ(*w, 19u);
        ...
        iovec iov2[3] = {{const_cast<char*>(a), 0}, {const_cast<char*>(c), 5}, {const_cast<char*>(a), 0}};
        auto w2 = run_runtime(
            runtime, created->obj->write(open, 19, std::span<const iovec>(iov2, 3), backend::Stability::kUnstable));
        ASSERT_TRUE(w2.has_value());
        EXPECT_EQ(*w2, 5u);
    }
    runtime.stop_and_join();
...
TEST(BackendWrite, CloneMisalignedEinvalPassthrough) {
    ...
    runtime.start();
    {
        ...
        if (!be.caps().has(backend::Cap::kCloneRange)) {
            std::printf(
                "  note: no reflink support on %s — CLONE EINVAL passthrough not "
                "reachable here\n",
                path.c_str());
        } else {
            auto cred = self_cred();
            ...
            auto w = run_runtime(
                runtime, src->obj->write(open, 0, std::span<const std::byte>(block), backend::Stability::kFileSync));
            ASSERT_TRUE(w.has_value());
