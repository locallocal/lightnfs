...
bool permits(const struct stat& sb, int mode, const UserPerm* p) {
    ...
    if (p->uid == sb.st_uid)
        shift = 6;
    else if (p->gid == sb.st_gid || std::find(p->groups.begin(), p->groups.end(), sb.st_gid) != p->groups.end())
        shift = 3;
...
int f_conf_get(ceph_mount_info* m, const char* key, char* buf, size_t len) {
    std::string v;
    if (std::string(key) == "fsid")
        v = "0f7a3b7e-fake-4c0d-9d1c-0123456789ab";
    else {
...
int64_t f_get_fs_cid(ceph_mount_info*) {
    return 7;
}
uint64_t f_get_instance_id(ceph_mount_info*) {
    return 4242;
}
...
int f_ll_lookup(ceph_mount_info*, Inode* parent, const char* name, Inode** out, struct ceph_statx* stx, unsigned,
                unsigned, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
...
int f_ll_getattr(ceph_mount_info*, Inode* in, struct ceph_statx* stx, unsigned, unsigned, const UserPerm* perms) {
    note_identity(perms);
...
int f_ll_setattr(ceph_mount_info*, Inode* in, struct ceph_statx* stx, int mask, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
    struct stat sb{};
    if (stat_inode(in, &sb) < 0) return -errno;
    ...
    if (mask & (capi::kSetMode | capi::kSetUid | capi::kSetGid | capi::kSetAtime | capi::kSetMtime |
                capi::kSetAtimeNow | capi::kSetMtimeNow) &&
        !owner)
        return -EPERM;
    ...
    if (mask & (capi::kSetAtime | capi::kSetMtime | capi::kSetAtimeNow | capi::kSetMtimeNow)) {
        ...
        if (mask & capi::kSetAtimeNow)
            times[0] = {0, UTIME_NOW};
        else if (mask & capi::kSetAtime)
            times[0] = stx->stx_atime;
        if (mask & capi::kSetMtimeNow)
            times[1] = {0, UTIME_NOW};
        else if (mask & capi::kSetMtime)
            times[1] = stx->stx_mtime;
...
int f_ll_create(ceph_mount_info* m, Inode* parent, const char* name, mode_t mode, int oflags, Inode** outp, Fh** fhp,
    ...
    *outp = in;
    if (fhp)
        *fhp = fh;
        ...
        f_ll_put(m, in), delete fh;
    return 0;
}

int f_ll_mknod(ceph_mount_info*, Inode* parent, const char* name, mode_t mode, dev_t rdev, Inode** out,
               struct ceph_statx* stx, unsigned, unsigned, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
    int rc = check_dir(parent, W_OK | X_OK, perms);
...
int f_ll_mkdir(ceph_mount_info*, Inode* parent, const char* name, mode_t mode, Inode** out, struct ceph_statx* stx,
               unsigned, unsigned, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
    int rc = check_dir(parent, W_OK | X_OK, perms);
    if (rc < 0) return rc;
...
int f_ll_symlink(ceph_mount_info*, Inode* parent, const char* name, const char* value, Inode** out,
                 struct ceph_statx* stx, unsigned, unsigned, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
    int rc = check_dir(parent, W_OK | X_OK, perms);
...
int f_ll_link(ceph_mount_info*, Inode* in, Inode* newparent, const char* name, const UserPerm* perms) {
    note_identity(perms);
    FAKE_FAIL();
    int rc = check_dir(newparent, W_OK | X_OK, perms);
    if (rc < 0) return rc;
    if (linkat(AT_FDCWD, proc_path(in->fd).c_str(), newparent->fd, name, AT_SYMLINK_FOLLOW) < 0) return -errno;
    bump_version(in->ino);
...
int f_ll_rename(ceph_mount_info*, Inode* parent, const char* name, Inode* newparent, const char* newname,
...
uint32_t f_ll_stripe_unit(ceph_mount_info*, Inode*) {
    return 4u << 20;
}
...
int f_readdirplus_r(ceph_mount_info* m, ceph_dir_result* dr, struct dirent* de, struct ceph_statx* stx, unsigned,
                    unsigned, Inode** out) {
    FAKE_FAIL();
    errno = 0;
...
void f_seekdir(ceph_mount_info*, ceph_dir_result* dr, int64_t off) {
    seekdir(dr->dir, off);
}
...
bool FakeCephApi::plant_stale_lock(const std::string& rel_path, const std::string& uuid, uint64_t start, uint64_t len) {
    auto& s = st();
    std::string full;
    {
        std::lock_guard lock(s.mu);
...
uint64_t FakeCephApi::reclaim_calls() {
    return st().reclaim_calls.load();
}
...
uint32_t FakeCephApi::last_uid() {
    return g_last_uid.load();
}
uint32_t FakeCephApi::last_gid() {
    return g_last_gid.load();
}
int FakeCephApi::live_inodes() {
    return st().live_inodes.load();
}
int FakeCephApi::live_fhs() {
    return st().live_fhs.load();
}
int FakeCephApi::live_dirs() {
    return st().live_dirs.load();
}
int FakeCephApi::live_perms() {
    return st().live_perms.load();
}
uint64_t FakeCephApi::getattr_calls() {
    return st().getattr_calls.load();
}
