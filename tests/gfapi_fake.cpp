...
int f_set_logging(glfs*, const char*, int) {
    return 0;
}
int f_init(glfs* fs) {
    auto& s = st();
...
glfs_object* f_h_lookupat(glfs* fs, glfs_object* parent, const char* path, struct stat* out, int follow) {
    note_identity();
...
glfs_object* f_h_creat(glfs* fs, glfs_object* parent, const char* name, int flags, mode_t mode, struct stat* out) {
    note_identity();
    if (take_fail()) return nullptr;
...
glfs_object* f_h_mkdir(glfs* fs, glfs_object* parent, const char* name, mode_t mode, struct stat* out) {
    note_identity();
    if (take_fail()) return nullptr;
...
glfs_object* f_h_mknod(glfs* fs, glfs_object* parent, const char* name, mode_t mode, dev_t dev, struct stat* out) {
    note_identity();
    if (take_fail()) return nullptr;
...
glfs_object* f_h_symlink(glfs* fs, glfs_object* parent, const char* name, const char* data, struct stat* out) {
    note_identity();
    if (take_fail()) return nullptr;
...
int f_h_link(glfs*, glfs_object* target, glfs_object* parent, const char* name) {
    note_identity();
    if (take_fail()) return -1;
    if (linkat(AT_FDCWD, proc_path(target->fd).c_str(), parent->fd, name, AT_SYMLINK_FOLLOW) < 0) return -1;
    return 0;
}

int f_h_rename(glfs*, glfs_object* olddir, const char* oldname, glfs_object* newdir, const char* newname) {
    note_identity();
    if (take_fail()) return -1;
...
bool permits(const struct stat& sb, int mode) {
    ...
    int shift = 0;
    if (t_fsuid == sb.st_uid)
        shift = 6;
    else if (t_fsgid == sb.st_gid || std::find(t_groups.begin(), t_groups.end(), sb.st_gid) != t_groups.end())
        shift = 3;
...
void release_locks_of(glfs_fd* g) {
    ...
    auto& s = st();
    std::lock_guard lock(s.mu);
    std::erase_if(s.locks, [&](const State::Seg& seg) { return seg.ino == g->ino && seg.owner == g->lkowner; });
}
...
int f_discard(glfs_fd* g, off_t off, size_t len) {
    if (take_fail()) return -1;
    return fallocate(g->fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, off, static_cast<off_t>(len));
}
ssize_t f_copy_file_range(glfs_fd* in, off64_t* off_in, glfs_fd* out, off64_t* off_out, size_t len, unsigned int flags,
                          glfs_stat*, glfs_stat*, glfs_stat*) {
    if (take_fail()) return -1;
...
int f_xreaddirplus_r(glfs_fd* g, uint32_t flags, glfs_xreaddirp_stat** xstat_p, struct dirent* ext,
                     struct dirent** res) {
    using namespace lnfs::backend::gfapi;
    if (take_fail()) return -1;
    *xstat_p = nullptr;
    ...
    *res = ext;
    ...
    if (flags) {
        ...
        int dfd = dirfd(g->dir);
        if ((flags & kXreaddirpStat) && fstatat(dfd, d->d_name, &x->st, AT_SYMLINK_NOFOLLOW) == 0) {
            x->has_stat = true;
            done |= kXreaddirpStat;
        }
...
glfs_object* f_xreaddirplus_get_object(glfs_xreaddirp_stat* x) {
    return x ? x->obj : nullptr;
}
void f_seekdir(glfs_fd* g, long off) {
    seekdir(g->dir, off);
}
void f_free(void* p) {
...
int f_posix_lock(glfs_fd* g, int cmd, struct flock* fl) {
    ...
    auto overlaps = [&](const State::Seg& seg) { return seg.ino == g->ino && seg.start < end && start < seg.end; };
...
uint32_t FakeGfapi::last_fsuid() {
    return g_last_uid.load();
}
uint32_t FakeGfapi::last_fsgid() {
    return g_last_gid.load();
}
int FakeGfapi::live_objects() {
    return st().live_objects.load();
}
int FakeGfapi::live_fds() {
    return st().live_fds.load();
}
uint64_t FakeGfapi::access_calls() {
    return st().access_calls.load();
}
