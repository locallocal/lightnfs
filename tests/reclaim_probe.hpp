...
inline void run_blocking(rt::Runtime& runtime, rt::Task<void> task) {
    ...
    bool done = false;
    rt::spawn(
        [](rt::Task<void> work, std::mutex* mu, std::condition_variable* cv, bool* done) -> rt::Task<void> {
...
class ReclaimProbe {
 public:
    ReclaimProbe(rt::Runtime& runtime, const std::string& state_dir, uint32_t fsid, backend::ObjId oid,
        ...
        {
            state::StateMgr a(cfg(1, state_dir, {}, {}));
            run_blocking(runtime_, list_client(a));
        }
 ...
 private:
    ...
    state::StateMgr::Config cfg(
        uint64_t epoch, const std::string& dir, std::function<backend::LockMgr*(uint32_t)> manager,
        std::function<rt::Task<Result<backend::ObjPtr>>(uint32_t, const backend::ObjId&)> resolve) {
