...
TEST(StateMgr, ConcurrentLifecycleMatrixIsDeadlockFree) {
    ...
    for (int c = 0; c < kClients; ++c) {
        rt::spawn(
            [](state::StateMgr* mgr, int idx, std::atomic<int>* done, std::atomic<int>* errors, std::mutex* mu,
               std::condition_variable* cv) -> rt::Task<void> {
                nfsv4::Verifier verf{};
                verf[0] = static_cast<std::byte>(idx);
                auto ex = co_await mgr->exchange_id("client-" + std::to_string(idx), verf, "sys/test/0", false);
                if (ex.status != 0) errors->fetch_add(1);
                auto cs = co_await mgr->create_session(ex.clientid, ex.sequenceid, "sys/test/0", {}, {}, 1);
                ...
                for (uint32_t seq = 1; seq <= kSeqPerSlot; ++seq) {
                    for (uint32_t slot = 0; slot < 2; ++slot) {
                        auto begin = co_await mgr->sequence_begin(cs.sessionid, slot, seq, 3, seq % 2 == 0, 1);
                        ...
                        co_await mgr->sequence_complete(cs.sessionid, slot, seq, seq % 2 == 0, {std::byte{7}});
                    }
                }
                // Retransmit of the last cached sequence must replay.
                auto replay = co_await mgr->sequence_begin(cs.sessionid, 0, kSeqPerSlot, 3, false, 1);
...
TEST(StateMgr, GraceListPersistsAndEarlyExit) {
    ...
    uint64_t clientid = 0;
    {
        ...
        bool ok = false;
        rt::spawn(
            [](state::StateMgr* mgr, uint64_t* out, std::mutex* mu, std::condition_variable* cv,
    ...
    bool ok = false;
    rt::spawn(
        [](state::StateMgr* mgr, std::mutex* mu, std::condition_variable* cv, bool* ok) -> rt::Task<void> {
            auto ex = co_await mgr->exchange_id("grace-client", {}, "sys/t/0", false);
            auto cs = co_await mgr->create_session(ex.clientid, ex.sequenceid, "sys/t/0", {}, {}, 1);
            (void)cs;
...
state::StateMgr::OpenArgs open_args(uint64_t clientid, uint8_t file, const char* owner, uint32_t access,
                                    uint32_t deny) {
...
TEST(StateMgr, ShareReservationMergeDowngradeClose) {
    ...
    run_on(runtime, [&]() -> rt::Task<void> {
        ...
        // A: READ, deny WRITE.
        auto o1 = co_await mgr.open(open_args(a.clientid, 1, "oa", state::kShareRead, state::kShareWrite), nullptr);
        ...
        EXPECT_EQ(co_await mgr.open_downgrade(o4.stateid, a.clientid, state::kShareRead, 0, &after_dg), kOk);
        EXPECT_EQ(after_dg.seqid, 3u);
        EXPECT_EQ(co_await mgr.open_downgrade(after_dg, a.clientid, state::kShareBoth, 0, nullptr),
                  st4(nfsv4::Status::kInval));
        ...
        nfsv4::Stateid out;
        EXPECT_EQ(co_await mgr.close_state(o4.stateid, a.clientid, &out), st4(nfsv4::Status::kOldStateid));
        EXPECT_EQ(co_await mgr.close_state(after_dg, a.clientid, &out), kOk);
        EXPECT_EQ(out.seqid, 4u);
        EXPECT_EQ(co_await mgr.close_state(after_dg, a.clientid, &out), st4(nfsv4::Status::kBadStateid));
...
TEST(StateMgr, CourtesyConflictAndTimeoutReclaim) {
    ...
    state::StateMgr mgr({.boot_epoch = 5, .state_dir = dir.path, .lease_seconds = 1, .courtesy_multiplier = 1});
    run_on(runtime, [&]() -> rt::Task<void> {
        auto a = co_await connect(mgr, "client-a", 1);
        auto b = co_await connect(mgr, "client-b", 2);
        auto oa = co_await mgr.open(open_args(a.clientid, 1, "oa", state::kShareRead, state::kShareWrite), nullptr);
...
TEST(StateMgr, LockStateidIoRacesParentClose) {
    ...
    std::thread watchdog([&] {
        for (int i = 0; i < 600 && !test_done.load(); ++i) std::this_thread::sleep_for(std::chrono::milliseconds(100));
        if (!test_done.load()) {
    ...
    run_on(runtime, [&]() -> rt::Task<void> {
        ...
        for (int round = 0; round < 100; ++round) {
            ...
            for (int spin = 0; spin < 200; ++spin) {
                auto io = co_await mgr.check_io(l.stateid, a.clientid, 1, oid_of(1), state::kShareWrite);
                if (io.status != kOk) break;
            }
            while (!closed.load(std::memory_order_acquire)) co_await rt::sleep_for(std::chrono::milliseconds(1));
        }
...
TEST(StateMgr, SingleShardConfigLifecycle) {
    ...
    std::atomic<int> errors{0};
    rt::spawn(
        [](state::StateMgr* mgr, std::atomic<bool>* done, std::atomic<int>* errors) -> rt::Task<void> {
            nfsv4::Verifier verf{};
            ...
            auto cs = co_await mgr->create_session(ex.clientid, ex.sequenceid, "sys/test/0", attrs, attrs, 1);
            ...
            if ((co_await mgr->destroy_clientid(ex.clientid)) != static_cast<uint32_t>(nfsv4::Status::kStaleClientid))
                ++*errors;
            done->store(true);
        }(&mgr, &done, &errors),
        runtime.reactor(0));
...
TEST(StateMgr, GraceDecoupledFromLeaseAndOperatorEnd) {
    ...
    state::StateMgr mgr({.boot_epoch = 2, .state_dir = dir.path, .lease_seconds = 90, .grace_seconds = 5});
    mgr.load_grace_list();
...
struct FakeNativeLocks final : backend::LockMgr {
    ...
    rt::Task<Result<void>> unlock(backend::Object&, const backend::LockOwnerId&, backend::LockRange r) override {
        calls.push_back(std::format("unlock {} {}", r.offset, r.length));
        co_return Result<void>{};
    }
    rt::Task<Result<std::optional<backend::LockConflict>>> test(backend::Object&, backend::LockRange, bool) override {
...
TEST(StateMgr, ReclaimLockPushDelayInGrace) {
    ...
    auto make_cfg = [&](uint64_t epoch) {
        ...
        cfg.native_locks.manager = [&](uint32_t fsid) -> backend::LockMgr* { return fsid == 1 ? &native : nullptr; };
        cfg.native_locks.resolve = [&](uint32_t, const backend::ObjId&) -> rt::Task<Result<backend::ObjPtr>> {
            co_return co_await memory.root();
        };
        return cfg;
    };
...
TEST(StateMgr, NativeLockPushRollbackAndRelease) {
    ...
    FakeNativeLocks native;
    backend::MemoryBackend memory(1);
    state::StateMgr::Config cfg{.boot_epoch = 5, .state_dir = dir.path, .lease_seconds = 1, .courtesy_multiplier = 1};
    cfg.native_locks.manager = [&](uint32_t fsid) -> backend::LockMgr* { return fsid == 1 ? &native : nullptr; };
    cfg.native_locks.resolve = [&](uint32_t, const backend::ObjId&) -> rt::Task<Result<backend::ObjPtr>> {
        co_return co_await memory.root();
    };
...
state::StateMgr::OpenArgs open_in(uint32_t fsid, uint64_t clientid, uint8_t file, const char* owner) {
...
TEST(StateMgr, PerFsidGraceIndependent) {
    ...
    run_on(runtime, [&]() -> rt::Task<void> {
        ...
        nfsv4::Stateid anon{};
        EXPECT_EQ((co_await second.check_io(anon, n.clientid, 2, oid_of(2), state::kShareWrite)).status, kOk);
        // Export 1 is: plain OPEN and anonymous WRITE wait, reads pass.
        EXPECT_EQ((co_await second.open(open_in(1, n.clientid, 1, "on"), nullptr)).status, st4(nfsv4::Status::kGrace));
        EXPECT_EQ((co_await second.check_io(anon, n.clientid, 1, oid_of(1), state::kShareWrite)).status,
                  st4(nfsv4::Status::kGrace));
        EXPECT_EQ((co_await second.check_io(anon, n.clientid, 1, oid_of(1), state::kShareRead)).status, kOk);
...
TEST(StateMgr, StableStoreHooksPerFsid) {
    ...
    runtime.start();
    FsMemStore store;
    state::StateMgr mgr({.boot_epoch = 1, .state_dir = dir.path, .stable = store.hooks(), .per_fsid_reclaim = true});
