...
struct MemClusterStore final : server::ClusterStore {
    ...
    Result<server::FenceRecord> acquire_fence(std::string_view node, uint64_t e, std::chrono::milliseconds ttl,
                                              bool force) override {
        log.push_back(std::string("acquire") + (force ? "!" : ""));
        bool expired = fence && wall_now_ms() > fence->expires_at_ms + server::kFenceSkewTolerance.count();
    ...
    Result<std::vector<std::pair<std::string, std::string>>> list_nodes() override {
        if (fail_list != Errno::kOk) return Err(fail_list);
        return std::vector<std::pair<std::string, std::string>>(node_addresses.begin(), node_addresses.end());
    }
    Result<std::optional<server::FenceRecord>> read_fs_fence(uint32_t fsid) override {
        ...
        for (const auto& [node, rec] : fences) {
            ...
            if (!best || (best_expired && (!fences_expired(rec) || c.expires_at_ms > best->expires_at_ms))) best = c;
        }
        return best;
    }
    Result<server::FenceRecord> acquire_fs_fence(uint32_t fsid, std::string_view node, uint64_t e,
        ...
        for (auto& [other, rec] : fences)
            if (other != node && hold_of(rec, fsid) && !force && !fences_expired(rec)) return Err(errno_from(EBUSY));
        for (auto& [other, rec] : fences)
    ...
    Result<std::optional<server::CatalogDoc>> read_catalog() override {
        ...
        return std::optional<server::CatalogDoc>{server::CatalogDoc{catalog_version(), catalog_docs.rbegin()->second}};
    }
    Result<uint64_t> write_catalog(uint64_t expected, std::string_view text) override {
        ...
        while (catalog_docs.size() > server::kCatalogHistoryKeep + 1) catalog_docs.erase(catalog_docs.begin());
        return version;
    }
    ...
    Result<void> put_catalog_applied(const server::CatalogApplied& applied) override {
        ...
        if (applied.node.empty() || applied.node == "toml" || applied.node == "lock" || applied.node == "history" ||
            applied.digest.find(' ') != std::string::npos)
