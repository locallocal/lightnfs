...
| READDIR cookie 策略 | `[[export]] strict_readdir_cookies` | **默认 true**：verifier 取目录 change 属性，分页期间目录被改即让客户端从头重列——不重复、不漏项（设计 04 §4.2 的选择）。代价是**持续高频变动的大目录上 `ls` 可能不收敛**。置 false 则回全 0 verifier 且从不校验（knfsd 在多数文件系统上的行为，POSIX `readdir()` 本就允许跨页重复/漏项）。用 `lightnfs_v3_bad_cookie_total` / `lightnfs_v4_readdir_not_same_total` 判断这条策略有没有在实际付代价 |
