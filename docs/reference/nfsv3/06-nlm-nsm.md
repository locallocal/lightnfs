...
**实现决定**：lightnfs 采用选项 1——不注册 100021/100024，v3 无字节锁（设计决策 D8；v3 挂载用 `-o nolock`），并在部署文档明示"v3 写不受 v4 锁/deny 约束"的边界。"锁表抽象"以 v4 的 `LockMgr`（`src/state/lock_mgr.hpp`）落地，并可下推到集群后端（gluster/lustre/cephfs 的 `native_locks()`）；若将来出现 v3 锁刚需，选项 2 可在其上补 NLM 过程（保留为按需项）。
