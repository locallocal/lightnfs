...
- ✅ v4.2 低成本特性：SEEK/ALLOCATE/DEALLOCATE、同步同服 COPY、CLONE（宣告 minorversion=2）；其后 READ_PLUS / cookieverf 语义化随 plan doc 10 §5.1 落地
- ✅ 读委托 + 回传通道发送侧（nfsv4/05 §5.6 阶段 2，2026-08-28，plan doc 10 §5.2）
- ✅ 第二后端启动（GlusterFS/libgfapi，按 06 分册 §6.6 映射表实现，2026-09-03）——**接口冻结的真实检验**：接口零改动跑通（仅两处可选扩展），plan doc 10 §5.3
- ✅ 第三后端 Lustre（FID 句柄 / HSM jukebox / OFD 原生锁，06 分册 §6.5，2026-09-04）
- ✅ 第四后端 CephFS（libcephfs；原生 change 计数 + MDS 仲裁锁——多网关一致性两个前提首次同时具备，06 分册 §6.8，2026-09-04）
...
| 后端接口第一次真实扩展时返工 | 06 分册两张映射表作为接口评审的验收材料；5.10 演进规则约束。**已验证**：三个集群后端接入接口零改动 |
