...
11. [共享导出清单（集群级集中式导出配置，设计稿）](11-shared-export-catalog.md) —— 把 `[[export]]` 从每台网关的本地 TOML 上移到 `shared_dir/catalog.toml`（带版本、历史、CAS 写入），本地只留身份与 `[backend_defaults]` 本机键；`lightnfs-ctl cluster catalog …` / `cluster export add|set|remove` 改清单；网关按 `catalog_refresh = auto|manual` 跟进；不重启地增删导出、改 `nodes`（不可变导出集快照 + RCU 发布）。`exports_source = "catalog"` 启用，默认 `local` 零变化
12. [共享导出清单——实现步骤拆分](12-shared-export-catalog-steps.md) —— 11 的实施计划（阶段 A–E：清单文档与存储 → 运行期可变导出集 → 启动与跟进 → 管理命令 → 验收与文档），每步带代码锚点、测试与验收标准；完成后撤下
