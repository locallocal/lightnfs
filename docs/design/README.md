...
10. [优化与功能落地记录](10-optimization-plan.md) —— v1 完成后的全量代码审查与 v1.x→v2 工作清单（缺陷修复、性能、可观测性/运维、协议功能、集群后端、重构、测试工程化）；全部条目已落地，保留为实现记录与代码注释 "plan doc 10 §x" 的引用落点
...
| D8 | 委托/pNFS | 读委托 + 回传通道已实现（plan doc 10 §5.2）；不做写/目录委托，不支持 pNFS | 04/09 |
