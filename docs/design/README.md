...
11. [多网关多活（每导出一个活动网关，方案）](11-multi-gateway-active-active.md) —— 09 的演进：per-fsid 围栏/epoch/grace、伪根 + fs_locations + NFS4ERR_MOVED 把客户端引到每个导出的属主网关、导出级故障接管与计划内迁移；CephFS 天生契合，v3/老客户端边界写明。实施计划见 12 册
12. [多网关多活——实现步骤拆分](12-multi-gateway-active-active-steps.md) —— 11 的实施计划：阶段 A–E（配置/per-fsid 存储与 grace → fs_locations 与 NFS4ERR_MOVED → per-fsid 控制器与接管 → 计划内迁移 → 三实例验收），每步带代码锚点、测试与验收标准；含本册对 11 册未定点的实现细化
