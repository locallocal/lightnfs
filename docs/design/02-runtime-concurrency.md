...
- 压测基准：echo 服务器（传输层）、null-RPC（L2）、伪后端（L4 以上全链路，memory 后端零延迟）——三层基准锁定各层开销预算；随 `lightnfs-ctl bench echo|nullrpc|fullpath` 交付，`scripts/bench_gate.sh` 对照 `tools/bench/baseline.txt` 做地板门禁（用法见 [performance/benchmarks.md](../testing/benchmarks.md)）。
