...
> **已实现（2026-09-06）**：完整方案与代码锚点见 [10 册](10-multi-gateway-active-active.md)（原 11 册，
> 其实施步骤原属 12 册，A–E 全部完成后与本册的步骤文档同样撤下，每步实现记录见 git 历史，未闭环项
...
> failover`（本册）与 `active-active`（10 册）是同一 `[cluster]` 段的两种互斥形态。
