#!/usr/bin/env bash
# Phase-4 (M6) kernel-mount acceptance on a root VM (development plan §6.3): real
...
#
# usage: accept_m4_vm.sh [FSX_OPS]   (overnight: FSX_OPS=2000000)
set -euo pipefail
...
lease=${LNFS_LEASE:-10}
work=$(mktemp -d "${TMPDIR:-/tmp}/lnfs-m4vm.XXXXXX")
data="$work/data"
mnt4="$work/mnt4"
mnt3="$work/mnt3"
state="$work/state"
...
echo "== cthon04 basic/general/special (vers=4.1)"
...
server_pid=""
echo
echo "M4 VM mount acceptance PASSED (vers=4.1 rw: cthon b/g/s + fsx $fsx_ops + restart reclaim + lease reclaim + v3/v4 mixed)"
