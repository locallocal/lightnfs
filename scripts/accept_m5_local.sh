#!/usr/bin/env bash
# Phase-5 (M7) loopback acceptance — no root, no kernel mount (development plan §6.3,
...
lease=${LNFS_LEASE:-3}
work=$(mktemp -d "${TMPDIR:-/tmp}/lnfs-m5.XXXXXX")
...
run_phase() {  # $1 build dir, $2 label
  ...
  local state="/tmp/lnfs-m5-state-$label" log="$work/server-$label.log"
...
echo
echo "M5 (v4.1 locks + security) loopback acceptance PASSED"
