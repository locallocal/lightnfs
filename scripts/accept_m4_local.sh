#!/usr/bin/env bash
# Phase-4 (M6) loopback acceptance — no root, no kernel mount (development plan §6.3,
...
#   pynfs       4.1 open/rename/currentstateid/verify/courteous groups plus the
#               phase-3 session groups (regression)
#   asan        the protocol phases again under ASAN, leak-checked graceful exit
#
# usage: accept_m4_local.sh
set -euo pipefail
...
lease=${LNFS_LEASE:-3}
work=$(mktemp -d "${TMPDIR:-/tmp}/lnfs-m4.XXXXXX")
...
run_phase() {  # $1 build dir, $2 label
  ...
  local state="/tmp/lnfs-m4-state-$label" log="$work/server-$label.log"
  ...
  if [[ $label == rel ]]; then
    ...
    (cd "$work/pynfs/nfs4.1" &&
     python3 ./testserver.py "127.0.0.1:$nfs_port$data" --minorversion=1 --noinit \
       ...
       exchange_id create_session destroy_session destroy_clientid sequence \
       lookup lookupp putfh compound secinfo_no_name \
    ...
    # Expected failures on this milestone, by test code (scripts/pynfs_m4_expected.txt,
    ...
    expected=$(tr '\n' ' ' < "$repo/scripts/pynfs_m4_expected.txt")
    unexpected=""
...
echo
echo "M4 (v4.1 read-write + full state) loopback acceptance PASSED"
