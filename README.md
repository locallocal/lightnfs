...
| **MOUNTv3** | MNT (with the mount point checked to be a directory), UMNT, EXPORT with export-table ACLs by CIDR. DUMP and UMNTALL answer, but the server keeps no authoritative rmtab, so DUMP always reports an empty mount list. |
